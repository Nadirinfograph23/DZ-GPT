...
export default async function handler(req, res) {
  ...
  let app
  try { const { app: importedApp } = await import('../server.js'); app = importedApp }
  catch (err) { console.error('[Vercel] server.js import FAILED:', err?.message); app = (_req, res) => res.status(500).json({ error: 'Server startup failed', message: err?.message, stack: err?.stack?.split('\n').slice(0, 15) }) }
  return app(req, res)
}