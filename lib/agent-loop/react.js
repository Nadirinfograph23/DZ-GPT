...
function buildSystemPrompt(hasGithubToken) {
  const tokenStatus = hasGithubToken
    ? '✅ GITHUB_TOKEN متاح — يمكن تنفيذ جميع العمليات مباشرةً'
    : '⚠️ GITHUB_TOKEN غير متاح — بعض العمليات ستفشل'
 ...
 */
export async function runReActLoop({ query, messages, aiGenerate, onStep, signal }) {
  const hasGithubToken = !!process.env.GITHUB_TOKEN
  const systemPrompt = buildSystemPrompt(hasGithubToken)
  const steps = []
  ...
  emit({ type: 'start', message: 'بدء ReAct Agent Loop...' })
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    if (parsed.type === 'tool_call') {
      ...
      let observation
      try {
        observation = await executeTool(parsed.tool, parsed.args)
      } catch (err) {
