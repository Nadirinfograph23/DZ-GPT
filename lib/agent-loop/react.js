...
// ── Context compression: شغّال عندما يكبر loopMessages ─────────────────────
// يُلخّص الملاحظات القديمة بدلاً من قطعها — يحافظ على الذاكرة الكاملة للوكيل
const MAX_LOOP_MSGS = 30
function compressLoopHistory(messages) {
  const system = messages.filter(m => m.role === 'system')
  const history = messages.filter(m => m.role !== 'system')
  if (history.length <= MAX_LOOP_MSGS) return messages

  // Keep system + first 2 turns (الطلب الأصلي) + last 16 turns (السياق الأحدث)
  const head = history.slice(0, 2)
  const tail = history.slice(-16)
  const droppedCount = history.length - head.length - tail.length

  const summaryMsg = {
    role: 'user',
    content: `[ملخص السياق المضغوط — ${droppedCount} رسالة محذوفة للحفاظ على الذاكرة. النتائج المهمة حُفظت في agentState أدناه. واصل التنفيذ.]`,
  }
  return [...system, ...head, summaryMsg, ...tail]
}

// ── Agent State Accumulator: يتراكم عبر كل iterations ─────────────────────
// يحتفظ بالنتائج الحرجة: مستودعات، ملفات، URLs، commit SHAs
function createAgentState() {
  return {
    repos: [],        // مستودعات مكتشفة
    files_pushed: [], // ملفات رُفعت
    urls: [],         // URLs منتجة (GitHub Pages, etc.)
    commits: [],      // SHA commits
    errors: [],       // أخطاء حدثت
  }
}

function updateAgentState(state, toolName, result) {
  if (!result || result.error) {
    if (result?.error) state.errors.push({ tool: toolName, err: result.error.slice(0, 100) })
    return
  }
  if (toolName === 'list_repos' && Array.isArray(result)) {
    state.repos = result.slice(0, 10).map(r => r.full_name || r.name)
  }
  if ((toolName === 'push_file' || toolName === 'push_files_batch') && result.commit?.sha) {
    state.commits.push(result.commit.sha.slice(0, 12))
    if (result.content?.path) state.files_pushed.push(result.content.path)
  }
  if (toolName === 'get_pages_status' && result.html_url) {
    state.urls.push(result.html_url)
  }
  if (toolName === 'enable_pages' && result.html_url) {
    state.urls.push(result.html_url)
  }
}

function buildStateHeader(state) {
  const parts = []
  if (state.repos.length)        parts.push(`📦 مستودعات: ${state.repos.join(', ')}`)
  if (state.files_pushed.length) parts.push(`📄 ملفات رُفعت: ${state.files_pushed.join(', ')}`)
  if (state.urls.length)         parts.push(`🔗 URLs: ${state.urls.join(', ')}`)
  if (state.commits.length)      parts.push(`✅ Commits: ${state.commits.join(', ')}`)
  if (state.errors.length)       parts.push(`⚠️ أخطاء: ${state.errors.map(e => e.err).join(' | ')}`)
  return parts.length ? `\n\n**[Agent State]**\n${parts.join('\n')}` : ''
}
...
const TOOL_SCHEMAS = {
  ...
  search_code: {
    desc: 'Search for a text/pattern inside all files in a GitHub repo (grep-like)',
    params: { repo: 'owner/repo (required)', query: 'text to search (required)' },
  },
  find_files: {
    desc: 'Find files by name pattern in a GitHub repo (e.g. "*.tsx", "App*")',
    params: { repo: 'owner/repo (required)', pattern: 'filename pattern (required)', branch: 'branch (default: main)' },
  },
  get_commits: {
    desc: 'Get the commit history of a GitHub repo',
    params: { repo: 'owner/repo (required)', branch: 'branch (default: main)', per_page: 'number of commits (default: 10, max: 30)' },
  },
  delete_file: {
    desc: 'Delete a file from a GitHub repo',
    params: { repo: 'owner/repo (required)', path: 'file path (required)', message: 'commit message', branch: 'branch (default: main)' },
  },
  get_tree: {
    desc: 'Get the full directory tree of a GitHub repo',
    params: { repo: 'owner/repo (required)', path: 'subfolder to show (optional)', branch: 'branch (default: main)' },
  },
  get_diff: {
    desc: 'Compare two branches or commits in a GitHub repo (real diff)',
    params: { repo: 'owner/repo (required)', base: 'base branch/commit (required)', head: 'head branch/commit (required)' },
  },
  create_issue: {
    desc: 'Create a new GitHub issue in a repo',
    params: { repo: 'owner/repo (required)', title: 'issue title (required)', body: 'issue body', labels: 'array of label strings' },
  },
  close_issue: {
    desc: 'Close an existing GitHub issue by number',
    params: { repo: 'owner/repo (required)', issue_number: 'issue number (required)' },
  },
  get_actions: {
    desc: 'Get recent GitHub Actions workflow runs for a repo',
    params: { repo: 'owner/repo (required)', per_page: 'number of runs (default: 5, max: 10)' },
  },
  create_release: {
    desc: 'Create a GitHub release for a repo',
    params: { repo: 'owner/repo (required)', tag: 'tag name e.g. v1.0.0 (required)', name: 'release title', body: 'release notes', prerelease: 'boolean' },
  },
  get_pr_files: {
    desc: 'Get the files changed in a Pull Request (for AI code review)',
    params: { repo: 'owner/repo (required)', pull_number: 'PR number (required)' },
  },
}
...
function parseLLMOutput(text) {
  ...
  // Strategy 7: JSON repair — fix common LLM JSON mistakes before giving up
  const jsonLike = text.match(/\{[\s\S]{10,800}\}/)
  if (jsonLike) {
    try {
      const repaired = jsonLike[0]
        .replace(/,\s*([}\]])/g, '$1')          // trailing commas
        .replace(/(['"])?([a-zA-Z_]+)\1\s*:/g, '"$2":')  // unquoted keys
  ...
  // Strategy 8: text_only — return text content for loop to decide
...
export async function runReActLoop({ query, messages, aiGenerate, githubToken, onStep, signal }) {
  ...
  // ── Guard: no token → use chat-only mode (no GitHub tools) ───────────────
  const isGithubActionQuery = GITHUB_ACTION_PATTERNS.some(p => p.test(query)) ||
    (/\bgithub\b/i.test(query) && /\b(create|push|add|delete|update|list|show|read|deploy|merge|clone|fork|commit|انشئ|ارفع|احذف|عدّل|اعرض|نشر|رفع|ادفع)\b/i.test(query))

  if (!hasGithubToken) {
    if (isGithubActionQuery) {
      const noTokenMsg = `😎 لتحت اختار **وكيل** وسجّل الدخول إلى GitHub`
      emit({ type: 'error', message: 'لا يوجد GitHub token' })
      return { content: noTokenMsg, steps, model: null }
    }
    // Chat-only mode: no GitHub tools — just AI reasoning
    emit({ type: 'thinking', iteration: 0, message: 'وضع التفكير المتعدد الخطوات...' })
    try {
      const chatResult = await aiGenerate({ messages, query, max_tokens: 4096 })
      emit({ type: 'done', message: 'اكتملت الإجابة', content: chatResult?.content })
      return { content: chatResult?.content || '', steps, model: chatResult?.model || 'chat-react' }
    } catch (chatErr) {
      emit({ type: 'error', message: chatErr.message })
      return { content: `⚠️ خطأ: ${chatErr.message}`, steps, model: null }
    }
  }
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    // ضغط السياق تلقائياً عندما يكبر كثيراً
    const compressed = compressLoopHistory(loopMessages)
    if (compressed.length < loopMessages.length) {
      loopMessages.splice(0, loopMessages.length, ...compressed)
      emit({ type: 'thinking', iteration, message: `🗜️ ضغط السياق: ${compressed.length} رسالة` })
    }
    ...
    if (parsed.type === 'tool_call') {
      ...
      updateAgentState(agentState, parsed.tool, observation)
      const stateHeader = buildStateHeader(agentState)
      const obsLimit = parsed.tool === 'push_files_batch' ? 5000 : parsed.tool === 'read_file' ? 4000 : 3000

      loopMessages.push(
        { role: 'assistant', content: llmOutput },
        {
          role: 'user',
          content: `**نتيجة الأداة "${parsed.tool}":**\n\`\`\`json\n${observationStr.slice(0, obsLimit)}\n\`\`\`${stateHeader}\n\nواصل التنفيذ. إذا انتهيت من كل المطلوب، أعطِ \`\`\`final_answer\`\`\`. إذا تحتاج خطوة أخرى، استخدم \`\`\`tool_call\`\`\`.`,
        }
      )
      continue
    }
...
const REACT_BLOCK_PATTERNS = [
  ...
  // ── استعلامات معلوماتية عن مستودعات GitHub (بحث/إرشاد لا تنفيذ) ───────────
  // "اعطني مستودعات github مجانية لتوليد الصور" → يجيب AI بمعلومات لا يُنفّذ كوداً
  /(?:اعطني|أعطني|اقترح|recommend|قائمة|list)\s+مستودعات/i,
  /مستودعات\s*(?:مجانية|مجاني|free|لتوليد|لتطوير|لبناء|للذكاء|للـ|for\b)/i,
  /(?:free|مجاني|مجانية)\s+(?:مستودعات|repos?)\s*(?:github|git)?/i,
  /github\s+repos?\s+(?:for|لـ|لتوليد|مجاني)/i,
  /أنواع\s+مستودعات|مقارنة\s+مستودعات|مستودعات\s+(?:رائعة|مفيدة|مشهورة)/i,
]
