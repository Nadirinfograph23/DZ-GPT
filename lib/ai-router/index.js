...
async function tryGroqCoder(messages, max_tokens) {
  ...
  try {
    ...
    if (validateContent(content)) {
      markGroqSuccess(apiKey, Date.now() - t0)
      console.log('[ai-router] ✓ Groq Coder (qwen-2.5-coder-32b)')
    ...
    console.warn('[ai-router] Groq Coder failed:', msg.slice(0, 80))
    throw e
  }
}
...
async function tryGroqReasoning(messages, max_tokens) {
  ...
  try {
    ...
    if (validateContent(content)) {
      markGroqSuccess(apiKey, Date.now() - t0)
      console.log('[ai-router] ✓ Groq Reasoning (deepseek-r1-distill-llama-70b)')
    ...
    console.warn('[ai-router] Groq Reasoning failed:', msg.slice(0, 80))
    throw e
  }
}
