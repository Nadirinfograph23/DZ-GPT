...
async function tryOpenRouter(messages, max_tokens) {
  ...
  try {
    const data = await postJSON(
      ...
      { model: 'meta-llama/llama-3.3-70b-instruct:free', messages, max_tokens: Math.min(max_tokens, 8192) },
      28000,
    )
    const content = data.choices?.[0]?.message?.content || null
    if (validateContent(content)) {
...
const CAPABILITY_ORDER = {
  ...
  // Default sequential order (original behavior)
  general:      ['openai', 'groq', 'gemini', 'mistral', 'nvidia', 'cohere', 'openrouter'],
}
...
export async function callAIRouter(messages, { max_tokens = 2000, requestId, taskHint = 'general' } = {}) {
  ...
  for (const { name, fn } of providers) {
    ...
    try {
      ...
      // null result (key missing) — provider skipped silently
...
export function getProviderStatus() {
  return [
    ...
    { name: 'groq',        available: !!process.env.GROQ_API_KEY, state: 'closed' },
...
export { getProviderScores, getRouterLogs, getRouterDiagnosticSummary, generateRequestId }
