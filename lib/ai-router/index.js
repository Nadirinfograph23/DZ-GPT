...
// ── OpenRouter Free Tier — يحل محل DeepSeek (نماذج مجانية عالية الجودة) ─────
// meta-llama/llama-3.3-70b-instruct:free هو الأقوى في الفئة المجانية
...
async function tryOpenRouter(messages, max_tokens) {
  ...
  try {
    ...
    const content = data.choices?.[0]?.message?.content || null
    if (validateContent(content)) {
      logger.info('[ai-router] ✓ OpenRouter')
      return { content, model: 'openrouter:llama-3.3-70b', truncated: isTruncated(content) }
    }
    return null
  } catch (e) {
    logger.warn('[ai-router] OpenRouter failed:', e.message)
    throw e
  }
}
