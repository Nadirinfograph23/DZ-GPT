...
export async function streamAIResponse(res, messages, { maxTokens, isComplex } = {}) {
  ...
  // 🔁 ReAct Loop — أرسل البادئة كأول token قبل بدء البث
  const _lastMsg = [...(messages || [])].reverse().find(m => m.role === 'user')?.content || ''
  const _isShort = _lastMsg.length < 40
  const _isStructured = /^(```|\{|\[|##\s*🌤️|##\s*⚽)/i.test(_lastMsg)
  if (!_isShort && !_isStructured) {
    writeSSE(res, { token: '🤔 **راني نخمم أصبر شوية...**\n\n' })
  }
