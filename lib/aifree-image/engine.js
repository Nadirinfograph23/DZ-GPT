 ...
 *
 * Both providers are called server-side — no browser exposure.
 */

const HF_TOKEN = process.env.HF_TOKEN || process.env.HUGGINGFACE_API_KEY || ''

// ── Model routing table ───────────────────────────────────────────────────────
// Each entry: { id, label, badge, provider, hfModel?, pollinationsModel? }
export const MODELS = [
  // ── Pollinations native (fast, free, reliable) ────────────────────────────
  ...
  { id: 'gptimage',        label: '✨ GPT Image',         badge: 'GPT',       provider: 'pollinations', pollinationsModel: 'gptimage' },

  // ── HuggingFace Inference (works from Vercel, fallback to Pollinations) ──
  { id: 'flux-schnell',    label: '⚡ FLUX Schnell',      badge: 'FAST',      provider: 'hf', hfModel: 'black-forest-labs/FLUX.1-schnell',         pollinationsModel: 'flux' },
  ...
  { id: 'seedream',        label: '🌱 Seedream',          badge: 'ByteDance', provider: 'hf', hfModel: 'ByteDance/Seedream-3',                      pollinationsModel: 'flux' },
  { id: 'nano-banana-pro', label: '🍌 Nano Banana Pro',  badge: 'PRO',       provider: 'hf', hfModel: 'Efficient-Large-Model/Sana_1600M_1024px_diffusers', pollinationsModel: 'turbo' },
]
...
export function getModels() {
  return MODELS.map(({ id, label, badge }) => ({ id, label, badge }))
}
