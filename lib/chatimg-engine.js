...
export async function generateWithChatIMG(prompt, {
  ...
  try {
    ...
    if (preferredModel === 'imgcreator') {
      // Nano Banana 2 (ImgCreatorAI) → Pollinations مباشرةً (9-12 ثانية مضمونة)
      ...
      // GPT Image 2 (ImgCreatorAI) → Pollinations مباشرةً
      ...
      // Stable Horde (جودة عالية ~60-120 ث) → Pollinations ضمان
      result = await tryStableHorde(enhanced, width, height)
        || await tryPollinations(enhanced, width, height)
    } else {
      // Auto + كل النماذج الأخرى: Pollinations مباشرةً (سريع ومضمون)
