      ...
      for (const p of parts) {
        if (p?.inlineData?.data) {
          ...
          if (buf.length > 2000) {
            ...
            return { buf, mime, model: 'DZ MEDIA PRO Nano', provider: 'DZ MEDIA PRO' }
          }
        }
      }
    } catch (e) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (ct.startsWith('image/') && buf.length > 2000) {
          ...
          return { buf, mime: ct, model: 'DZ MEDIA FLUX', provider: 'DZ MEDIA' }
        }
      } else {
    ...
    try {
      ...
      if (r.ok && ct.startsWith('image/')) {
        ...
        if (buf.length > 2000) {
          ...
          return { buf, mime: ct, model: 'DZ MEDIA BASIC', provider: 'DZ MEDIA BASIC' }
        }
      }
    } catch (e) {
    ...
    if (r.ok) {
      ...
      if (b64) {
        ...
        if (buf.length > 2000) {
          ...
          return { buf, mime: 'image/png', model: 'DZ MEDIA PRO Vision', provider: 'DZ MEDIA PRO' }
        }
      }
    } else {
...
export const CHATIMG_MODELS = [
  { id: 'auto',        label: '🤖 DZ MEDIA Auto',         badge: 'AUTO',  tier: 'fast',    group: 'DZ MEDIA',       desc: 'أسرع مزود متاح تلقائياً' },
  { id: 'nano-banana', label: '🍌 DZ MEDIA PRO Nano',      badge: 'PRO',   tier: 'fast',    group: 'DZ MEDIA PRO',   desc: 'جودة 4K عالية مجاناً' },
  { id: 'gpt-image-2', label: '🤖 DZ MEDIA PRO Vision',   badge: 'PRO',   tier: 'premium', group: 'DZ MEDIA PRO',   desc: 'نموذج الرؤية المتقدم' },
  { id: 'hf',          label: '⚡ DZ MEDIA FLUX',          badge: 'FAST',  tier: 'fast',    group: 'DZ MEDIA',       desc: 'سريع ومجاني' },
]
