...
// ═══════════════════════════════════════════════════════════════════════════════
// PROVIDER C2: Gemini 2.0 Flash Image Generation — مجاني (free tier)
// يعمل من أي IP — موثوق — ~5-15 ثانية
// ═══════════════════════════════════════════════════════════════════════════════
async function tryGemini(prompt, width, height) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      console.warn(`[dzmedia:gemini] HTTP ${r.status}: ${errText.slice(0, 120)}`)
      return null
    }
    const data = await r.json()
    const parts = data?.candidates?.[0]?.content?.parts || []
    for (const part of parts) {
      if (part.inlineData?.mimeType?.startsWith('image/') && part.inlineData.data) {
        ...
        if (buf.length > 2000) {
          ...
          return { buf, mime: part.inlineData.mimeType, model: 'Gemini Flash Image', provider: 'DZ MEDIA PRO' }
        }
      }
    }
    ...
    clearTimeout(timer)
    if (e.name !== 'AbortError') console.warn('[dzmedia:gemini]', e.message)
  }
  return null
}
...
export async function generateWithChatIMG(prompt, {
  ...
  try {
    ...
    if (preferredModel === 'imgcreator') {
      // Nano Banana 2: ImgCreatorAI → Gemini مضمون → Pollinations
      ...
      // GPT Image 2: ImgCreatorAI → Gemini مضمون → Pollinations
      ...
      // Stable Horde (جودة عالية ~60-120 ث) → Gemini سريع كـ backup
      result = await tryStableHorde(enhanced, width, height)
        || await tryGemini(enhanced, width, height)
        || await tryPollinations(enhanced, width, height)
    } else {
      // Auto: Pollinations سريع → Gemini مضمون → retry Pollinations
