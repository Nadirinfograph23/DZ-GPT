...
async function tryImgCreatorAI(prompt, width, height, preferModel = null) {
  ...
  for (const { id: modelId, label: modelLabel } of models) {
    ...
    if (genResult.quota) {
      // Try fresh session once (new cookies = potentially new credits)
      ...
      if (!retry || retry.quota || retry.loginRequired || !retry.taskId) continue
      const result = await _icPoll(fresh.csrf, fresh.cookies, retry.taskId, modelLabel)
