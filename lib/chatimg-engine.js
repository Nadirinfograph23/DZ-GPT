 ...
 * مزودون مجانيون مُختبرون فعلياً: Pollinations · Stable Horde · Gemini · HuggingFace
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
async function tryPollinations(prompt, width, height, attempt = 0, model = 'flux') {
  ...
  try {
    ...
    if (r.ok && ct.startsWith('image/')) {
      const buf = Buffer.from(await r.arrayBuffer())
      if (buf.length > 2000) {
        console.log(`[dzmedia:pollinations/${model}] ✅ ${buf.length.toLocaleString()} bytes`)
        return { buf, mime: ct, model: `DZ Image (${model})`, provider: 'DZ MEDIA' }
      }
    } else {
      console.warn(`[dzmedia:pollinations/${model}] HTTP ${r.status}`)
    }
  } catch (e) {
    clearTimeout(timer)
    if (e.name !== 'AbortError') console.warn(`[dzmedia:pollinations/${model}]`, e.message)
  }
  return null
}
...
export async function generateWithChatIMG(prompt, {
  ...
  try {
    ...
    if (preferredModel === 'turbo') {
      result = await tryPollinations(enhanced, width, height, 0, 'turbo')
          ||   await tryPollinations(enhanced, width, height, 1, 'flux')
          ||   await tryGemini(enhanced, width, height)

    } else if (preferredModel === 'flux-realism') {
      result = await tryPollinations(enhanced, width, height, 0, 'flux-realism')
          ||   await tryPollinations(enhanced, width, height, 1, 'flux')
          ||   await tryGemini(enhanced, width, height)

    } else if (preferredModel === 'flux-anime') {
      result = await tryPollinations(enhanced, width, height, 0, 'flux-anime')
          ||   await tryPollinations(enhanced, width, height, 1, 'flux')
          ||   await tryGemini(enhanced, width, height)

    } else if (preferredModel === 'flux-3d') {
      result = await tryPollinations(enhanced, width, height, 0, 'flux-3d')
          ||   await tryPollinations(enhanced, width, height, 1, 'flux')
          ||   await tryGemini(enhanced, width, height)

    } else if (preferredModel === 'flux-cablyai') {
      result = await tryPollinations(enhanced, width, height, 0, 'flux-cablyai')
          ||   await tryPollinations(enhanced, width, height, 1, 'flux-realism')
          ||   await tryPollinations(enhanced, width, height, 2, 'flux')
          ||   await tryGemini(enhanced, width, height)

    } else if (preferredModel === 'playground') {
      result = await tryPollinations(enhanced, width, height, 0, 'playground-v2')
          ||   await tryPollinations(enhanced, width, height, 1, 'flux')
          ||   await tryGemini(enhanced, width, height)
      ...
      result = await tryGemini(enhanced, width, height)
          ...
          ||   await tryPollinations(enhanced, width, height, 0, 'flux')

    } else if (preferredModel === 'horde') {
      // Stable Horde — حصة خاصة 10/ساعة لكل مستخدم
      ...
      result = await tryStableHorde(enhanced, width, height)
          ||   await tryPollinations(enhanced, width, height, 0, 'flux')
          ||   await tryGemini(enhanced, width, height)
      if (result) consumeHordeRate(ip)

    } else {
      // auto: FLUX سريع → HF → Gemini → retry
      result = await tryPollinations(enhanced, width, height, 0, 'flux')
          ||   await tryHuggingFace(enhanced, width, height)
          ||   await tryGemini(enhanced, width, height)
          ||   await tryPollinations(enhanced, width, height, 1, 'turbo')
    }
...
// ── قائمة النماذج المتاحة (مجانية ومُختبرة فعلياً) ───────────────────────────
export const CHATIMG_MODELS = [
  ...
  { id: 'gemini',       label: '🍌 Gemini Flash Image',      badge: 'PRO',   tier: 'fast',   group: 'DZ MEDIA PRO', desc: 'Gemini 2.0 Flash — جودة فائقة مجاني مع API key',     waitSecs: 20  },
