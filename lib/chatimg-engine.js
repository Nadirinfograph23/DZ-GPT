...
// ═══════════════════════════════════════════════════════════════════════════════
// PROVIDER C: DZ MEDIA BASIC — Pollinations (مجاني دائماً — بدون model param)
// ملاحظة: FLUX models أصبحت مدفوعة (402). نستخدم النماذج المجانية فقط.
// ═══════════════════════════════════════════════════════════════════════════════
// Free models on Pollinations (no payment required):
const POLL_FREE_MODELS = [
  '',          // default (sana) — always free
  'turbo',     // free tier
  'sana',      // explicitly free
]
    ...
    try {
      ...
      if (r.ok && ct.startsWith('image/')) {
        ...
        if (buf.length > 2000) {
          ...
          return { buf, mime: ct, model: `Pollinations (${label})`, provider: 'DZ MEDIA BASIC' }
        }
      } else {
...
async function getImgCreatorSession(forceRefresh = false) {
  ...
  // Try multiple entry points for session (site may redirect)
  const ENTRY_URLS = [
    'https://imgcreatorai.io/nanobanana',
...
// Generate endpoints to try in order (site may have restructured routes)
const IC_GENERATE_ENDPOINTS = [
  { url: 'https://imgcreatorai.io/nanobanana/generate-guest', pageId: 'nanobanana_page' },
  { url: 'https://imgcreatorai.io/generate-guest',            pageId: 'nanobanana_page' },
  { url: 'https://imgcreatorai.io/api/generate-guest',        pageId: 'nanobanana_page' },
]

async function _icGenerate(csrf, cookies, prompt, aspect, model) {
  for (const { url: genUrl, pageId } of IC_GENERATE_ENDPOINTS) {
    const ac = new AbortController()
    ...
    try {
      const r = await fetch(genUrl, {
        ...
        body:    JSON.stringify({ prompt, aspect_ratio: aspect, model, resolution: '1K', pageId }),
        signal:  ac.signal,
      })
      clearTimeout(timer)

      // 404 = wrong endpoint, try next
      if (r.status === 404) { console.warn(`[imgcreator:${model}] 404 at ${genUrl} — trying next`); continue }
      ...
      if (r.status === 403 || r.status === 401) {
        console.warn(`[imgcreator:${model}] ${r.status} auth required at ${genUrl}`)
        return { loginRequired: true }
      }
      if (!r.ok) {
        console.warn(`[imgcreator:${model}] HTTP ${r.status} at ${genUrl}:`, data.message || '')
        continue
      }
      // success — task_id may be nested differently
      const taskId = data.task_id || data.taskId || data.id || data.data?.task_id
      if (!data.success && !taskId) {
        console.warn(`[imgcreator:${model}] no success/taskId at ${genUrl}:`, JSON.stringify(data).slice(0, 120))
        continue
      }
      ...
      clearTimeout(timer)
      if (e.name !== 'AbortError') console.warn(`[imgcreator:${model}] ${genUrl}:`, e.message)
    }
  }
  return null
}

// Poll endpoints to try in order
const IC_POLL_PATHS = [
  (id) => `https://imgcreatorai.io/query/${id}`,
  (id) => `https://imgcreatorai.io/nanobanana/query/${id}`,
  (id) => `https://imgcreatorai.io/api/query/${id}`,
]
...
export async function generateWithChatIMG(prompt, {
  ...
  try {
    ...
    if (preferredModel === 'imgcreator') {
      // nano-banana-2 first, then Gemini → Pollinations fallback
      ...
      // gpt-image-2 first, then nano-banana-2 → Gemini fallback
      ...
      // Auto: race الأسرع بين Gemini و Pollinations
