...
// ═══════════════════════════════════════════════════════════════════════════════
// PROVIDER C: DZ MEDIA BASIC — Pollinations (مجاني دائماً — متعدد النماذج)
// مُختبر: flux-schnell, sdxl-turbo, playground-v25, sana كلها تعمل مجاناً
// ═══════════════════════════════════════════════════════════════════════════════
// Fast free models on Pollinations (tested 2026-06, all return <1s):
const POLL_FREE_MODELS = [
  'flux-schnell',      // سريع جداً — FLUX.1 schnell مجاني
  'sdxl-turbo',        // SDXL Turbo — مجاني وسريع
  'playground-v25',    // Playground v2.5 — مجاني وسريع
  'sana',              // Sana model — مجاني
  '',                  // default (sana) — fallback أبطأ لكن موثوق
]

async function tryPollinations(prompt, width, height, _modelHint = 'flux', attempt = 0) {
  ...
  for (const model of POLL_FREE_MODELS) {
    const modelParam = model ? `&model=${model}` : ''
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}`
      + `?width=${width}&height=${height}&seed=${seed}&nologo=true&enhance=false&private=true&_n=${nonce}${modelParam}`
    // Fast named models return in <1s; default (sana) can take up to 90s
    const timeoutMs = model ? 20_000 : 90_000
    const ac    = new AbortController()
    ...
    try {
      ...
      clearTimeout(timer)
      if (r.status === 402) {
        console.warn(`[dzmedia:basic] 402 on model="${model || 'default'}" — trying next`)
        await delay(300)
        continue
      }
      if (r.ok && ct.startsWith('image/')) {
        const buf = Buffer.from(await r.arrayBuffer())
        if (buf.length > 2000) {
          const label = model || 'sana'
          console.log(`[dzmedia:basic/${label}] ✅ ${buf.length.toLocaleString()} bytes`)
          return { buf, mime: ct, model: `Pollinations ${label.toUpperCase()}`, provider: 'DZ MEDIA BASIC' }
        }
      } else {
        console.warn(`[dzmedia:basic] HTTP ${r.status} model="${model || 'default'}"`)
      }
    } catch (e) {
      clearTimeout(timer)
      if (e.name !== 'AbortError') console.warn(`[dzmedia:basic/${model || 'default'}]`, e.message)
    }
...
export async function generateWithChatIMG(prompt, {
  ...
  try {
    ...
    if (preferredModel === 'imgcreator') {
      ...
      // Auto: race بين Gemini و Pollinations (كلاهما سريع)
      const race = await Promise.any([
        tryGemini(enhanced, width, height).then(r => { if (!r) throw new Error('null'); return r }),
        tryPollinations(enhanced, width, height).then(r => { if (!r) throw new Error('null'); return r }),
      ]).catch(() => null)

      if (race) result = race
      else {
        result = await tryStableHorde(enhanced, width, height)
          || await tryImgCreatorAI(enhanced, width, height)
...
export const CHATIMG_MODELS = [
  { id: 'auto',           label: '🤖 Auto (أسرع مزود)',              badge: 'AUTO',  tier: 'fast',    group: 'DZ MEDIA',       desc: 'يختار تلقائياً أسرع مزود متاح',              waitSecs: 2    },
  { id: 'imgcreator',     label: '🍌 Nano Banana 2 (ImgCreator)',    badge: 'FREE',  tier: 'fast',    group: 'DZ MEDIA PRO',   desc: 'Gemini backend — مجاني + Horde fallback',     waitSecs: 60   },
  { id: 'imgcreator-gpt', label: '🖼️ GPT Image 2 (ImgCreator)',     badge: 'FREE',  tier: 'fast',    group: 'DZ MEDIA PRO',   desc: 'GPT-4o backend — مجاني + Horde fallback',     waitSecs: 60   },
  { id: 'horde',          label: '🌐 Stable Horde (مجاني لا نهاية)',  badge: 'FREE∞', tier: 'medium',  group: 'DZ MEDIA',       desc: 'شبكة مجتمعية مجانية بلا حدود — ~60-120ث',    waitSecs: 90   },
  { id: 'nano-banana',    label: '✨ Gemini 2.0 Flash Image',         badge: 'PRO',   tier: 'fast',    group: 'DZ MEDIA PRO',   desc: 'جودة عالية + Horde fallback',                 waitSecs: 5    },
  { id: 'hf',             label: '⚡ FLUX.1 / SDXL (HF)',             badge: 'FAST',  tier: 'fast',    group: 'DZ MEDIA',       desc: 'HuggingFace — مجاني + Horde fallback',        waitSecs: 20   },
]
