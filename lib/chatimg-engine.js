...
// Poll endpoints — tested order: nanobanana prefix first (matches generate prefix)
const IC_POLL_PATHS = [
...
async function tryImgCreatorAI(prompt, width, height, preferModel = null) {
  ...
  for (const { id: modelId, label: modelLabel } of models) {
    ...
    if (genResult.quota) {
      // IP المحلي نفد تماماً → تدوير مباشر عبر Vercel (IP مختلف = حصة جديدة)
      // لا فائدة من جلسة جديدة — المشكلة IP وليست cookies
