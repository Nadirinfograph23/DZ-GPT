...
//
// ── نماذج Dahl الفعلية (inference.dahl.global/v1) ──────────────────────────────
// MiniMax-M2.7  → سريع، متعدد اللغات، مثالي للمحادثة العامة
// Kimi-K2.6     → استدلال + سياق طويل + كود
// GLM-5.2-FP8   → متوازن بين الجودة والسرعة
const _MODEL_DEFAULTS = {
  realtime:     process.env.DAHL_MODEL_REALTIME     || 'MiniMaxAI/MiniMax-M2.7',
  general:      process.env.DAHL_MODEL_GENERAL      || 'MiniMaxAI/MiniMax-M2.7',
  multilingual: process.env.DAHL_MODEL_MULTILINGUAL || 'MiniMaxAI/MiniMax-M2.7',
  translation:  process.env.DAHL_MODEL_TRANSLATION  || 'MiniMaxAI/MiniMax-M2.7',
  reasoning:    process.env.DAHL_MODEL_REASONING    || 'moonshotai/Kimi-K2.6',
  research:     process.env.DAHL_MODEL_RESEARCH     || 'moonshotai/Kimi-K2.6',
  technical:    process.env.DAHL_MODEL_TECHNICAL    || 'moonshotai/Kimi-K2.6',
  code:         process.env.DAHL_MODEL_CODE         || 'moonshotai/Kimi-K2.6',
  retrieval:    process.env.DAHL_MODEL_RETRIEVAL    || 'zai-org/GLM-5.2-FP8',
  agent:        process.env.DAHL_MODEL_AGENT        || 'moonshotai/Kimi-K2.6',
  longcontext:  process.env.DAHL_MODEL_LONGCONTEXT  || 'moonshotai/Kimi-K2.6',
  vision:       process.env.DAHL_MODEL_VISION       || 'MiniMaxAI/MiniMax-M2.7',
  website:      process.env.DAHL_MODEL_WEBSITE      || 'moonshotai/Kimi-K2.6',
  html:         process.env.DAHL_MODEL_HTML         || 'moonshotai/Kimi-K2.6',
}
...
export async function tryDahl(messages, max_tokens = 2000, opts = {}) {
  ...
  try {
    // NOTE: circuit.onAttempt / recordSuccess / recordFailure are intentionally
    // NOT called here. The ai-router (callAIRouter) owns circuit bookkeeping for
    // all providers in its registry — including Dahl — via _isCircuitOpen /
    // _recordSuccess / _recordFailure. Calling them here too would double-count
    // every event and prematurely open/close the breaker.
    // Exception: dahlHealthCheck() updates the circuit directly because it runs
    // outside the router context.
    ...
    // Throw for retryable errors so the router records the fallback chain event.
    // Return null for permanent failures (bad request, auth error, etc.)
