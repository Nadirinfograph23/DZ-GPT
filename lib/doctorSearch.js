...
function renderTable(real, fallbackSpec, fallbackCity) {
  const header =
    `${RLM}| # | الاسم | التخصص | المدينة | الهاتف | المصدر |\n` +
    `${RLM}|---|---|---|---|---|---|`
  const rows = real.map((d, i) => {
    const name = ltr(d.name)
    ...
    const link = d.profileUrl ? `[${name}](${d.profileUrl})` : name
    return `${RLM}| ${i + 1} | ${link} | ${spec} | ${city} | ${phone} | ${src} |`
  })
...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  const { hasGps = false, sourceCount = 8, byName = false, queryName = '' } = opts
  ...
  if (!real.length && !dirs.length) {
    return (
      header + '\n\n' +
      `${RLM}لم أجد نتائج حالياً من المصادر المتاحة.\n\n` +
      `${RLM}💡 **اقتراحات:**\n` +
  ...
  // Table display — structured, RTL-friendly markdown table
  let body = ''
  if (real.length > 0) {
    const countLabel = real.length === 1
      ? 'نتيجة واحدة'
      : `${real.length} نتائج`
    body = `${RLM}_وجدنا ${countLabel}، مرتّبة حسب توفر الهاتف ثم عدد المصادر:_\n\n` +
  ...
  if (dirs.length) {
    out += '\n\n---\n\n' +
      `${RLM}📂 **بحث مباشر في المواقع:**\n\n` +
      dirs.map(d => `${RLM}• 🔗 [${d.sources[0]}](${d.profileUrl})`).join('\n')
  }

  const gpsTag = hasGps ? ' · 📡 مرتّب حسب القرب منك' : ''
  const modeTag = byName ? ' (بحث بالاسم)' : ''
  out += `\n\n${RLM}---\n${RLM}_🔍 تم البحث في ${sourceCount} مصادر${modeTag}${gpsTag}_`
  return out
}
