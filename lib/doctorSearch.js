...
export function mapLink(name, city, lat, lng, label, address = '') {
  // Search the selected doctor by name + the actual address collected from
  // the medical source. On Android, Google's Maps URL opens the Maps app when
  // it is installed; otherwise it opens the same search in the browser.
  const addressQuery = [name, address, city, 'Algérie'].filter(Boolean).join(', ')
  const q = encodeURIComponent(addressQuery)
  const gmUrl = addressQuery
    ? `https://www.google.com/maps/search/?api=1&query=${q}`
    : (lat && lng
      ? `https://www.google.com/maps/search/?api=1&query=${lat},${lng}`
      : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([name, city, 'Algérie'].filter(Boolean).join(', '))}`)
  const display = label || [address, city].filter(Boolean).join('، ') || name || 'خريطة'
