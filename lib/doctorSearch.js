...
// Fuzzy name similarity: share of common tokens (Jaccard on word tokens)
function nameSimilarity(a, b) {
  const tokA = new Set(normalizeName(a).split(' ').filter(t => t.length > 1))
  const tokB = new Set(normalizeName(b).split(' ').filter(t => t.length > 1))
...
export function mergeResults(perSourceLists) {
  ...
  // ── Fuzzy dedup pass: merge near-duplicate names (same doctor from different sources) ──
  // Two non-directory results with nameSimilarity ≥ 0.75 AND same city → merge into one
  const kept = []
  for (const item of items) {
    if (item.directoryLink) { kept.push(item); continue }
    const dupIdx = kept.findIndex(k =>
      !k.directoryLink &&
      nameSimilarity(item.name, k.name) >= 0.75 &&
      (normalizeText(item.city) === normalizeText(k.city) ||
       !item.city || !k.city)
    )
    if (dupIdx === -1) {
      kept.push(item)
    } else {
      // Merge into existing: enrich data from duplicate
      const k = kept[dupIdx]
      k.name = pickBetter(k, item, 'name')
      k.speciality = pickBetter(k, item, 'speciality')
      k.city = pickBetter(k, item, 'city')
      k.address = pickBetter(k, item, 'address')
      k.phone = k.phone || item.phone
      k.profileUrl = k.profileUrl || item.profileUrl
      for (const s of item.sources) if (!k.sources.includes(s)) k.sources.push(s)
      for (const u of item.sourceUrls) if (!k.sourceUrls.includes(u)) k.sourceUrls.push(u)
    }
  }
  return kept
}
