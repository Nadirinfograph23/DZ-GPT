...
export async function searchDoctors({ speciality, city, localResults = [], userLocation = null } = {}) {
  ...
  // Google Maps fallback when fewer than 3 real results
  if (real.length < 3) {
    const gmUrl = `https://www.google.com/maps/search/${encodeURIComponent(`${speciality} ${city} Algeria`)}`
    dirs.push({
      name: `Google Maps — ${speciality} (${city})`,
      speciality, city, address: '', phone: '',
      profileUrl: gmUrl,
      sources: ['google-maps'],
      sourceUrls: [gmUrl],
...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  ...
  const lines = real.map((d, i) => {
    ...
    const distLine = (typeof d.distanceKm === 'number')
      ? `📏 المسافة: ${ltr(`~${d.distanceKm} كم`)}`
      : ''
    const parts = [
      rtlLine(`**${i + 1}. ${ltr(d.name)}**`),
      cityLine ? rtlLine(`📍 ${cityLine}`) : '',
      rtlLine(`🧠 التخصص: ${specLine}`),
      d.phone ? rtlLine(`📞 ${formatPhone(d.phone)}`) : '',
      addrLine ? rtlLine(`🏠 ${addrLine}`) : '',
      distLine ? rtlLine(distLine) : '',
      rtlLine(`🌐 المصدر: ${d.sources.join('، ')}`),
      d.profileUrl ? rtlLine(`🔗 [الملف الشخصي](${d.profileUrl})`) : '',
      rtlLine(`🗺️ [عرض على الخريطة](${osmUrl(d.name, cityLine)}) · [Google Maps](${googleMapsUrl(d.name, cityLine)})`),
    ].filter(Boolean)
    return parts.join('  \n')
  }).join('\n\n')
  ...
  if (dirs.length) {
    out += '\n\n---\n' + rtlLine('📂 **مصادر إضافية للبحث المباشر:**') + '\n' +
      dirs.map(d => {
        const label = d.googleMaps ? 'Google Maps' : d.sources[0]
        return rtlLine(`- 🔗 [${label}](${d.profileUrl})`)
      }).join('\n')
  }
  const gpsTag = hasGps ? ' · 📡 GPS مفعّل (مرتّب حسب القرب)' : ''
  out += '\n\n' + rtlLine(`_تم البحث في ${sourceCount} مصادر بالتوازي وبأولوية متساوية${gpsTag}_`)
  return out
}
