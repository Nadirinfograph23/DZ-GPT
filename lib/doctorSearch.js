...
// Bidi control characters (work in plain text — no HTML/dependency needed):
//   RLM  \u200F  Right-to-Left Mark — forces RTL paragraph direction
//   LRI  \u2066  Left-to-Right Isolate — opens an LTR isolated run
//   PDI  \u2069  Pop Directional Isolate — closes LRI/RLI/FSI
const RLM = '\u200F'
const ltr = (s) => `\u2066${s}\u2069`
// Force every line to be treated as RTL (so digits/Latin words inside don't
// flip the whole line). Also wraps the line content in an RLI...PDI to isolate.
const rtlLine = (s) => `${RLM}\u2067${s}\u2069`

// Algerian-style phone format (0XXX XX XX XX), kept LTR & isolated so it
// never appears reversed inside an Arabic paragraph.
function formatPhone(p) {
  ...
  } else if (d.length === 9 && d.startsWith('5') === false && /^[567]/.test(d)) {
    pretty = `0${d.slice(0,3)} ${d.slice(3,5)} ${d.slice(5,7)} ${d.slice(7,9)}`
  }
  return ltr(pretty)
}
...
export function formatResults(results, specialityLabel, cityLabel) {
  ...
  if (!real.length && !dirs.length) {
    return header + '\n\n' + rtlLine('لم أجد نتائج حالياً من المصادر المتاحة. جرّب تخصصاً مرادفاً أو ولاية مجاورة.')
  }

  const lines = real.map((d, i) => {
    const cityLine = d.city || cityLabel
    const parts = [
      rtlLine(`**${i + 1}. ${ltr(d.name)}**`),
      cityLine ? rtlLine(`📍 ${cityLine}`) : '',
      rtlLine(`🧠 التخصص: ${d.speciality || specialityLabel}`),
      d.phone ? rtlLine(`📞 ${formatPhone(d.phone)}`) : '',
      d.address ? rtlLine(`🏠 ${d.address}`) : '',
      rtlLine(`🌐 المصدر: ${d.sources.join('، ')}`),
      d.profileUrl ? rtlLine(`🔗 [الملف الشخصي](${d.profileUrl})`) : '',
      rtlLine(`🗺️ [عرض على الخريطة](${osmUrl(d.name, cityLine)})`),
    ].filter(Boolean)
    return parts.join('  \n') // two-space + newline = hard line break in markdown
  }).join('\n\n')

  let out = header + '\n\n' + (lines || rtlLine('_(لا أطباء مفصّلون من المصادر القابلة للقراءة في هذا البحث)_'))
  if (dirs.length) {
    out += '\n\n---\n' + rtlLine('📂 **مصادر إضافية للبحث المباشر:**') + '\n' +
      dirs.map(d => rtlLine(`- 🔗 [${d.sources[0]}](${d.profileUrl})`)).join('\n')
  }
  out += '\n\n' + rtlLine('_تم البحث في 5 مصادر بالتوازي وبأولوية متساوية_')
  return out
}
