...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  const { hasGps = false, sourceCount = 9, byName = false, queryName = '' } = opts
  ...
  // ── Card display (mobile-first, clickable address + phone) ──────────────────
  let body = ''
  if (real.length > 0) {
    const withPhone  = real.filter(d => d.phone).length
    const withAddr   = real.filter(d => d.address || d.city).length
    const withLink   = real.filter(d => d.profileUrl && !d.directoryLink).length
    const stats = [
      `**${real.length}** طبيب`,
    ...
    // Use cards for rich display, table as compact fallback when many results
    const useCards = real.length <= 12
    body = `${RLM}> ✅ ${stats}\n\n` + (
      useCards
        ? renderCards(real, specialityLabel, cityLabel)
        : renderTable(real, specialityLabel, cityLabel)
    )
  } else {
    body = `${RLM}> _(لا أطباء مفصّلون — تحقق من المواقع أدناه)_`
  }
  ...
  const modeTag = byName ? ' · 🔎 بحث بالاسم' : ''
  const tipGps  = !hasGps ? `\n${RLM}💡 _أرسل موقعك الجغرافي لترتيب النتائج حسب القرب منك_` : ''
  out += `\n\n${RLM}---\n${RLM}_🔍 تم البحث في **${sourceCount} مصادر** طبية جزائرية${modeTag}_ · _اضغط 📍 للخريطة · اضغط 📞 للاتصال المباشر_${tipGps}`
  return out
}
