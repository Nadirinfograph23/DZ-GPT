...
function renderTable(real, fallbackSpec, fallbackCity) {
  // Stable ordered Markdown table — this is the primary doctor result view.
  // Only the 📍 icon is the map action; its URL is specific to that doctor.
  const header =
    `${RLM}| # | 👨‍⚕️ الطبيب | 🏥 التخصص | 📍 العنوان | 📞 الهاتف | 🌐 المصادر |\\n` +
    `${RLM}|:---:|---|---|---|---|---|`
  const rows = real.map((d, i) => {
    ...
    const sourceCell = (d.sources || []).map((s, sourceIndex) => {
      const url = d.sourceUrls?.[sourceIndex] || ''
      return url ? `[${s}](${url})` : s
    }).join('، ') || '—'
    return `${RLM}| ${i + 1} | **${name}** | ${spec} | ${locationCell} | ${phoneCell} | ${sourceCell} |`
  })
