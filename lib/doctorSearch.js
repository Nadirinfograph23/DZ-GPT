...
function renderTable(real, fallbackSpec, fallbackCity) {
  const header =
    `${RLM}| # | 👨‍⚕️ الاسم | 🏥 التخصص | 📍 المدينة | 📞 الهاتف | ✅ المصدر | 🔗 الملف |\n` +
    `${RLM}|:---:|---|---|---|---|---|:---:|`
  const rows = real.map((d, i) => {
    // Clean doctor name — ensure د. prefix
    ...
    const city = (d.cityAr || translateCity(d.city) || fallbackCity || '—').replace(/\|/g, '·')
    const addr = (d.addressAr || translateAddress(d.address) || '').replace(/\|/g, '·')
    const location = addr ? `${city} — ${addr}` : city
    const phone = d.phone ? `\`${formatPhone(d.phone)}\`` : '—'
    const src = d.sources.join('، ').replace(/\|/g, '·')
    const profileCell = d.profileUrl && !d.directoryLink
      ? `[📋 عرض](${d.profileUrl})`
      : '—'
    const nameLinkCell = name
    const distStr = typeof d.distanceKm === 'number' ? ` _(~${d.distanceKm} كم)_` : ''
    return `${RLM}| ${i + 1} | **${nameLinkCell}** | ${spec} | ${location}${distStr} | ${phone} | ${src} | ${profileCell} |`
  })
...
// Render results as clean doctor cards — RTL, with bullet points as requested
function renderCards(real, fallbackSpec, fallbackCity) {
  return real.map((d, i) => {
    ...
    // Clean name — preserve original casing, ensure د. prefix
    const cleanName = /^(dr\.?\s|docteur\s|د\.?\s|الدكتور\s)/i.test(d.name.trim())
      ? d.name.trim()
      : `د. ${d.name.trim()}`

    const locationStr = [addrLine, cityLine].filter(Boolean).join('، ')
    const distStr = (typeof d.distanceKm === 'number')
      ? ` (${ltr(`~${d.distanceKm} كم`)})`
      : ''

    const lines = [
      `${RLM}${genderIcon} **${ltr(cleanName)}**`,
      specLine   ? `${RLM}• ${emoji} التخصص: **${specLine}**` : '',
      locationStr ? `${RLM}• 📍 العنوان: ${locationStr}${distStr}` : (cityLine ? `${RLM}• 📍 المدينة: ${cityLine}${distStr}` : ''),
      d.phone    ? `${RLM}• 📞 الهاتف: **${formatPhone(d.phone)}**` : `${RLM}• 📞 الهاتف: _غير متوفر_`,
    ].filter(Boolean)
...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  ...
  if (real.length > 0) {
    ...
    const stats = [
      `${real.length} طبيب`,
      withPhone  ? `📞 ${withPhone} رقم هاتف` : null,
      withLink   ? `🔗 ${withLink} ملف إلكتروني` : null,
      hasGps     ? '📡 مرتّب حسب القرب' : 'مرتّب حسب توفر البيانات',
    ].filter(Boolean).join(' · ')

    body = `${RLM}> ✅ ${stats}\n\n` +
      renderTable(real, specialityLabel, cityLabel)
  } else {
    body = `${RLM}> _(لا أطباء مفصّلون من المصادر القابلة للقراءة في هذا البحث)_`
  }
  ...
  out += `\n\n${RLM}---\n${RLM}_🔍 تم البحث في **${sourceCount} مصادر** طبية جزائرية${modeTag}_`
  return out
}
