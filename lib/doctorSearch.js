...
// ───────────────────────── Source: BEESIHA (FR) ─────────────────────────
// beesiha.com/fr is mostly JS-rendered; we attempt a lightweight server fetch
// of common search-URL patterns and fall back to a directory deep-link so the
// source always contributes. Same shape & dedup contract as the other SPA
// sources above (sihhatech / machrou3).
async function fetchBeesiha(speciality, city) {
  const q = `${speciality || ''} ${city || ''}`.trim()
  const candidates = q
    ? [
        `https://www.beesiha.com/fr/recherche?q=${encodeURIComponent(q)}`,
        `https://www.beesiha.com/fr/search?q=${encodeURIComponent(q)}`,
        `https://www.beesiha.com/fr?s=${encodeURIComponent(q)}`,
      ]
    : ['https://www.beesiha.com/fr']
  const out = []
  ...
  for (const url of candidates) {
    ...
    try {
      const $ = cheerio.load(r.html)
      $('[class*="doctor"], [class*="Doctor"], [class*="praticien"], [class*="card"], [class*="Card"], article, li').each((_, el) => {
        const $el = $(el)
        const text = $el.text().replace(/\s+/g, ' ').trim()
        if (!text) return
        const name = $el.find('h1, h2, h3, h4, a, strong').first().text().trim()
        if (!name || name.length < 3 || name.length > 120) return
        if (!/dr|docteur|د\.|دكتور|cabinet|clinique|عياد|praticien/i.test(name + ' ' + text)) return
        const phone = extractPhone($el, text)
        const href = $el.find('a').first().attr('href') || ''
        const profileUrl = href.startsWith('http')
          ? href
          : (href ? `https://www.beesiha.com${href.startsWith('/') ? '' : '/'}${href}` : url)
        // Try to pull a specialty/address line if present
        const specGuess = $el.find('[class*="spec"], [class*="Spec"]').first().text().trim()
        const addrGuess = $el.find('[class*="addr"], [class*="Addr"], [class*="loc"], [class*="Loc"]').first().text().trim()
        out.push({
          name,
...
// Render results as detailed cards (clearer for a single result or rich detail).
...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  const { hasGps = false, sourceCount = 7, byName = false, queryName = '' } = opts
  const headerLabel = byName
    ? `🔎 **نتائج البحث عن: ${ltr(queryName || specialityLabel)}**`
    : `🩺 **${specialityLabel}${cityLabel ? ` في ${cityLabel}` : ''}**`
  const header = `${RLM}${headerLabel}`
  const real = results.filter(d => !d.directoryLink)
  const dirs = results.filter(d => d.directoryLink)

  if (!real.length && !dirs.length) {
    return header + '\n\n' + RLM + 'لم أجد نتائج حالياً من المصادر المتاحة. جرّب اسماً أو تخصصاً مرادفاً أو ولاية مجاورة.'
  }

  // Use a table when 2+ real results; otherwise a single card is clearer.
  let body = ''
  if (real.length >= 2) {
    body = renderTable(real, specialityLabel, cityLabel) +
      '\n\n' + `${RLM}_📋 تفاصيل أوسع:_` + '\n\n' + renderCards(real, specialityLabel, cityLabel)
  } else if (real.length === 1) {
    body = renderCards(real, specialityLabel, cityLabel)
  } else {
  ...
  out += '\n\n' + `${RLM}_تم البحث في ${sourceCount} مصادر بالتوازي${modeTag}${gpsTag}_`
  return out
}
