...
//   sahadoc, algerie-docto, addalile, salim-dz, pj-dz, docteur360, sihhatech, machrou3, beesiha, altibbi
...
// ───────────────────────── Source: DZDOC (live directory) ─────────────────────────
// DZDOC exposes a server-rendered search page. We discover the numeric
// specialty/wilaya IDs from its own filter form, then parse profile links.
async function fetchDzdoc(speciality, city) {
  const home = await politeFetch('https://dzdoc.com/', { timeoutMs: 9000 })
  if (!home.ok || !home.html) return { source: 'dzdoc', results: [], sourceUrl: 'https://dzdoc.com/', error: home.error || `HTTP ${home.status}` }
  const $home = cheerio.load(home.html)
  const norm = s => normalizeText(s || '')
  const findOption = (selectRe, wanted) => {
    let value = ''
    $home('select').each((_, el) => {
      const $s = $home(el)
      const blob = norm($s.attr('name') + ' ' + $s.attr('id') + ' ' + $s.attr('class'))
      if (!selectRe.test(blob) || value) return
      $s.find('option').each((__, opt) => {
        const txt = norm($home(opt).text())
        if (!value && (txt === norm(wanted) || txt.includes(norm(wanted)) || norm(wanted).includes(txt))) value = $home(opt).attr('value') || ''
      })
    })
    return value
  }
  const specId = findOption(/special/i, speciality)
  const regionId = findOption(/region|wilaya/i, city)
  const query = new URLSearchParams()
  if (regionId) query.set('region', regionId)
  if (specId) query.set('specialite', specId)
  const url = `https://dzdoc.com/recherche.php?${query.toString()}`
  const r = await politeFetch(url, { timeoutMs: 9000 })
  if (!r.ok || !r.html) return { source: 'dzdoc', results: [], sourceUrl: url, error: r.error || `HTTP ${r.status}` }

  const $ = cheerio.load(r.html)
  const out = []
  $('a[href*="profil.php"]').each((_, el) => {
    const $a = $(el)
    const name = $a.text().replace(/\\s+/g, ' ').trim()
    if (!name || /^(profil|profile)$/i.test(name)) return
    let $container = $a.parent()
    for (let i = 0; i < 4 && $container.length; i++) {
      const txt = $container.text().replace(/\\s+/g, ' ').trim()
      if (txt.length >= name.length && txt.length <= 700) break
      $container = $container.parent()
    }
    const cardText = $container.text().replace(/\\s+/g, ' ').trim()
    const phone = extractPhone($container, cardText)
    const href = $a.attr('href') || ''
    const profileUrl = href.startsWith('http') ? href : new URL(href, 'https://dzdoc.com/').href
    const cleaned = cardText
      ...
      .replace(/\\s{2,}/g, ' ')
      .trim()
    const specialityText = speciality || ''
    const address = cleaned.replace(specialityText, '').replace(/\\s{2,}/g, ' ').trim()
    out.push({ name, speciality: specialityText, city, address, phone, profileUrl })
  })

  // Enrich missing phone/address from the actual doctor profile pages.
  const enriched = await Promise.all(out.slice(0, 15).map(async d => {
    if (d.phone && d.address) return d
    const p = await politeFetch(d.profileUrl, { timeoutMs: 7000 })
    if (!p.ok || !p.html) return d
    const $p = cheerio.load(p.html)
    const text = $p('body').text().replace(/\\s+/g, ' ')
    if (!d.phone) d.phone = extractPhone($p('body'), text)
    if (!d.address) {
      const mapText = $p('a[href*="google.com/maps"], a[href*="maps.google"]').first().text().trim()
      d.address = mapText || d.address
    }
    return d
  }))
  return { source: 'dzdoc', results: enriched, sourceUrl: url }
}

// ───────────────────────── Source: DOCTEUR360 (live search pages) ─────────────────────────
const DOCTEUR360_SPEC_SLUGS = {
  'طبيب عام': 'medecin-generaliste', 'عام': 'medecin-generaliste',
  'طبيب أسنان': 'chirurgien-dentiste', 'أسنان': 'chirurgien-dentiste',
  'طبيب قلب': 'cardiologue', 'قلب': 'cardiologue',
  'طبيب أطفال': 'pediatre', 'أطفال': 'pediatre',
  'طبيب عيون': 'ophtalmologue', 'عيون': 'ophtalmologue',
  'طبيب جلدية': 'dermatologue', 'جلدية': 'dermatologue',
  'طبيب نساء وتوليد': 'gynecologue-obstetricien', 'نساء': 'gynecologue-obstetricien',
  'طبيب نفسي': 'psychiatre', 'نفسي': 'psychiatre',
  'طبيب مسالك بولية': 'urologue', 'مسالك': 'urologue',
  'طبيب أعصاب': 'neurologue', 'أعصاب': 'neurologue',
  'طبيب جهاز هضمي': 'gastro-enterologue', 'هضمي': 'gastro-enterologue',
  'طبيب رئة': 'pneumologue', 'رئة': 'pneumologue',
  'طبيب غدد': 'endocrinologue', 'غدد': 'endocrinologue',
  'طبيب كلى': 'nephrologue', 'كلى': 'nephrologue',
  'طبيب أورام': 'oncologue', 'أورام': 'oncologue',
  'أنف وأذن وحنجرة': 'orl', 'orl': 'orl',
}

function docteur360SpecSlug(speciality) {
  const key = normalizeText(speciality)
  return DOCTEUR360_SPEC_SLUGS[key] || slugify(speciality).replace(/medecin-generaliste|generaliste/g, 'medecin-generaliste')
}

async function fetchDocteur360Live(speciality, city) {
  const specSlug = docteur360SpecSlug(speciality)
  const citySlug = slugify(city)
  const url = `https://docteur360.com.dz/specialiste/${specSlug}/${citySlug}/${citySlug}`
  const r = await politeFetch(url, { timeoutMs: 10000 })
  if (!r.ok || !r.html) return { source: 'docteur360', results: [], sourceUrl: url, error: r.error || `HTTP ${r.status}` }
  const $ = cheerio.load(r.html)
  const out = []
  $('a[href*="/medecins/"], a[href*="/medecin/"], a').filter((_, el) => {
    const txt = $(el).text().replace(/\s+/g, ' ').trim()
    return /voir le profil/i.test(txt) || /\/medecins?\//i.test($(el).attr('href') || '')
  }).each((_, el) => {
    const $a = $(el)
    const name = $a.text().replace(/\\s+/g, ' ').trim()
    if (!name || name.length < 3 || /voir le profil/i.test(name)) return
    let $container = $a.parent()
    for (let i = 0; i < 5 && $container.length; i++) {
      const txt = $container.text().replace(/\\s+/g, ' ').trim()
      if (txt.length >= name.length && txt.length <= 900) break
      $container = $container.parent()
    }
    const cardText = $container.text().replace(/\\s+/g, ' ').trim()
    const phone = extractPhone($container, cardText)
    const mapText = $container.find('a[href*="maps.google"], a[href*="google.com/maps"]').first().text().trim()
    const href = $a.attr('href') || ''
    const profileUrl = href.startsWith('http') ? href : new URL(href, 'https://docteur360.com.dz/').href
    const address = mapText || (() => {
      const after = cardText.replace(name, '').replace(phone, '').replace(/Voir le profil/gi, '').trim()
      return after.replace(/Ouvert.*?(?=(\\d{6,}|[A-ZÀ-Ü][^,]{2,}\\s*,))/i, '').trim()
    })()
    out.push({ name: name.replace(/^Docteur\\s+/i, ''), speciality, city, address, phone, profileUrl })
  })
  ...
  return { source: 'docteur360', results: unique, sourceUrl: url }
}

// All sources have EQUAL priority — fetched in parallel.
export const SOURCES = [
  // Primary live sources requested for doctor lookup.
  // Do not silently fall back to unrelated directory-only sources:
  // the UI needs real doctor records (name/address/phone) to render its table.
  { id: 'dzdoc',      fetcher: fetchDzdoc },
  { id: 'docteur360', fetcher: fetchDocteur360Live },
]
...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  const { hasGps = false, sourceCount = 2, byName = false, queryName = '' } = opts
