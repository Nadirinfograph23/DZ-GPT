// Doctor search for DZ Agent.
// Authoritative sources used by the doctor feature:
//   • DZDOC — https://dzdoc.com/
//   • Docteur360 — https://docteur360.com.dz/
// Results from these two sources are merged and displayed in one compact table.
// Phone numbers are tel: links and addresses open Google Maps.
//
...
// ───────────────────────── Source: ADDALILE MEDICAL ─────────────────────────
// Uses the dedicated /tib/ medical directory endpoint for better results
async function fetchAddalile(speciality, city) {
  const cityKey = normalizeText(city || '')
  const specKey = normalizeText(speciality || '')

  // Map specialty to addalile /tib/ slug
  const ADDALILE_SPEC_MAP = {
    'dentiste': 'dentiste', 'cardiologue': 'cardiologie', 'orthopediste': 'orthopedie',
    'orthopédiste': 'orthopedie', 'gynecologue': 'gynecologie', 'gynécologue': 'gynecologie',
    'pediatre': 'pediatrie', 'pédiatre': 'pediatrie', 'ophtalmologue': 'ophtalmologie',
    'dermatologue': 'dermatologie', 'generaliste': 'medecine-generale', 'généraliste': 'medecine-generale',
    'medecin generaliste': 'medecine-generale', 'orl': 'orl', 'psychiatre': 'psychiatrie',
    'rhumatologue': 'rhumatologie', 'urologue': 'urologie', 'neurologue': 'neurologie',
    'chirurgien': 'chirurgie',
  }
  const ADDALILE_CITY_MAP = {
  ...
  const specSlug = ADDALILE_SPEC_MAP[specKey] || slugify(speciality || '')
  const citySlug = ADDALILE_CITY_MAP[cityKey] || ADDALILE_CITY_MAP[city?.toLowerCase()] || slugify(city || '')

  // Try /tib/ medical directory first, fall back to general search
  const candidates = []
  if (specSlug && citySlug) {
    candidates.push(`https://www.addalile.com/tib/${specSlug}/${citySlug}/`)
    candidates.push(`https://www.addalile.com/tib/${citySlug}/${specSlug}/`)
    candidates.push(`https://www.addalile.com/tib/?s=${encodeURIComponent(`${speciality} ${city}`)}`)
  }
  candidates.push(`https://addalile.com/tib/?s=${encodeURIComponent(`${speciality} ${city}`)}`)
  candidates.push(`https://addalile.com/?s=${encodeURIComponent(`${speciality} ${city} طبيب`)}`)
  ...
  for (const url of candidates) {
    usedUrl = url
    const r = await politeFetch(url, { timeoutMs: 8000 })
    if (!r.ok || !r.html) continue
    try {
      const $ = cheerio.load(r.html)
      // Addalile /tib/ uses .tib-item, .listing-item, article, or .entry structures
      const selectors = [
        '.tib-item', '.tib-card', '.listing-item', '.doctor-item',
      ...
      $(selectors.join(', ')).each((_, el) => {
        ...
        const name = (
          $card.find('.tib-name, .doctor-name, h1, h2, h3, h4, .entry-title').first().text().trim()
          || $el.text().trim()
        )
        if (!name || name.length < 3 || name.length > 120) return
        const cardText = $card.text().replace(/\s+/g, ' ')
        if (!/dr\.?|د\.?|دكتور|docteur|cabinet|clinique|عياد|طبيب|médecin/i.test(name + ' ' + cardText)) return

        const phone = extractPhone($card, cardText)
        const addr = $card.find('.tib-address, .address, [class*="addr"], [class*="loc"]').first().text().trim()
          || $card.find('p, span').filter((_, e) => /rue|avenue|cité|quartier|commune|شارع|حي/i.test($(e).text())).first().text().trim()
        const specFound = $card.find('.tib-spec, .specialty, [class*="spec"]').first().text().trim()
          || $card.find('span, small').filter((_, e) => /dentiste|cardio|gyneco|pediatre|ophtalmologue|dermatologue/i.test($(e).text())).first().text().trim()

        const href = ($card.find('a').first().attr('href') || '').trim()
        const profileUrl = href.startsWith('http') ? href
          : (href ? `https://www.addalile.com${href.startsWith('/') ? '' : '/'}${href}` : url)

        out.push({
          name,
          speciality: specFound || speciality || '',
          city: city || '',
          address: addr || '',
          phone,
          profileUrl,
  ...
  if (unique.length === 0) {
    return {
      source: 'addalile',
      results: [{
        name: `Addalile — ${speciality}${city ? ' (' + city + ')' : ''}`.trim(),
        speciality, city, address: '', phone: '',
        profileUrl: `https://www.addalile.com/tib/?s=${encodeURIComponent(`${speciality} ${city}`)}`,
        directoryLink: true,
      }],
      sourceUrl: usedUrl,
    }
  }
  return { source: 'addalile', results: unique.slice(0, 15), sourceUrl: usedUrl }
}
...
async function fetchDzdoc(speciality, city) {
  const homeUrl = 'https://dzdoc.com/'
  const home = await politeFetch(homeUrl, { timeoutMs: 9000 })
  if (!home.ok || !home.html) return { source: 'dzdoc', results: [], sourceUrl: homeUrl, error: home.error || `HTTP ${home.status}` }

  const $home = cheerio.load(home.html)
  const clean = (v) => normalizeText(String(v || '').replace(/\s+/g, ' '))
  const wantedSpec = clean(speciality)
  const wantedCity = clean(city)
  let specId = '', regionId = ''

  $home('select').each((_, el) => {
    const $sel = $home(el)
    const marker = clean($sel.attr('name') || $sel.attr('id') || '')
    const options = $sel.find('option').toArray()
    if (!specId && /special|spec|specialite/.test(marker)) {
      const hit = options.find(o => {
        const t = clean($home(o).text())
        return t === wantedSpec || t.includes(wantedSpec) || wantedSpec.includes(t)
      })
      if (hit) specId = $home(hit).attr('value') || ''
    }
    if (!regionId && /region|wilaya|ville/.test(marker)) {
      const hit = options.find(o => {
        const t = clean($home(o).text())
        return t === wantedCity || t.includes(wantedCity) || wantedCity.includes(t)
      })
      if (hit) regionId = $home(hit).attr('value') || ''
    }
  })

  const DZDOC_SPEC_IDS = {
    'médecin généraliste': '1', 'medecin generaliste': '1', 'généraliste': '1', 'generaliste': '1',
  }
  const DZDOC_REGION_IDS = {
    'alger': '16', 'الجزائر': '16', 'oran': '31', 'وهران': '31',
    'constantine': '25', 'قسنطينة': '25', 'annaba': '23', 'عنابة': '23',
    'setif': '19', 'sétif': '19', 'سطيف': '19', 'batna': '5', 'باتنة': '5',
    'blida': '9', 'البليدة': '9', 'bejaia': '6', 'بجاية': '6',
    'tlemcen': '13', 'تلمسان': '13', 'tizi ouzou': '15', 'تيزي وزو': '15',
    'biskra': '7', 'بسكرة': '7', 'skikda': '21', 'سكيكدة': '21',
    'ouargla': '30', 'ورقلة': '30', 'el oued': '39', 'الوادي': '39',
  }
  specId = specId || DZDOC_SPEC_IDS[wantedSpec] || ''
  regionId = regionId || DZDOC_REGION_IDS[wantedCity] || ''

  if (!specId || !regionId) {
    const url = 'https://dzdoc.com/recherche.php'
    return { source: 'dzdoc', results: [{
      name: `DZDOC — ${speciality}${city ? ' (' + city + ')' : ''}`.trim(),
      speciality, city, address: '', phone: '', profileUrl: url, directoryLink: true,
    }], sourceUrl: url }
  }

  const searchUrl = `https://dzdoc.com/recherche.php?region=${encodeURIComponent(regionId)}&specialite=${encodeURIComponent(specId)}`
  const r = await politeFetch(searchUrl, { timeoutMs: 9000 })
  if (!r.ok || !r.html) return { source: 'dzdoc', results: [], sourceUrl: searchUrl, error: r.error || `HTTP ${r.status}` }

  const $ = cheerio.load(r.html)
  const cards = []
  $('a[href*="profil.php?id="]').each((_, el) => {
    const $a = $(el)
    const href = $a.attr('href') || ''
    const profileUrl = href.startsWith('http') ? href : `https://dzdoc.com/${href.replace(/^\//, '')}`
    const $row = $a.closest('tr')
    const name = $a.text().replace(/\s+/g, ' ').trim()
    if (!name || name.length < 2 || /profil/i.test(name)) return
    const specialityText = $row.length ? $row.find('td').eq(1).text().replace(/\s+/g, ' ').trim() : speciality
    const address = $row.length ? $row.find('td').eq(2).text().replace(/\s+/g, ' ').trim() : ''
    cards.push({ name, speciality: specialityText || speciality, city, address, phone: '', profileUrl })
  })
  ...
  for (const d of unique.slice(0, 10)) {
    const p = await politeFetch(d.profileUrl, { timeoutMs: 7000 })
    if (!p.ok || !p.html) continue
    try {
      const $p = cheerio.load(p.html)
      const bodyText = $p('body').text().replace(/\s+/g, ' ')
      d.phone = extractPhone($p('body'), bodyText)
      const addr = $p('h1,h2,h3,h4,p,li,div').filter((_, e) => {
        const t = $p(e).text().replace(/\s+/g, ' ').trim()
        return t && /rue|avenue|boulevard|cité|quartier|lotissement|coopérative|حي|شارع|طريق/i.test(t) && t.length < 180
      }).first().text().replace(/\s+/g, ' ').trim()
      if (addr && !/special|formation|langue|horaire|téléphone/i.test(addr)) d.address = addr
    } catch {}
  }
  return { source: 'dzdoc', results: unique.slice(0, 15), sourceUrl: searchUrl }
}

// ───────────────────────── Source: DOCTEUR360 ─────────────────────────
async function fetchDocteur360(speciality, city) {
  const specSlugMap = {
    'dentiste': 'chirurgien-dentiste', 'chirurgien dentiste': 'chirurgien-dentiste',
    'cardiologue': 'cardiologue', 'orthopediste': 'orthopediste', 'orthopédiste': 'orthopediste',
    'gynecologue': 'gynecologue', 'gynécologue': 'gynecologue',
    'pediatre': 'pediatre', 'pédiatre': 'pediatre', 'ophtalmologue': 'ophtalmologue',
    'dermatologue': 'dermatologue', 'generaliste': 'medecin-generaliste',
    'généraliste': 'medecin-generaliste', 'medecin generaliste': 'medecin-generaliste',
    'psychiatre': 'psychiatre', 'rhumatologue': 'rhumatologue', 'urologue': 'urologue',
    'neurologue': 'neurologue', 'orl': 'orl', 'pneumologue': 'pneumologue',
    'gastro-enterologue': 'gastro-enterologue', 'gastro-entérologue': 'gastro-enterologue',
  }
  const citySlugMap = {
    'alger': 'alger', 'الجزائر': 'alger', 'oran': 'oran', 'وهران': 'oran',
    'constantine': 'constantine', 'قسنطينة': 'constantine', 'annaba': 'annaba', 'عنابة': 'annaba',
    'blida': 'blida', 'البليدة': 'blida', 'batna': 'batna', 'باتنة': 'batna',
    'setif': 'setif', 'sétif': 'setif', 'سطيف': 'setif', 'bejaia': 'bejaia', 'بجاية': 'bejaia',
    'tlemcen': 'tlemcen', 'تلمسان': 'tlemcen', 'tizi ouzou': 'tizi-ouzou', 'تيزي وزو': 'tizi-ouzou',
    'biskra': 'biskra', 'بسكرة': 'biskra', 'skikda': 'skikda', 'سكيكدة': 'skikda',
    'ouargla': 'ouargla', 'ورقلة': 'ouargla', 'el oued': 'el-oued', 'الوادي': 'el-oued',
  }
  const sk = normalizeText(speciality), ck = normalizeText(city)
  const specSlug = specSlugMap[sk] || slugify(speciality || '')
  const citySlug = citySlugMap[ck] || slugify(city || '')
  const candidates = []
  if (specSlug && citySlug) {
    candidates.push(`https://docteur360.com.dz/specialiste/${specSlug}/${citySlug}`)
    candidates.push(`https://docteur360.com.dz/medecins/${citySlug}`)
  } else if (citySlug) {
    candidates.push(`https://docteur360.com.dz/medecins/${citySlug}`)
  } else {
    candidates.push('https://docteur360.com.dz/categorie/medecins')
  }
  ...
  for (const url of candidates) {
    ...
    try {
      ...
      $('h2, h3').each((_, el) => {
        ...
        if (!name || /Résultats|médecins|spécialiste/i.test(name)) return
        const $container = $h.closest('article, li')
        const card = $container.length ? $container : $h.parent()
        const text = card.text().replace(/\s+/g, ' ').trim()
        if (!text || text.length > 900) return
        const specialityText = card.find('a[href*="/specialiste/"], h3, p').filter((_, e) =>
          /médecin|docteur|chirurg|cardio|pédiatr|ophtal|dermato|urolog|gynéco|psychi|pneumo|orl|gastro/i.test($(e).text())
        ).first().text().trim()
        const phone = extractPhone(card, text)
        const mapsHref = card.find('a[href*="maps.google"], a[href*="google.com/maps"]').first().attr('href') || ''
        const address = card.find('a[href*="maps.google"], a[href*="google.com/maps"]').first().text().replace(/\s+/g, ' ').trim()
          || card.find('p, span').filter((_, e) => /rue|avenue|boulevard|cité|quartier|centre/i.test($(e).text())).first().text().replace(/\s+/g, ' ').trim()
        if (name && (phone || address || specialityText)) {
          const profileHref = $h.find('a').first().attr('href') || ''
          out.push({
            name: name.replace(/^Docteur\s+/i, '').trim(),
            speciality: specialityText || speciality,
            city,
            address: address || '',
            phone,
            profileUrl: profileHref.startsWith('http') ? profileHref : (profileHref ? `https://docteur360.com.dz${profileHref}` : url),
            sourceMapUrl: mapsHref,
          })
        }
      })
      if (out.length) break
  ...
  if (!unique.length) return {
    source: 'docteur360',
    results: [{ name: `Docteur360 — ${speciality}${city ? ' (' + city + ')' : ''}`.trim(), speciality, city, address: '', phone: '', profileUrl: usedUrl, directoryLink: true }],
    sourceUrl: usedUrl,
  }
  return { source: 'docteur360', results: unique.slice(0, 15), sourceUrl: usedUrl }
}
...
// ───────────────────────── Source: MACHROU3 (Doc directory) ─────────────────────────
// machrou3.com/Doc is a JS-driven directory; we attempt a lightweight server
// fetch and fall back to a deep-link entry so the source always contributes.
async function fetchMachrou3(speciality, city) {
  const q = encodeURIComponent(`${speciality || ''} ${city || ''}`.trim())
  const searchUrl = `https://machrou3.com/Doc?q=${q}`
  const r = await politeFetch(searchUrl)
  const out = []
  if (r.ok && r.html) {
    try {
      const $ = cheerio.load(r.html)
      $('[class*="doctor"], [class*="Doctor"], [class*="card"], [class*="Card"], article, li').each((_, el) => {
        const $el = $(el)
        const text = $el.text().replace(/\s+/g, ' ').trim()
        if (!text) return
        const name = $el.find('h1, h2, h3, h4, a, strong').first().text().trim()
        if (!name || name.length < 3 || name.length > 120) return
        if (!/dr|docteur|د\.|دكتور|cabinet|clinique|عياد/i.test(name + ' ' + text)) return
        const phone = extractPhone($el, text)
        const href = $el.find('a').first().attr('href') || ''
        const profileUrl = href.startsWith('http') ? href : (href ? `https://machrou3.com${href.startsWith('/') ? '' : '/'}${href}` : searchUrl)
        out.push({ name, speciality: speciality || '', city: city || '', address: '', phone, profileUrl })
      })
    } catch { /* parse failed — fall back to directory */ }
  }
  const seen = new Set()
  ...
  if (unique.length === 0) {
    return {
      source: 'machrou3',
      results: [{
        name: `Annuaire machrou3 — ${speciality || ''}${city ? ' (' + city + ')' : ''}`.trim(),
  ...
  return { source: 'machrou3', results: unique.slice(0, 12), sourceUrl: searchUrl }
}

// ───────────────────────── Source: BEESIHA (FR) ─────────────────────────
// beesiha.com/fr supports both JS-rendered pages and some static search pages.
// We try multiple URL patterns in order and parse whatever content is available.
const BEESIHA_SPEC_SLUG = {
  'dentiste': 'dentiste', 'cardiologue': 'cardiologue', 'orthopediste': 'orthopedie',
  'orthopédiste': 'orthopedie', 'gynecologue': 'gynecologie', 'gynécologue': 'gynecologie',
  'pediatre': 'pediatrie', 'pédiatre': 'pediatrie', 'ophtalmologue': 'ophtalmologie',
  'dermatologue': 'dermatologie', 'generaliste': 'medecine-generale', 'généraliste': 'medecine-generale',
  'medecin generaliste': 'medecine-generale', 'psychiatre': 'psychiatrie',
  'rhumatologue': 'rhumatologie', 'urologue': 'urologie', 'neurologue': 'neurologie',
  'chirurgien': 'chirurgie', 'orl': 'orl',
}
const BEESIHA_CITY_SLUG = {
  'alger': 'alger', 'oran': 'oran', 'constantine': 'constantine', 'annaba': 'annaba',
  'batna': 'batna', 'setif': 'setif', 'sétif': 'setif', 'blida': 'blida',
  'bejaia': 'bejaia', 'tlemcen': 'tlemcen', 'tizi ouzou': 'tizi-ouzou',
  'biskra': 'biskra', 'mostaganem': 'mostaganem', 'skikda': 'skikda',
  'ouargla': 'ouargla', 'ghardaia': 'ghardaia', 'bechar': 'bechar',
  'jijel': 'jijel', 'guelma': 'guelma', 'sidi bel abbes': 'sidi-bel-abbes',
  'tiaret': 'tiaret', 'medea': 'medea', 'msila': 'msila', 'boumerdes': 'boumerdes',
  'bouira': 'bouira', 'mila': 'mila', 'souk ahras': 'souk-ahras', 'tipaza': 'tipaza',
  'el oued': 'el-oued', 'laghouat': 'laghouat', 'adrar': 'adrar', 'chlef': 'chlef',
}

async function fetchBeesiha(speciality, city) {
  const specKey = normalizeText(speciality || '')
  const cityKey = normalizeText(city || '')
  const specSlug = BEESIHA_SPEC_SLUG[specKey] || slugify(speciality || '')
  const citySlug = BEESIHA_CITY_SLUG[cityKey] || slugify(city || '')
  const q = `${speciality || ''} ${city || ''}`.trim()

  const candidates = []
  // Structured URL (most reliable if supported)
  if (specSlug && citySlug) candidates.push(`https://www.beesiha.com/fr/${specSlug}/${citySlug}`)
  if (specSlug && citySlug) candidates.push(`https://www.beesiha.com/fr/medecins/${specSlug}?wilaya=${citySlug}`)
  if (q) {
    candidates.push(`https://www.beesiha.com/fr/recherche?q=${encodeURIComponent(q)}`)
    candidates.push(`https://www.beesiha.com/fr/search?q=${encodeURIComponent(q)}`)
    candidates.push(`https://www.beesiha.com/fr?s=${encodeURIComponent(q)}`)
    candidates.push(`https://www.beesiha.com/fr/recherche?specialite=${encodeURIComponent(specSlug)}&ville=${encodeURIComponent(citySlug)}`)
  }
  if (!candidates.length) candidates.push('https://www.beesiha.com/fr')
  ...
  for (const url of candidates) {
    ...
    try {
      const $ = cheerio.load(r.html)
      // Wide selector net for beesiha card structures
      const $els = $([
        '[class*="doctor"]', '[class*="Doctor"]', '[class*="praticien"]', '[class*="Praticien"]',
        '[class*="medecin"]', '[class*="Medecin"]', '[class*="card"]', '[class*="Card"]',
        'article', '.result-item', '.search-result', '.profile-card',
      ].join(', '))
      $els.each((_, el) => {
        const $el = $(el)
        const text = $el.text().replace(/\s+/g, ' ').trim()
        if (!text || text.length < 5) return
        const name = $el.find('h1, h2, h3, h4, a[href*="medecin"], a[href*="doctor"], strong').first().text().trim()
        if (!name || name.length < 3 || name.length > 120) return
        if (!/dr\.?|docteur|د\.?|دكتور|cabinet|clinique|praticien|médecin|medecin/i.test(name + ' ' + text)) return
        const phone = extractPhone($el, text)
        const href = $el.find('a').first().attr('href') || ''
        const profileUrl = href.startsWith('http') ? href
          : (href ? `https://www.beesiha.com${href.startsWith('/') ? '' : '/'}${href}` : url)
        const specGuess = $el.find('[class*="spec"], [class*="Spec"], [class*="special"]').first().text().trim()
          || $el.find('p, small, span').filter((_, e) => /dentiste|cardio|pediatr|gyneco|généraliste|dermato/i.test($(e).text())).first().text().trim()
        const addrGuess = $el.find('[class*="addr"], [class*="Addr"], [class*="loc"], [class*="Loc"], [class*="adresse"]').first().text().trim()
          || $el.find('p, small').filter((_, e) => /rue|avenue|boulevard|cité|quartier|commune/i.test($(e).text())).first().text().trim()
        out.push({
          name,
          speciality: specGuess || speciality || '',
          city: city || '',
          address: addrGuess || '',
          phone,
          profileUrl,
      ...
      if (out.length) break
    } catch { /* parse failed, try next URL */ }
  }
  // Dedup intra-source
  const seen = new Set()
  const unique = []
  ...
  if (unique.length === 0) {
    return {
      source: 'beesiha',
      results: [{
        name: `Annuaire beesiha — ${speciality || ''}${city ? ' (' + city + ')' : ''}`.trim(),
        speciality: speciality || '', city: city || '', address: '', phone: '',
        profileUrl: `https://www.beesiha.com/fr/recherche?q=${encodeURIComponent(q || specSlug)}`,
        directoryLink: true,
      }],
      sourceUrl: lastUrl,
    }
  }
  return { source: 'beesiha', results: unique.slice(0, 12), sourceUrl: lastUrl }
}

// ───────────────────────── Source: ALTIBBI (DZ medical guide) ─────────────────────────
async function fetchAltibbi(speciality, city) {
  const citySlugMap = {
    'alger': 'alger', 'الجزائر': 'alger', 'oran': 'oran', 'وهران': 'oran',
    'constantine': 'constantine', 'قسنطينة': 'constantine', 'annaba': 'annaba', 'عنابة': 'annaba',
    'blida': 'blida', 'البليدة': 'blida', 'batna': 'batna', 'باتنة': 'batna',
    'setif': 'setif', 'سطيف': 'setif', 'tlemcen': 'tlemcen', 'تلمسان': 'tlemcen',
  }
  const cityKey = Object.keys(citySlugMap).find(k => normalizeText(city).includes(normalizeText(k)))
  const citySlug = cityKey ? citySlugMap[cityKey] : encodeURIComponent(city)

  const specSlugMap = {
    'dentiste': 'طبيب-اسنان', 'cardiologue': 'طبيب-قلب', 'orthopédiste': 'طبيب-عظام',
    'gynécologue': 'طبيب-نساء-وتوليد', 'pédiatre': 'طبيب-اطفال', 'ophtalmologue': 'طبيب-عيون',
    'dermatologue': 'طبيب-جلدية', 'généraliste': 'طبيب-عام', 'orl': 'طبيب-انف-واذن-وحنجرة',
    'psychiatre': 'طبيب-نفسي', 'neurologue': 'طبيب-اعصاب', 'urologue': 'طبيب-مسالك-بولية',
    'chirurgien': 'جراح',
  }
  const specKey = Object.keys(specSlugMap).find(k => normalizeText(speciality).includes(normalizeText(k)))
  const specSlug = specKey ? specSlugMap[specKey] : encodeURIComponent(speciality)

  const baseUrl = `https://altibbi.com/الدليل-الطبي/الجزائر/${citySlug}/${specSlug}`
  const fallbackUrl = `https://altibbi.com/الدليل-الطبي/الجزائر`
  const searchUrl = specSlug && citySlug ? baseUrl : fallbackUrl
  ...
  if (r.ok && r.html) {
    try {
      const $ = cheerio.load(r.html)
      $('[class*="doctor"], [class*="Doctor"], [class*="physician"], .card, article, li').each((_, el) => {
        const $el = $(el)
        const text = $el.text().replace(/\s+/g, ' ').trim()
        if (!text || text.length < 10) return
        const nameEl = $el.find('h1, h2, h3, h4, a, strong').first()
        const name = nameEl.text().trim()
        if (!name || name.length < 3 || name.length > 120) return
        if (!/dr|docteur|د\.|دكتور|طبيب|عيادة/i.test(name + ' ' + text)) return
        const phone = extractPhone($el, text)
        const href = ($el.find('a').first().attr('href') || '').trim()
        const profileUrl = href.startsWith('http') ? href : (href ? `https://altibbi.com${href.startsWith('/') ? '' : '/'}${href}` : searchUrl)
        out.push({ name, speciality, city, address: '', phone, profileUrl })
      })
    } catch { /* parse failed */ }
  }
  ...
  if (unique.length === 0) {
    return {
      source: 'altibbi',
      results: [{
        name: `دليل altibbi — ${speciality}${city ? ' (' + city + ')' : ''}`.trim(),
        speciality, city, address: '', phone: '',
        profileUrl: searchUrl,
        directoryLink: true,
      }],
  ...
  return { source: 'altibbi', results: unique.slice(0, 12), sourceUrl: searchUrl }
}

// ───────────────────────── Source: SALIM-DZ (Medical Directory) ─────────────────────────
// salim-dz.com — Algerian medical directory with city/specialty filtering
async function fetchSalimDz(speciality, city) {
  const q = `${speciality || ''} ${city || ''}`.trim()
  const encoded = encodeURIComponent(q)

  // Try multiple URL patterns for salim-dz
  const candidates = [
    `https://www.salim-dz.com/search?q=${encoded}&type=doctor`,
    `https://www.salim-dz.com/medecins?specialite=${encodeURIComponent(speciality || '')}&ville=${encodeURIComponent(city || '')}`,
    `https://www.salim-dz.com/?s=${encoded}`,
    `https://www.salim-dz.com/annuaire-medical?q=${encoded}`,
  ]
  ...
  for (const url of candidates) {
    ...
    try {
      const $ = cheerio.load(r.html)
      // Wide selector net for salim-dz card structures
      $([ '[class*="doctor"]', '[class*="medecin"]', '[class*="praticien"]',
          '[class*="card"]', 'article', '.result', '.listing-item', 'li.item',
      ].join(', ')).each((_, el) => {
        const $el = $(el)
        const text = $el.text().replace(/\s+/g, ' ').trim()
        if (!text || text.length < 5) return
        const name = $el.find('h1, h2, h3, h4, a, strong, .name, .title').first().text().trim()
        if (!name || name.length < 3 || name.length > 120) return
        if (!/dr\.?|د\.?|دكتور|docteur|cabinet|clinique|médecin|medecin|طبيب/i.test(name + ' ' + text)) return
        const phone = extractPhone($el, text)
        const addr = $el.find('[class*="addr"], [class*="loc"], [class*="adresse"], address').first().text().trim()
          || $el.find('p, span').filter((_, e) => /rue|avenue|cité|boulevard|commune|شارع|حي/i.test($(e).text())).first().text().trim()
        const specFound = $el.find('[class*="spec"], [class*="special"]').first().text().trim()
        const href = ($el.find('a').first().attr('href') || '').trim()
        const profileUrl = href.startsWith('http') ? href
          : (href ? `https://www.salim-dz.com${href.startsWith('/') ? '' : '/'}${href}` : url)
        out.push({ name, speciality: specFound || speciality || '', city: city || '', address: addr || '', phone, profileUrl })
      })
      if (out.length >= 3) break
    } catch { /* try next URL */ }
  }
  ...
  if (unique.length === 0) {
    return {
      source: 'salim-dz',
      results: [{
        name: `SALIM — ${speciality}${city ? ' (' + city + ')' : ''}`.trim(),
        speciality, city, address: '', phone: '',
        profileUrl: `https://www.salim-dz.com/?s=${encodeURIComponent(q)}`,
        directoryLink: true,
      }],
      sourceUrl: usedUrl,
    }
  }
  return { source: 'salim-dz', results: unique.slice(0, 12), sourceUrl: usedUrl }
}
  ...
  for (const list of perSourceLists) {
    for (const raw of list.results || []) {
      ...
      if (!existing) {
        ...
        existing.sourceMapUrl = existing.sourceMapUrl || raw.sourceMapUrl || ''
...
export function mapLink(name, city, lat, lng, label, address = '') {
  // The visible address is the map target. Prefer the exact address + city;
  // use coordinates only when there is no usable address.
  const addressQuery = [address, city, 'Algérie'].filter(Boolean).join(', ')
  const nameQuery = [name, city, 'Algérie'].filter(Boolean).join(', ')
  const q = encodeURIComponent(addressQuery || nameQuery)
  const gmUrl = q
    ? `https://www.google.com/maps/search/?api=1&query=${q}`
    : (lat && lng ? `https://www.google.com/maps/search/?api=1&query=${lat},${lng}` : '#')
  const display = label || address || city || name || 'خريطة'
...
// Render results as a Markdown table (RTL friendly via leading RLM and dir="auto").
function renderTable(real, fallbackSpec, fallbackCity) {
  // Original DZ Agent doctor result format: one compact, ordered RTL table.
  // Columns intentionally stay limited to doctor, specialty, address and phone.
  // The address itself is the map action; source names are not shown to users.
  const header =
    `${RLM}| # | 👨‍⚕️ الطبيب | 🏥 التخصص | 📍 العنوان | 📞 الهاتف |\\n` +
    `${RLM}|:---:|---|---|---|---|`
  const rows = real.map((d, i) => {
    ...
    const locationLabel = addrTxt
      ? `${addrTxt}، ${cityTxt}`.replace(/^،\s*/, '').replace(/،\s*$/, '') + distStr
      : (cityTxt || 'الجزائر') + distStr
    // The visible address is clickable and targets the selected doctor/address.
    const locationCell = mapLink(cleanName, cityTxt, d.lat, d.lng, locationLabel, addrTxt).replace(/\|/g, '·')
    const phoneCell = d.phone ? telLink(d.phone).replace(/\|/g, '·') : '—'
    return `${RLM}| ${i + 1} | **${name}** | ${spec} | ${locationCell} | ${phoneCell} |`
  })
  ...
  return real.map((d, i) => {
    ...
    if (addrLine || cityLine) {
      const locationLabel = addrLine
        ...
        : cityLine
      // Keep the source address in the Maps query so the selected doctor is targeted.
      addrStr = `${RLM}• ${mapLink(cleanName, cityLine, d.lat, d.lng, locationLabel, addrLine)}${distStr}`
    }
...
export function formatResults(results, specialityLabel, cityLabel, opts = {}) {
  const { hasGps = false, sourceCount = 10, byName = false, queryName = '' } = opts
  ...
  if (!real.length && !dirs.length) {
    return header + '\n\n' +
      `${RLM}> ⚠️ لم أجد نتائج حالياً من ${sourceCount} مصادر طبية جزائرية.\n\n` +
      `${RLM}💡 جرّب ولاية مجاورة أو تخصصاً مرادفاً أو ابحث باسم الطبيب مباشرة.`
  }
  let body = ''
  if (real.length) {
    const withPhone = real.filter(d => d.phone).length
    const withAddr = real.filter(d => d.address || d.city).length
    const stats = [`**${real.length}** طبيب`, withPhone ? `📞 ${withPhone} رقم` : null, withAddr ? `📍 ${withAddr} عنوان` : null, hasGps ? '📡 مرتب حسب القرب' : 'مرتّب حسب اكتمال البيانات'].filter(Boolean).join(' · ')
    body = `${RLM}> ✅ ${stats} · 🔍 ${sourceCount} مصادر طبية جزائرية\n\n` + renderTable(real, specialityLabel, cityLabel)
  } else {
    body = `${RLM}> ⚠️ لم تتوفر بيانات مفصلة — روابط البحث المباشر في المصادر:\n\n` +
      `${RLM}| 🌐 المصدر | 🔗 البحث |\n${RLM}|---|---|\n` +
      dirs.map(d => `${RLM}| **${d.name || d.sources?.[0] || 'دليل طبي'}** | [🔍 فتح](${d.profileUrl}) |`).join('\n')
  }
  let out = header + '\n\n' + body
  if (dirs.length && real.length) {
    out += '\n\n---\n\n' + `${RLM}📂 **مصادر البحث المباشر:**\n\n` +
