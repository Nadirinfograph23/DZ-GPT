...
export const SOURCES = [
  ...
  { id: 'salim-dz',      fetcher: fetchSalimDz },
  { id: 'pj-dz',         fetcher: fetchPjDz },
  { id: 'docteur360',    fetcher: fetchDocteur360 },
  { id: 'sihhatech',     fetcher: fetchSihhatech },
  { id: 'machrou3',      fetcher: fetchMachrou3 },
  { id: 'beesiha',       fetcher: fetchBeesiha },
  { id: 'altibbi',       fetcher: fetchAltibbi },
]
...
function renderTable(real, fallbackSpec, fallbackCity) {
  const header =
    `${RLM}| # | 👨‍⚕️ الاسم | 🏥 التخصص | 📍 العنوان | 📞 الهاتف | 🔗 الملف |\n` +
    `${RLM}|:---:|---|---|---|---|:---:|`
  const rows = real.map((d, i) => {
    ...
    const profileCell = d.profileUrl && !d.directoryLink
      ? `[📋 عرض](${d.profileUrl})`
      : '—'
    return `${RLM}| ${i + 1} | **${name}** | ${spec} | ${locationCell} | ${phoneCell} | ${profileCell} |`
  })
