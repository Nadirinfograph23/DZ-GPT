...
// ── Download from direct URL (Cobalt fallback) ───────────────────
async function fetchToFile(url, dest, job) {
...
async function executeDownload(job) {
  ...
  if (dlpBin) {
    // yt-dlp — up to 3 client rotations
    let lastErr = null
    for (let i = 0; i < 3; i++) {
      try {
  ...
  throw new Error('فشل التحميل — جميع الطرق استُنفذت (yt-dlp + Cobalt)')
}
