...
async function executeDownload(job) {
  ...
  // ── 1.5 FACEBOOK — مسار مخصص (متوازٍ + حلّ الـ redirect) ─────────
  // Facebook لا تعمل مع Viddari — نستدعي social-extract الداخلي
  // الذي يشغّل 7 مستخرجات بالتوازي ويُرجع أول CDN URL يعمل
  const isFacebook = /facebook\.com|fb\.watch|fb\.com/i.test(url)
  if (isFacebook) {
    try {
      emit(id, 'status', { status: 'running', message: '⏳ جارٍ استخراج رابط Facebook...' })
      const extractUrl = `http://127.0.0.1:${process.env.PORT || 5000}/api/dz-agent/social-extract`
      const exResp = await fetch(`${extractUrl}?url=${encodeURIComponent(url)}`, {
        headers: { 'User-Agent': 'DZ-DLMgr/2.0 internal', 'X-Forwarded-For': '127.0.0.1' },
        signal: AbortSignal.timeout(30000),
      })
      if (exResp.ok) {
        const info = await exResp.json().catch(() => null)
        const cdnUrl = (info?.video || [])[0]?.url
        if (cdnUrl) {
          const ext  = format === 'audio' ? 'm4a' : 'mp4'
          const dest = path.join(DL_TMP_DIR, `${id}.${ext}`)
          job.ext  = ext
          if (info.title && !job.title) job.title = info.title
          emit(id, 'status', { status: 'running', message: '📥 Facebook ✅ — جارٍ التحميل...' })
          ...
          if (fs.existsSync(dest) && fs.statSync(dest).size > 1024) {
            ...
            job.status   = 'done'
            emit(id, 'done', { ext, size: fs.statSync(dest).size, filename: job.filename })
            console.log(`[DLMgr:${id}] ✅ Facebook parallel extraction success (${info.source})`)
            return
          }
        }
      }
    } catch (e) {
      console.warn(`[DLMgr:${id}] Facebook extractor failed: ${e.message?.slice(0, 100)}`)
      emit(id, 'retry', { attempt: 0, error: 'استخراج Facebook فشل — جارٍ تجربة yt-dlp...' })
    }
  }
