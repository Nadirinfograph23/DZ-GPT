...
async function getYtDlpCookies() {
  try {
    ...
    if (process.env.YOUTUBE_COOKIES) {
      ...
      fs.writeFileSync(p, process.env.YOUTUBE_COOKIES, { mode: 0o600 })
      return ['--cookies', p]
    }
  } catch {}
  return []
...
// TESTED 2026-07-30: android is the ONLY reliable client from datacenter IPs.
// ios / mweb require GVS PO Token → "No video formats found" on datacenter IPs.
// android always returns at least format 18 (360p muxed mp4) — no PO Token needed.
const YT_CLIENTS = ['android', 'android_vr', 'tv', 'web_safari', 'mweb', 'ios']
const USER_AGENTS = [
...
function ytDlpDownload(bin, url, fmt, outTpl, cookies, clientIdx, job) {
  return new Promise((resolve, reject) => {
    ...
    const isAudio = job.format === 'audio'
    const args = [
      '-f', fmt,
      '-o', outTpl,
      ...
      '--socket-timeout', '30',
      // 2026: ffmpeg is available — for audio downloads, extract audio track from
      // muxed format 18 (360p mp4) so the output is clean m4a instead of full video.
      ...(isAudio ? ['-x', '--audio-format', 'm4a', '--audio-quality', '0'] : ['--merge-output-format', 'mp4']),
      ...cookies,
      url,
    ]
...
async function fetchToFile(url, dest, job, extraHeaders = {}) {
  // إضافة تلقائية لـ headers خاصة بفيسبوك — fbcdn.net تتطلب Referer صريحاً
  if (!extraHeaders['Referer'] && (url.includes('fbcdn.net') || url.includes('fbsbx.com'))) {
    extraHeaders = {
...
async function executeDownload(job) {
  ...
  // ── 1.5 SOCIAL EXTRACT — مسار داخلي موحّد لجميع المنصات ─────────────
  // يستدعي /api/social/extract الذي يُشغّل extractors متخصصة لكل منصة
  // (tikwm لـ TikTok, saveclip لـ Instagram, fxtwitter لـ X/Twitter, إلخ)
  // مع Fallback تلقائي لـ Cobalt ثم yt-dlp عبر orchestrateMediaDownload
  const isSocialPlatform = /facebook\.com|fb\.watch|fb\.com|tiktok\.com|instagram\.com|reddit\.com|redd\.it|x\.com|twitter\.com|pinterest\.com/i.test(url)
  if (isSocialPlatform) {
    try {
      const platformLabel = /facebook|fb\.watch|fb\.com/.test(url) ? 'Facebook'
        : /tiktok/.test(url) ? 'TikTok'
        : /instagram/.test(url) ? 'Instagram'
        : /reddit|redd\.it/.test(url) ? 'Reddit'
        : /x\.com|twitter/.test(url) ? 'X/Twitter'
        : /pinterest/.test(url) ? 'Pinterest'
        : 'Social'
      emit(id, 'status', { status: 'running', message: `⏳ جارٍ استخراج رابط ${platformLabel}...` })
      const extractUrl = `http://127.0.0.1:${process.env.PORT || 5000}/api/social/extract`
      const exResp = await fetch(`${extractUrl}?url=${encodeURIComponent(url)}`, {
        headers: { 'User-Agent': 'DZ-DLMgr/2.0 internal' },
        signal: AbortSignal.timeout(40000),
      })
      if (exResp.ok) {
        const info = await exResp.json().catch(() => null)
        const cdnUrl = (info?.video || [])[0]?.url || (info?.audio || [])[0]?.url
        if (cdnUrl) {
          const ext  = format === 'audio' ? ((info?.audio || [])[0]?.ext || 'm4a') : 'mp4'
          const dest = path.join(DL_TMP_DIR, `${id}.${ext}`)
          job.ext = ext
          if (info.title && !job.title) job.title = info.title
          emit(id, 'status', { status: 'running', message: `📥 ${platformLabel} ✅ — جارٍ التحميل...` })
          // fbcdn.net URLs تحتاج Referer — fetchToFile يضيفها تلقائياً
          await fetchToFile(cdnUrl, dest, job)
          if (fs.existsSync(dest) && fs.statSync(dest).size > 1024) {
            job.filePath = dest; job.ext = ext
            job.status   = 'done'
            emit(id, 'done', { ext, size: fs.statSync(dest).size, filename: job.filename })
            console.log(`[DLMgr:${id}] ✅ ${platformLabel} social-extract success (${info.provider || info.source})`)
            return
          }
        }
      }
    } catch (e) {
      console.warn(`[DLMgr:${id}] social-extract failed: ${e.message?.slice(0, 100)}`)
      emit(id, 'retry', { attempt: 0, error: 'social-extract فشل — جارٍ تجربة yt-dlp...' })
    }
  }
  ...
  let fmtSel
  if (format === 'audio') {
    // 2026 FIX: From datacenter IPs (Vercel/Replit), YouTube android client returns
    // ONLY format 18 (360p muxed mp4). Audio-only DASH formats require PO Token.
    // Chain: prefer pure m4a → mp3 → any bestaudio → format 18 → absolute best.
    fmtSel = 'bestaudio[ext=m4a]/bestaudio[ext=mp3]/bestaudio/18/best'
    job.ext = 'm4a'
  } else {
    const h = parseInt(quality) || 0
    const hStr = h >= 1080 ? '1080' : h >= 720 ? '720' : h >= 480 ? '480' : h >= 360 ? '360' : ''
    // 2026 FIX: DASH bestvideo requires PO Token from datacenter IPs.
    // Final fallback 18/best guarantees a working muxed 360p download.
    fmtSel = hStr
      ? `bestvideo[height<=${hStr}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=${hStr}]+bestaudio/best[height<=${hStr}]/18/best`
      : 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/18/best'
    job.ext = 'mp4'
  }
