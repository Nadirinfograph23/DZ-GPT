...
// ── Entity Definition Guard (مستورد بشكل lazy لتجنب circular imports) ────────
let _entityGuard = null
async function _getEntityGuard() {
  if (!_entityGuard) {
    try { _entityGuard = await import('./entity-question-guard.js') } catch { _entityGuard = { isEntityDefinitionQuery: () => false } }
  }
  return _entityGuard
}

// نسخة متزامنة تعتمد على cache بعد أول استخدام
let _entityGuardSync = null
;(async () => { _entityGuardSync = await _getEntityGuard() })()
...
export function detectTimeSensitiveIntent(query = '') {
  ...
  // "من هو وزير الخارجية الحالي؟" ليست حدثاً زمنياً — هي سؤال تعريف كيان
  // الصفات: الحالي/السابق/الجديد لا تعني خبراً حياً في سياق التعريف
  if (_entityGuardSync?.isEntityDefinitionQuery?.(q)) {
    return { isTimeSensitive: false, eventType: 'entity_definition', confidence: 0, searchQuery: q, trigger: 'entity_guard' }
  }
  // ─────────────────────────────────────────────────────────────────────────
