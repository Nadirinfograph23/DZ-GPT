...
const SYMPTOM_KEYWORDS = [
  ...
  // أعراض جلدية وتحسسية — مهمة جداً
  'احمرار', 'حكة شديدة', 'تهيج', 'تهيج جلدي', 'حساسية', 'رد فعل تحسسي',
  'بثور', 'قشرة', 'تقشر', 'جلد جاف', 'جلد متشقق', 'أكزيما', 'اكزيما',
  'صدفية', 'شرى', 'طفح جلدي', 'طفح تحسسي', 'حرق في الجلد', 'وخز في الجلد',
  'جلد أحمر', 'بقع حمراء', 'بقع على الجلد', 'حكة في', 'احمرار في',
  // دارجة جزائرية
  ...
  'جلدي يحكني', 'يحكني', 'وجهي احمر', 'يدي تحكني', 'رجلي تحكني',
  'جلدي متهيج', 'جلدي احمر', 'عندي حساسية', 'عندي حكة',
  // فرنسية
  ...
  'démangeaison', 'rougeur', 'éruption', 'eczéma', 'psoriasis', 'urticaire',
  // مصطلحات طبية
  'ضغط', 'سكري', 'كوليسترول', 'ربو', 'أنيميا', 'فقر دم', 'صرع',
  'شلل', 'سرطان', 'قرحة', 'حصوة', 'حصى', 'ورم', 'خراج', 'ناسور',
  'هرمونات', 'درقية', 'غدة', 'بنكرياس', 'رئة', 'قلب',
  'التهاب جلدي', 'داء جلدي', 'حساسية جلدية', 'قصور كلوي', 'التهاب كلوي',
]
...
// ──────────────────────────────────────────────────────────────────────
// SECTION 1b: Capability Self-Query Guard
// ──────────────────────────────────────────────────────────────────────
// أسئلة عن وجود الأداة/الوكيل (ليست أعراضاً طبية) → NOT health agent
// مثال: "هل تملك وكيل صحي؟" | "عندك أداة طبية؟"
const CAPABILITY_QUERY_GUARD_RE = /(?:هل\s+(?:تملك|عندك|لديك|عندك|يوجد)\s*(?:وكيل|أداة|خدمة|ميزة|تطبيق)|(?:عندك|تملك|لديك)\s+(?:وكيل|أداة).*(?:صح|طب|صحة|طبي|صحي|health)|(?:وكيل|أداة)\s+(?:صحي|صحية|طبي|طبية)|ما\s+(?:هو|هي)\s+وكيل\s+الصحة|أين\s+(?:وكيل|أداة)\s+الصحة|كيف\s+أ(?:صل|ستخدم|وصل)\s+(?:لـ|إلى|ل)?\s*(?:وكيل|أداة)\s+الصحة|health\s+(?:tool|agent|feature)|do\s+you\s+have.*health)/i
...
const DZ_HEALTH_KB = [
  {
    name: 'الحساسية الجلدية / التهاب الجلد التماسي',
    prevalence: 'شائع جداً — يصيب 15-20% من السكان بشكل أو بآخر',
    symptoms: ['احمرار', 'حكة', 'حكة شديدة', 'طفح جلدي', 'بثور', 'تورم', 'تهيج', 'جلد جاف', 'أكزيما', 'شرى', 'بقع حمراء'],
    advice: 'تجنب المادة المثيرة للحساسية — استخدم مرطبات للجلد — تجنب الصابون القوي — استشر طبيب جلدية إذا استمر أكثر من أسبوع',
    drugs: ['كريم هيدروكورتيزون 1% (بدون وصفة للحالات الخفيفة)', 'مضادات الهيستامين مثل سيتيريزين (بدون وصفة)', 'مرطب جلد مثل السيتافيل', 'كريم كالامين للحكة'],
    triage: 'LOW',
  },
  {
    name: 'الأكزيما (التهاب الجلد الأتوبي)',
    prevalence: 'شائع — يصيب 10-15% خاصة الأطفال والشباب',
    symptoms: ['حكة', 'احمرار', 'جلد جاف', 'تقشر', 'بثور', 'تهيج جلدي', 'أكزيما'],
    advice: 'ترطيب الجلد يومياً — تجنب العوامل المثيرة (غبار، صابون، توتر) — الاستحمام بماء فاتر لا ساخن — ملابس قطنية',
    drugs: ['مرطبات مكثفة (سيتافيل، فازلين)', 'كريم تاكروليموس أو كورتيكوستيرويدات موضعية (بوصفة)', 'مضادات الهيستامين للحكة الليلية'],
    triage: 'LOW',
  },
  {
    name: 'مرض السكري',
...
// ──────────────────────────────────────────────────────────────────────
// SPORTS GUARD — كلمات رياضية تمنع اعتراض Health Agent
// مثال: "القادم" تنتهي بـ "دم" (blood) → false positive بدون هذا الـ guard
// ──────────────────────────────────────────────────────────────────────
const SPORTS_GUARD_RE = /(?:مباراة|مباريات|نتيجة|نتائج|ترتيب|هداف|دوري|بطولة|كأس|ملعب|لاعب|تسجيل|هدف|أهداف|تشكيلة|مدرب|انتقال|ميسي|رونالدو|مبابي|بنزيمة|زيدان|نادي|ريال|برشلونة|مانشستر|يلعب|يلاقي|القادم|الجولة|الموسم|فريق|أندية|منتخب|الخضر|مولودية|اتحاد.*الجزائر|شبيبة.*القبائل|وفاق.*سطيف|اتحاد.*العاصمة|شباب.*بلوزداد|JSK|MCA|USMA|CRB|ESS|CSC|MCO|NAHD|CRBélimane)/i
...
function hasAny(text, list) {
  ...
  return list.some(k => {
    ...
    // للكلمات القصيرة (≤3 أحرف): نشترط حدود الكلمة (مسبوقة ومتبوعة بمسافة/بداية/نهاية)
    // هذا يمنع false positives مثل "دم" في "القادم" أو "دموي" في "دموية المباراة"
    if (kl.length <= 3) {
      return new RegExp(`(^|\\s|[،,.:؟?!])${kl}(\\s|[،,.:؟?!]|$)`).test(t)
    }
...
export function detectHealthIntent(message) {
  ...
  // Guard: Sports queries → NOT health agent
  // يمنع أسئلة مثل "اتحاد الجزائر القادم" من التطابق مع "دم" (blood)
  if (SPORTS_GUARD_RE.test(text)) {
    return { isHealthQuery: false, isDoctorSearch: false, isEmergency: false, symptoms: [], hasDrugQuestion: false }
  }

  // Guard: Capability self-queries → NOT health agent (تُوجَّه لـ self-query handler)
  // مثال: "هل تملك وكيل صحي؟" → self-query handler يشرح الأداة بالتفصيل
  if (CAPABILITY_QUERY_GUARD_RE.test(text)) {
    return { isHealthQuery: false, isDoctorSearch: false, isEmergency: false, symptoms: [], hasDrugQuestion: false }
  }
...
export function parseHealthResponse(rawContent, originalQuery, symptoms) {
  // إذا كان المحتوى فارغاً — استخدم KB fallback
  if (!rawContent || rawContent.trim().length < 10) {
    return buildKBFallbackResponse(symptoms, false, originalQuery)
  }
  try {
    // محاولة 1: استخراج JSON من code block
    let jsonStr = null
    const jsonMatch = rawContent.match(/```json\s*([\s\S]*?)\s*```/)
    if (jsonMatch) {
      jsonStr = jsonMatch[1]
    } else {
      // محاولة 2: JSON خام بدون code block
      const rawMatch = rawContent.match(/\{[\s\S]*"interpretation"[\s\S]*\}/)
      if (rawMatch) jsonStr = rawMatch[0]
    }

    const data = JSON.parse(jsonStr || rawContent.trim())

    return {
      ok: true,
      interpretation:  data.interpretation    || 'تحليل الأعراض المذكورة',
      ...
      suggest_doctor:  data.suggest_doctor !== false,
    ...
    // محتوى نصي بدون JSON — استخدم النص الخام كـ interpretation مع KB fallback
    const kbFallback = buildKBFallbackResponse(symptoms, false, originalQuery)
    return {
      ...kbFallback,
      interpretation: rawContent.slice(0, 500) || kbFallback.interpretation,
    }
  }
}

// ──────────────────────────────────────────────────────────────────────
// MAIN EXPORT 3b: buildKBFallbackResponse
// بناء رد طبي كامل من قاعدة البيانات المحلية — بدون LLM
// ──────────────────────────────────────────────────────────────────────
export function buildKBFallbackResponse(symptoms = [], hasDrugQuestion = false, originalQuery = '') {
  // ابحث عن مطابقات في KB
  const matches = []
  const symp = (symptoms.join(' ') + ' ' + (originalQuery || '')).toLowerCase()
  for (const entry of DZ_HEALTH_KB) {
    const hits = entry.symptoms.filter(s => symp.includes(s.toLowerCase())).length
    if (hits > 0) matches.push({ entry, hits })
  }
  matches.sort((a, b) => b.hits - a.hits)

  // بناء الرد من المطابقات
  const topMatch = matches[0]?.entry || null
  const allAdvice = [...new Set(matches.flatMap(m => [m.entry.advice]))]
  const allDrugs  = [...new Set(matches.flatMap(m => m.entry.drugs))]
  const causes    = matches.map(m => m.entry.name).slice(0, 3)
  const triageLevels = matches.map(m => m.entry.triage)
  const triage    = triageLevels.includes('HIGH') ? 'HIGH'
    : triageLevels.includes('MEDIUM') ? 'MEDIUM' : 'LOW'

  // تفسير بناءً على الأعراض
  const symptomText = symptoms.length > 0
    ? `أعراض من نوع: ${symptoms.join('، ')}`
    : 'الأعراض المذكورة'

  const interpretation = topMatch
    ? `بناءً على ${symptomText}، قد تشير هذه الأعراض إلى ${topMatch.name}. ${topMatch.prevalence ? `(${topMatch.prevalence})` : ''} يُنصح باستشارة طبيب للتشخيص الدقيق.`
    : `تشير ${symptomText} إلى حالة تستوجب متابعة طبية. الأسباب متعددة وتختلف حسب كل حالة. يُنصح بالاستشارة الطبية.`

  const advice = allAdvice.length > 0 ? allAdvice : [
    'استرح جيداً واشرب كميات كافية من الماء',
    'راقب الأعراض — إذا ازدادت سوءاً استشر طبيباً',
    'لا تأخذ أدوية بدون وصفة طبية إلا ما هو متعارف عليه',
  ]

  return {
    ok: true,
    interpretation,
    possible_causes: causes.length > 0 ? causes : ['حالة عامة تستوجب متابعة طبية'],
    triage_level:    triage,
    triage_reason:   triage === 'HIGH'
      ? 'أعراض تستوجب تقييماً طبياً سريعاً'
      : triage === 'MEDIUM'
      ? 'أعراض متوسطة — يُنصح بمراجعة الطبيب'
      : 'أعراض خفيفة — يمكن المتابعة المنزلية',
    advice,
    medications_info: allDrugs.length > 0
      ? `أدوية شائعة لهذه الحالات (تتطلب وصفة طبية في معظمها): ${allDrugs.join('، ')}`
      : null,
    suggest_doctor:  triage !== 'LOW',
    emergency_note:  triage === 'HIGH' ? '⚠️ إذا كانت الأعراض شديدة — اتصل بـ 14 أو 115' : null,
    disclaimer:      'هذا ليس تشخيصاً طبياً — استشر طبيباً مختصاً للتشخيص الدقيق',
    symptoms_found:  symptoms,
    original_query:  (originalQuery || '').slice(0, 120),
    source:          'kb_fallback',
  }
