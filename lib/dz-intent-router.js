 ...
 *
 * الأولويات:
 *  0. وظائف/مسابقات (JOBS_CONCOURS) — أعلى أولوية — بحث حي إلزامي
 *  1. سؤال صريح
 *  2. كيان مسمّى
 *  3. ذاكرة السياق
 *  4. الرسالة السابقة
...
// ── استيراد كاشفات النوايا المعزولة (لا تعارض) ───────────────────────────
import { isJobsQuery } from './dz-jobs-intent.js'
import { isImageSearchQuery } from './image-search/index.js'
...
export const INTENTS = {
  GREETING:          'GREETING',
  JOBS_CONCOURS:     'JOBS_CONCOURS',   // ★ وظائف ومسابقات — أولوية قبل كل شيء
  IMAGE_SEARCH:      'IMAGE_SEARCH',    // ★ بحث عن صور حقيقية (Pinterest/Wikimedia/DDG)
...
export const SOURCES = {
  NONE:                'none',
  JOBS_CONCOURS_LIVE:  'jobs-concours-live-search',  // ★ بحث حي في الوظائف والمسابقات
  IMAGE_SEARCH_LIVE:   'image-search-pinterest-wikimedia-ddg', // ★ بحث صور حقيقية
...
const DARIJA_MAP = [
  ...
  { re: /(شحال|كم)\s+(ربح|فاز|نتيجة|سكور)\s+(ريال\s+مدريد|برشلونة|الجزائر|الخضر|الأهلي)/i,
    intent: 'SPORTS_RESULTS' },
  // نمط "X ضد Y" / "X vs Y" — تحليل مباراة أو نتيجة مباشرة
  { re: /[\u0600-\u06FF\w]{2,}\s+ضد\s+[\u0600-\u06FF\w]{2,}/i,
    intent: 'SPORTS_RESULTS' },
  { re: /[\u0600-\u06FF\w]{2,}\s+vs\.?\s+[\u0600-\u06FF\w]{2,}/i,
    intent: 'SPORTS_RESULTS' },
...
const SPORTS_RESULTS_PATTERNS = [
  ...
  // نمط "X ضد Y" — تحليل مباراة أو نتيجة (مثال: الجزائر ضد بوليفيا)
  /[\u0600-\u06FF\w]{2,}\s+ضد\s+[\u0600-\u06FF\w]{2,}/,
  /[\u0600-\u06FF\w]{2,}\s+vs\.?\s+[\u0600-\u06FF\w]{2,}/i,
]
...
export function classifyIntent(message = '', history = []) {
  ...
  // ── STEP 1.5a: IMAGE SEARCH GUARD (أولوية عالية — قبل التصنيف العام) ─────
  // الصور الحقيقية يجب أن تذهب لمحرك البحث عن الصور — لا للأخبار أو التاريخ
  if (isImageSearchQuery(msg)) {
    return _result(
      INTENTS.IMAGE_SEARCH, 95, {}, 'search-images-pinterest-wikimedia', SOURCES.IMAGE_SEARCH_LIVE,
      false, null, null, 'image-search-fastpath'
    )
  }

  // ── STEP 1.5b: JOBS/CONCOURS GUARD (أولوية قصوى — قبل كل شيء آخر) ───────
  // الوظائف والمسابقات تتغير يومياً → لا يمكن الإجابة من معرفة التدريب أبداً
  // يجب أن يُصنَّف كـ JOBS_CONCOURS ليُفعَّل البحث الحي في server.js
  if (isJobsQuery(msg)) {
    return _result(
      INTENTS.JOBS_CONCOURS, 97, {}, 'search-jobs-concours-live', SOURCES.JOBS_CONCOURS_LIVE,
      false, null, null, 'jobs-concours-fastpath'
    )
  }
...
function _getActionAndSource(intent, entities = {}) {
  switch (intent) {
    ...
    // ★ IMAGE_SEARCH — بحث حقيقي في Pinterest + Wikimedia + DDG
    case INTENTS.IMAGE_SEARCH:
      return { action: 'search-images-pinterest-wikimedia', source: SOURCES.IMAGE_SEARCH_LIVE }

    // ★ JOBS_CONCOURS — بحث حي إلزامي في الوظائف والمسابقات الجزائرية
    case INTENTS.JOBS_CONCOURS:
      return { action: 'search-jobs-concours-live', source: SOURCES.JOBS_CONCOURS_LIVE }
...
export function buildIntentBlock(classification) {
  ...
  // ★ IMAGE_SEARCH — إضافة تعليمات صريحة للنموذج
  if (intent === INTENTS.IMAGE_SEARCH) {
    block += `\n🖼️ [IMAGE_SEARCH_INTENT — ACTIVE]\n`
    block += `⛔ النية = IMAGE_SEARCH — هذا طلب صور حقيقية وليس أخباراً:\n`
    block += `  ① ❌ ممنوع البحث في الأخبار أو الويكيبيديا كمصدر نصي — هذا بحث عن صور\n`
    block += `  ② ✅ المصادر المستخدمة: Pinterest + Wikimedia Commons + DuckDuckGo Images\n`
    block += `  ③ ✅ للصور التاريخية/النادرة → يُفضَّل Pinterest لأنه يحتوي أرشيف واسع\n`
    block += `  ④ ✅ اعرض النتائج الواردة كصور منسقة مع الرابط والمصدر\n`
    block += `  ⑤ ❌ ممنوع الإجابة النصية عن الموضوع — الهدف عرض الصور فقط\n`
    block += `[/IMAGE_SEARCH_INTENT]\n`
  }

  // ★ JOBS_CONCOURS — إضافة تعليمات صريحة للنموذج
  if (intent === INTENTS.JOBS_CONCOURS) {
    block += `\n🚨 [DZ_JOBS_CONCOURS_INTENT — ACTIVE]\n`
    block += `⛔ النية = JOBS_CONCOURS — قواعد OVERRIDE سارية بشكل مطلق:\n`
    block += `  ① ❌ ممنوع الإجابة من بيانات التدريب — الوظائف تتغير يومياً\n`
    block += `  ② ✅ اقرأ [DZ_JOBS_SEARCH_RESULTS] الموجود أسفله واعرض نتائجه كبطاقات منظّمة\n`
    block += `  ③ ✅ إذا كانت النتائج فارغة → اقترح المصادر الرسمية (mfp.gov.dz, anem.dz, emploitic.com)\n`
    block += `  ④ ❌ قاعدة "ما هو X = إجابة مباشرة" لا تنطبق هنا — انتظر [DZ_JOBS_SEARCH_RESULTS]\n`
    block += `[/DZ_JOBS_CONCOURS_INTENT]\n`
  }
