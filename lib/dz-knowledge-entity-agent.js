...
const MIN_CONFIDENCE = 0.72   // خفّضنا للسماح بمزيد من النتائج مع تدرّج الثقة
...
const KNOWLEDGE_TRIGGERS = [
  ...
  // الرئيس الأمريكي / الفرنسي... الحالي / السابق
...
function _buildEntityResponse({ entity, entityType, wikidataResult, wikiExtract, dbpediaResult, confidence, roleData, originalQuery }) {
  ...
  if (wikiExtract?.extract && wikiExtract.extract.length > 30 && entityType !== ENTITY_TYPE.ROLE_QUERY) {
    lines.push('### 📖 نبذة', '')
    lines.push(wikiExtract.extract.slice(0, 900).trim())
    lines.push('')
  ...
  if (roleData?.personId) {
    lines.push(`| 🔗 **Wikidata (SPARQL)** | [${roleData.personId}](https://www.wikidata.org/wiki/${roleData.personId}) | ${pct}% |`)
  }
  if (wikiExtract?.url) {
  ...
  lines.push('')
  lines.push(`> 🗓️ *آخر تحقق: ${now} — مصادر: Wikidata / Wikipedia / DBpedia*`)
  lines.push(`> ⚠️ *LLM يُستخدم للتنسيق فقط — المصادر أعلاه هي المرجع الحقيقي*`)
...
export async function resolveKnowledgeEntity(query = '') {
  ...
  if (hasNoResults || confidence < MIN_CONFIDENCE) {
    const result = {
