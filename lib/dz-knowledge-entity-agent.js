...
const LIVE_SEARCH_TRIGGERS = [
  ...
  /(?:breaking|عاجل:?\s)/i,
  // ✅ FIX: أسئلة "ما هي أبرز/أهم/أحدث أخبار X هذا الأسبوع" — أخبار حية وليست تعريف كيان
  /(?:^|\s)(?:ما\s+(?:هو|هي|هم))\s+(?:أبرز|أهم|أحدث|آخر)\s+(?:أخبار|خبر)/i,
  /(?:^|\s)(?:ما\s+(?:هو|هي|هم))\s+(?:أخبار|خبر)\s+/i,
  // ✅ FIX: أي جملة تحتوي "أبرز أخبار / أهم أخبار" → بحث حي
  /(?:أبرز|أهم)\s+(?:أخبار|خبر)\s+/i,
  // ✅ FIX: "هذا الأسبوع / هذا الشهر" مع أخبار → دائماً بحث حي
  /(?:أخبار|خبر).{0,30}(?:هذا\s+الأسبوع|هذا\s+الشهر|هذا\s+العام|الأسبوع\s+الماضي)/i,
  /(?:هذا\s+الأسبوع|هذا\s+الشهر).{0,30}(?:أخبار|خبر)/i,
]
...
function _buildCapitalResponse(countryAr, data) {
  return [
    ...
    `| 👥 عدد السكان | ${data.pop} |`,
    ``,
    `> 📅 *معلومات جغرافية ثابتة — DZ AGENT 2026*`,
  ].join('\n')
}
