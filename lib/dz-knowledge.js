...
export const IMPOSSIBLE_DZ_ENTITIES = [
  ...
  {
    pattern: /(?:من\s+هو\s+|شكون\s+(?:هو\s+)?)?رئيس\s+(?:ال)?إفريقيا(?:\s+(?:حاليا?|الحالي|الآن|درك|دروك))?/i,
    response: [
...
/**
 * هل يتعلق السؤال بكيان مستحيل (ملك الجزائر، رئيس إفريقيا...)?
 */
export function isImpossibleDZEntity(text) {
  return IMPOSSIBLE_DZ_ENTITIES.find(e => e.pattern.test(text)) || null
}
