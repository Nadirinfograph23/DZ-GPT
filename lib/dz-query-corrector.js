...
const STOPWORDS = new Set([
  ...
  // ── أسماء لاعبين محمية (لا تُصحَّح كأسماء فرق) ──────────────────────────
  'هالاند','هالند','mbappe','مبابي','mbappé','رونالدو','ronaldo',
  'ميسي','messi','صلاح','salah','نيمار','neymar','محرز','mahrez',
  'بنزيمة','benzema','مانه','mane','زيدان','زيدان','haaland',
  'بنناصر','bennacer','عطال','atal','بوعناجه','bounedjah',
]);
...
// إزالة علامات الترقيم من الطرفين لمقارنة الـ STOPWORDS بشكل صحيح
...
function fixTeamNames(text) {
  ...
  while (i < tokens.length) {
    ...
    const tok0c = stripPunct(tok0)
    const tok1c = tok1 ? stripPunct(tok1) : null
    const tok2c = tok2 ? stripPunct(tok2) : null

    // ① جرب ثلاث كلمات معاً — فقط إذا لا تحتوي stopwords
    if (tok1c && tok2c && !STOPWORDS.has(tok0c) && !STOPWORDS.has(tok1c) && !STOPWORDS.has(tok2c)) {
      const phrase = `${tok0c} ${tok1c} ${tok2c}`
    ...
    if (tok1c && !STOPWORDS.has(tok0c) && !STOPWORDS.has(tok1c)) {
      const phrase = `${tok0c} ${tok1c}`
      const fix = fuzzyFindTeam(phrase)
      if (fix) {
        outTokens.push(...fix.corrected.split(' '))
        corrections.push({ type: 'team', from: phrase, to: fix.corrected })
    ...
    if (!STOPWORDS.has(tok0c)) {
      const fix = fuzzyFindTeam(tok0c)
      if (fix) {
