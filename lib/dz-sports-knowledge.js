...
export function detectWC2026TodayQuery(query = '') {
  ...
  const _now = Date.now()
  const WC_SEASON_START = 1749600000000 // 2026-06-11T00:00:00Z
  const WC_SEASON_END   = 1753055999000 // 2026-07-20T23:59:59Z
...
export function buildWC2026RichMatchCard(fix = {}) {
  ...
  if (fix.startTime) {
    const [_stH, _stM] = fix.startTime.split(':')
    const _dzH = String((parseInt(_stH, 10) + 1) % 24).padStart(2, '0')
    lines.push(`⏰ **الموعد:** ${_dzH}:${_stM} (بتوقيت الجزائر)`)
  }
