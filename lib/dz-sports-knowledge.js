...
export function detectWC2026TodayQuery(query = '') {
  ...
  if (/(?:مباريات|نتائج|برنامج|جدول).*(?:كأس\s+العالم|مونديال|FIFA).*(?:اليوم|الليلة|الآن)/i.test(query)) return true
  if (/(?:كأس\s+العالم|مونديال|FIFA).*(?:مباريات|نتائج|برنامج|جدول).*(?:اليوم|الليلة)/i.test(query)) return true
  return false
}
...
export function buildWC2026RichMatchCard(fix = {}) {
  ...
  const hasScore = fix.homeScore !== null && fix.homeScore !== undefined && fix.awayScore !== null
  ...
  const dateStr = (() => {
    try {
      return new Date(fix.date).toLocaleDateString('ar-DZ', {
  ...
  const scoreDisplay = hasScore
    ? `## ${f1} ${fix.homeTeam} 🆚 ${fix.awayTeam} ${f2}\n### 🎯 النتيجة: **${fix.homeScore} – ${fix.awayScore}**`
    : `## ${f1} ${fix.homeTeam} 🆚 ${fix.awayTeam} ${f2}`

  const rows = [
    `| 🏷️ الحالة  | ${statusBadge} |`,
    `| 🏆 البطولة | **${fix.competition || 'كأس العالم FIFA 2026'}** |`,
    `| 🎯 الجولة  | **${fix.round || 'دور المجموعات'}** |`,
    `| 📅 التاريخ | **${dateStr}** |`,
  ]
  if (fix.startTime) rows.push(`| ⏰ التوقيت | **${fix.startTime}** (بتوقيت الجزائر) |`)
  if (fix.venue)     rows.push(`| 🏟️ الملعب  | **${fix.venue}** |`)
  if (fix.city)      rows.push(`| 📍 المدينة | **${fix.city}**${fix.country ? `, ${fix.country}` : ''} |`)
  ...
  if (fix.kooraLink) lines.push(``, `🔗 [تفاصيل المباراة على Kooora](${fix.kooraLink})`)
...
export const WC2026_FULL_FIXTURES = [
  ...
  { group: 'A', homeTeam:'المكسيك',          awayTeam:'جنوب أفريقيا',       date:'2026-06-11', startTime:'03:00', venue:'Estadio Azteca',               city:'Mexico City',   country:'المكسيك',      statusType:'upcoming', competition:'كأس العالم FIFA 2026', round:'الجولة 1' },
  ...
  { group: 'B', homeTeam:'كندا',              awayTeam:'البوسنة والهرسك',    date:'2026-06-12', startTime:'02:00', venue:'BC Place',                     city:'Vancouver',     country:'كندا',         statusType:'upcoming', competition:'كأس العالم FIFA 2026', round:'الجولة 1' },
  ...
  { group: 'D', homeTeam:'الولايات المتحدة', awayTeam:'باراغواي',           date:'2026-06-12', startTime:'19:00', venue:'MetLife Stadium',               city:'New York',      country:'الولايات المتحدة', statusType:'upcoming', competition:'كأس العالم FIFA 2026', round:'الجولة 1' },
  ...
  { group: 'J', homeTeam:'الأرجنتين',         awayTeam:'الجزائر',            date:'2026-06-17', startTime:'22:00', venue:'MetLife Stadium',               city:'East Rutherford', country:'الولايات المتحدة', statusType:'upcoming', competition:'كأس العالم FIFA 2026', round:'الجولة 1' },
  ...
  { group: 'J', homeTeam:'الجزائر',           awayTeam:'النمسا',             date:'2026-06-21', startTime:'22:00', venue:'Arrowhead Stadium',             city:'Kansas City',   country:'الولايات المتحدة', statusType:'upcoming', competition:'كأس العالم FIFA 2026', round:'الجولة 2' },
  ...
  { group: 'J', homeTeam:'الأردن',            awayTeam:'الجزائر',            date:'2026-06-25', startTime:'22:00', venue:'AT&T Stadium',                  city:'Dallas',        country:'الولايات المتحدة', statusType:'upcoming', competition:'كأس العالم FIFA 2026', round:'الجولة 3' },
...
export function buildWC2026TodayFixtures(dateStr = '') {
  ...
  return WORLD_CUP_2026.algeriaGroup.fixtures.filter(f => f.date === date)
}
...
export function buildWC2026MatchVsResponse(team1 = '', team2 = '') {
  ...
  if (fixture) {
    ...
    const now = new Date()
    const matchStart = new Date(fixture.date + 'T' + fixture.startTime + ':00Z')
    ...
    // توقيت الجزائر (UTC+1)
    const [hStr, mStr] = fixture.startTime.split(':')
    const dztHour = (parseInt(hStr, 10) + 1) % 24
    const dztLabel = `${String(dztHour).padStart(2, '0')}:${mStr} (بتوقيت الجزائر)`
