...
export function detectWC2026TodayQuery(query = '') {
  const WC_KW = /كأس\s*العالم|المونديال|مونديال|world\s*cup|FIFA|فيفا\s*2026/i
  const TODAY_KW = /اليوم|الليلة|الآن|هاذ\s+اليوم|هذا\s+اليوم|النهار/i
  return WC_KW.test(query) && TODAY_KW.test(query)
}
