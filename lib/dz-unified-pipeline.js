...
import { getAgentIdentityBlock, getCapabilityResponse } from './dz-agent-identity.js'
...
export function enrichQueryContext(query = '', history = []) {
  ...
  // ① ب — سؤال عن خدمة محددة (توليد صور / بناء مواقع / GitHub / ...)
  const capabilityResp = getCapabilityResponse(query)
  if (capabilityResp) {
    const _capContent = typeof capabilityResp === 'object' ? (capabilityResp.content || '') : String(capabilityResp)
    parts.push(
      `## 🎯 توجيه الخدمة — معلومات مهمة للنموذج\n` +
      `المستخدم يسأل عن خدمة بعينها. استخدم هذه المعلومات الجاهزة كأساس لردك:\n\n` +
      _capContent + `\n\n` +
      `تعليمات: اعرض هذا الرد بأسلوبك الطبيعي ولغة المستخدم. يمكنك إضافة تفاصيل مفيدة لكن لا تحذف أي رابط أو توجيه مذكور.`
    )
    return {
      intent:      { intent: 'CAPABILITY_QUERY', confidence: 99, action: 'guide-to-service' },
      entities:    {},
      taskHint:    'multilingual',
      systemBlock: parts.join('\n\n'),
      isSelfQuery: false,
