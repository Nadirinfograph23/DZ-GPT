...
import { enrichQueryContext, enrichQueryContextAsync } from '../dz-unified-pipeline.js'
import { analyzeMultiIntent, buildMultiIntentSystemLayer } from '../multi-intent-engine.js'
...
export async function runAutonomous({ task, aiGenerate }) {
  ...
  try {
    ...
    const result = {
      ...
      app:      ctx.app      ? { template: ctx.app.template, title: ctx.app.title, fileCount: Object.keys(ctx.app.files).length, totalBytes: ctx.app.totalBytes, previewHtml: ctx.app.previewHtml || null } : null,
...
async function synthesize({ aiGenerate, query, lang, ctx, bus }) {
  ...
  // ── Unified pipeline enrichment (async — حقن بيانات 365score + كووورة) ──
  const unified = await enrichQueryContextAsync(query)

  // ── Multi-Intent Detection — كشف الأسئلة/المهام المتعددة ────────────────
  const _v3MultiAnalysis = analyzeMultiIntent(query)
  const _v3MultiLayer    = buildMultiIntentSystemLayer(_v3MultiAnalysis)
  const _v3TokenBudget   = _v3MultiAnalysis.isMulti
    ? Math.min(800 + _v3MultiAnalysis.taskCount * 500, 3000)
    : 800
  ...
  const sysParts = [
    ...
    _v3MultiAnalysis.isMulti
      ? `⚡ رُصِد ${_v3MultiAnalysis.taskCount} أسئلة/مهام — يجب الإجابة عن كل واحدة بالترتيب دون استثناء.`
  ...
  // Hallucination guard: if no agent produced real findings, skip AI and return fallback
  if (facts.length === 0) {
    bus.emit('agent.result', { agent: 'synthesis', model: 'fallback', len: 0, reason: 'no-findings' })
    return fallbackSummary(query, ctx, lang)
  }
  ...
  try {
    const r = await aiGenerate({ messages, query, max_tokens: _v3TokenBudget })
