...
export function buildInlineHtml(files) {
  let html = files['public/index.html'] || files['index.html'] || ''
  if (!html) return null
  const css = files['public/styles.css'] || files['styles.css'] || ''
  const js  = files['public/app.js']    || files['app.js']    || ''
  if (css) {
    html = html.replace(/<link\s[^>]*href=["']\/styles\.css["'][^>]*\/?>/i, `<style>${css}</style>`)
  }
  if (js) {
    html = html.replace(/<script\s[^>]*src=["']\/app\.js["'][^>]*><\/script>/i, `<script>${js}</script>`)
  }
  return html
}
