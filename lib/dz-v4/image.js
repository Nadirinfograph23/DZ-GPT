...
export async function generateImage({ prompt, model, negativePrompt, aiGenerate } = {}) {
  // Auto-translate Arabic / French → English so FLUX.1-schnell understands the
  // scene. Falls back to the original prompt silently on failure.
  ...
  for (const [endpoint, buildBody] of candidates) {
    try {
      ...
      return {
        ...
        promptUsed: enhanced,
        originalPrompt: tr.original,
        translated: tr.translated,
        sourceLanguage: tr.language,
        model: modelLabel,
        bytes: buf.length,
  ...
  // ── Pollinations.ai free fallback (no token needed, always works) ────────
  try {
    const seed = Math.floor(Math.random() * 999999)
    const polUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(enhanced)}?model=flux&width=768&height=768&seed=${seed}&nologo=true&enhance=false`
    const ac2 = new AbortController()
    const t2 = setTimeout(() => ac2.abort(), 40_000)
    let polRes
    try {
      polRes = await fetch(polUrl, { signal: ac2.signal })
    ...
    if (polRes.ok) {
      ...
      if (ct.startsWith('image/')) {
        ...
        const id = newId()
        STORE.set(id, { mime: ct, bytes: buf, prompt: enhanced, model: 'pollinations/flux', createdAt: Date.now() })
        return {
          ok: true,
          id,
          url: `/api/dz-agent-v4/image/${id}`,
  ...
  // Last resort: deterministic SVG placeholder so the engine never returns empty.
