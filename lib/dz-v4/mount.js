...
import { generateImage, imageToImage, getImage, imageStats, POLLINATIONS_MODELS } from './image.js'
import { textToVideo, imageToVideo, getVideo, videoStats, checkVideoQuota } from './video.js'
...
export function mountDzAgentV4(app, { aiGenerate } = {}) {
  ...
  r.get('/image/models', (_req, res) => {
    res.json({ ok: true, models: POLLINATIONS_MODELS })
  })
  ...
  r.post('/img2img', async (req, res) => {
    ...
    try {
      const result = await imageToImage({ prompt, imageUrl, imageBase64: imgB64, strength, aiGenerate })
  ...
  r.post('/img2video', async (req, res) => {
    ...
    try {
      const result = await imageToVideo({ imageUrl, prompt, ip })
      res.json(result)
    } catch (e) { res.status(500).json({ ok: false, error: e.message }) }
  })
