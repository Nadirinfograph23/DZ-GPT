...
// نماذج Text-to-Video (2025-2026)
const T2V_MODELS = [
  'Wan-AI/Wan2.1-T2V-1.3B',   // وان 2.1 خفيف — فيديو حقيقي سريع
  'THUDM/CogVideoX-2b',        // CogVideoX 2B — جودة عالية
  'Lightricks/LTX-Video',      // LTX — أسرع نموذج مفتوح
  'genmo/mochi-1-preview',     // Mochi احتياطي
]

// نماذج Image-to-Video (2025-2026)
const I2V_MODELS = [
  'Lightricks/LTX-Video',                           // LTX — أفضل I2V مجاني
  'stabilityai/stable-video-diffusion-img2vid',     // SVD كلاسيكي
  'ali-vilab/i2vgen-xl',                            // i2vgen احتياطي
]

function nextT2VModel() {
  const m = T2V_MODELS[_t2vIdx % T2V_MODELS.length]; _t2vIdx++; return m
}
function nextI2VModel() {
  const m = I2V_MODELS[_i2vIdx % I2V_MODELS.length]; _i2vIdx++; return m
}
...
function buildT2VBody(model, prompt) {
  switch (model) {
    case 'Wan-AI/Wan2.1-T2V-1.3B':
      return { inputs: prompt, parameters: { num_frames: 16, num_inference_steps: 20 } }
    case 'THUDM/CogVideoX-2b':
      return { inputs: prompt, parameters: { num_frames: 16, num_inference_steps: 20, guidance_scale: 6 } }
    case 'Lightricks/LTX-Video':
      return { inputs: prompt, parameters: { num_frames: 25, num_inference_steps: 25, width: 512, height: 288 } }
    case 'genmo/mochi-1-preview':
      return { inputs: prompt, parameters: { num_frames: 16 } }
    default:
...
function buildI2VBody(model, imgB64, prompt) {
  switch (model) {
    case 'Lightricks/LTX-Video':
      return { inputs: imgB64, parameters: { prompt, num_frames: 25, num_inference_steps: 25 } }
    case 'stabilityai/stable-video-diffusion-img2vid':
      return { inputs: imgB64, parameters: { decode_chunk_size: 8, num_frames: 14 } }
    case 'ali-vilab/i2vgen-xl':
      return { inputs: imgB64, parameters: prompt ? { prompt } : {} }
    default:
      return { inputs: imgB64, parameters: prompt ? { prompt } : {} }
  }
}
...
async function tryHFTextToVideo(prompt, preferredModel = null) {
  ...
  const primary  = preferredModel || nextT2VModel()
  const rest     = T2V_MODELS.filter(m => m !== primary)
  const rotation = [primary, ...rest]

  for (const m of rotation) {
    const t = nextHFToken() || token
    console.log(`[Video T2V] جرّب ${m}...`)
    const result = await hfInference({ model: m, token: t, body: buildT2VBody(m, prompt) })
...
async function tryHFImageToVideo(imageUrl, prompt) {
  ...
  const primary  = nextI2VModel()
  const rest     = I2V_MODELS.filter(m => m !== primary)
  const rotation = [primary, ...rest]

  for (const m of rotation) {
    const t = nextHFToken() || token
    console.log(`[Video I2V] جرّب ${m}...`)
    const result = await hfInference({ model: m, token: t, body: buildI2VBody(m, imgB64, prompt) })
    if (result) return result
  }
  return null
}
...
export async function textToVideo({
  ...
  // 1. Pollinations — سريع، بلا token
  console.log('[Video T2V] جرّب Pollinations...')
  winner = await tryPollinationsVideo(prompt, width, height, duration)
  if (winner) console.log(`[Video T2V] ✅ Pollinations — ${winner.buf.length.toLocaleString()} bytes`)

  // 2. HuggingFace نماذج حقيقية بالتدوير
