// DZ Agent V4 PRO — Video Generation Engine v4
// نماذج مجانية 2025-2026 بالتدوير الآلي:
//
//   Text-to-Video:
//     1. Pollinations Video API    — مجاني تماماً، بلا token، أسرع
//     2. Wan-AI/Wan2.1-T2V-1.3B   — HuggingFace مجاني، سريع
//     3. Wan-AI/Wan2.1-T2V-14B    — HuggingFace جودة عالية
//     4. Lightricks/LTX-Video     — أسرع نموذج مفتوح
//     5. THUDM/CogVideoX1.5-5B    — جودة عالية
//     6. ByteDance/AnimateDiff     — GIF سريع
//     7. Skywork/SkyReels-V2       — جديد 2026
//     8. tencent/HunyuanVideo      — احترافي
//     9. genmo/mochi-1-preview     — إبداعي
//    10. damo-vilab/t2v-ms-1.7b    — خفيف
//
//   Image-to-Video:
//     1. Pollinations (إطارات)
//     2. Wan-AI/Wan2.1-I2V-14B
//     3. Lightricks/LTX-Video
//     4. THUDM/CogVideoX-5b-I2V
//     5. stabilityai/SVD-XT
//     6. ali-vilab/i2vgen-xl
//     7. ByteDance/AnimateDiff
//
//   حصص المستخدم:
//     - Pollinations: غير محدود
//     - HF Fast (Wan 1.3B, AnimateDiff, LTX): 15 فيديو/يوم
//     - HF Heavy (Wan 14B, CogVideoX, HunyuanVideo): 4 فيديو/يوم
...
// ── Pollinations Video API (مجاني، بلا token) ────────────────────────────────
async function tryPollinationsVideo(prompt, width = 480, height = 288) {
  const endpoints = [
    // الـ endpoint الرسمي الجديد
    { url: 'https://video.pollinations.ai/prompt/' + encodeURIComponent(prompt), method: 'GET', body: null },
    // البديل POST
    {
      url:    'https://video.pollinations.ai/',
      method: 'POST',
      body:   JSON.stringify({ prompt, model: 'wan', width, height, duration: 3 }),
    },
  ]
  ...
  for (const ep of endpoints) {
    ...
    try {
      ...
      if (r.ok && mime && buf.length > 1000) {
        console.log(`[Video T2V] ✅ Pollinations: ${mime} — ${buf.length.toLocaleString()} bytes`)
        return { buf, mime, model: 'pollinations/wan' }
      }
    } catch (err) {
      console.log(`[Video T2V] Pollinations ${ep.method} failed: ${err.message}`)
    }
...
export async function textToVideo({
  ...
  // ── 1. Pollinations Video API (مجاني، بلا حصة) ──────────────────────────
  winner = await tryPollinationsVideo(prompt, width, height)

  // ── 2. HuggingFace بالتدوير (مع حصة يومية) ──────────────────────────────
  if (!winner && getHFTokens().length > 0) {
    winner = await tryHFTextToVideo(prompt, preferredModel, ip)
  }

  // ── 3. Fallback: إطارات سينمائية من Pollinations ─────────────────────────
  if (!winner) {
    const frames = pollinationsFrames(prompt)
    return {
      ok: true, url: frames[0], frames, isFrames: true,
      prompt, model: 'DZ Cinematic AI', provider: 'Pollinations Frames',
      quota: checkVideoQuota(ip),
      note: '🎬 إطارات سينمائية — أضف HF_TOKEN للحصول على فيديو حقيقي.',
    }
  }
  ...
  return {
    ...
    provider: winner.model.startsWith('pollinations') ? 'Pollinations AI' : 'HuggingFace',
...
export async function imageToVideo({
  ...
  // ── 2. Pollinations فيديو من prompt غني ──────────────────────────────────
  if (!winner) {
    winner = await tryPollinationsVideo(`${prompt}, based on image animation`, 480, 288)
  }

  // ── 3. Fallback: إطارات متحركة ────────────────────────────────────────────
  if (!winner) {
    const frames = pollinationsFrames(prompt, true)
    return {
      ok: true, url: frames[0], frames, isFrames: true,
      prompt, model: 'DZ Animate AI', provider: 'Pollinations Frames',
      quota: checkVideoQuota(ip),
      note: '🎬 إطارات متحركة — أضف HF_TOKEN للفيديو الحقيقي.',
    }
  }
