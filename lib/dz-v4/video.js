...
const STORE            = new Map()
const TIMEOUT_FAST     = 60_000    // 60 ثانية للنماذج السريعة
const TIMEOUT_HEAVY    = 120_000   // 2 دقائق للنماذج الثقيلة
const WAIT_RETRY_MAX   = 2         // أقل محاولات = فشل أسرع = الانتقال للنموذج التالي
const WAIT_RETRY_DELAY = 8_000
...
export async function textToVideo({
  ...
  if (!winner) {
    const hasToken = getHFTokens().length > 0
    return {
      ok: false,
      error: hasToken
        ? 'نماذج الفيديو تحتاج وقت تحميل — حاول مجدداً بعد لحظة ⏳'
        : 'HF_TOKEN غير مُعيَّن — أضفه في الإعدادات لتفعيل توليد الفيديو.',
...
export async function imageToVideo({
  ...
  if (!winner) {
    ...
    return {
      ok: false,
      error: hasToken
        ? 'نماذج تحريك الصورة تحتاج وقت تحميل — حاول مجدداً بعد لحظة ⏳'
        : 'HF_TOKEN غير مُعيَّن — أضفه في الإعدادات لتفعيل توليد الفيديو.',
      quota: checkVideoQuota(ip),
    }
  }
