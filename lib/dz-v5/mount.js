  ...
  router.post('/chat', async (req, res) => {
    ...
    if (!stream) {
      ...
      if (useReAct) {
        ...
        // Standard AI for non-GitHub queries — with unified pipeline enrichment
        try {
          const unified = enrichQueryContext(query, messages)
    ...
    try {
      if (useReAct) {
        ...
        // SSE standard — with unified pipeline enrichment
        const unified = enrichQueryContext(query, messages)
        const enrichedMessages = unified.systemBlock
          ? [{ role: 'system', content: unified.systemBlock }, ...messages]
          : messages
