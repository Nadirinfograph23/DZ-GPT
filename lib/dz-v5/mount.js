...
import { isMapQuery, handleMapQuery } from '../../modules/dz-maps/index.js'
...
export function mountDzAgentV5(app, opts = {}) {
  ...
  router.post('/chat', async (req, res) => {
    ...
    // ── DZ Maps Intelligence — intercept map queries before AI/ReAct ──────────
    if (isMapQuery(query)) {
      try {
        const userLocation = req.body.userLocation || null
        const mapResult = await handleMapQuery(query, userLocation)
        if (mapResult) {
          return res.json({
        ...
        console.error('[dz-v5/maps] Error:', mapErr.message)
      }
    }
    ...
    if (!stream) {
      ...
      if (useReAct) {
        ...
        // Standard AI for non-GitHub queries — with unified pipeline enrichment (async: 365score + كووورة)
        try {
          const unified = await enrichQueryContextAsync(query, messages)
          const enrichedMessages = unified.systemBlock
            ? [{ role: 'system', content: unified.systemBlock }, ...messages]
            : messages
          const result = await safeGenerateAI({ messages: enrichedMessages, query, max_tokens: 4096 })
          return res.json({
            content: result?.content || '',
    ...
    try {
      if (useReAct) {
        ...
        // SSE standard — with unified pipeline enrichment (async: 365score + كووورة)
        const unified = await enrichQueryContextAsync(query, messages)
        const enrichedMessages = unified.systemBlock
          ? [{ role: 'system', content: unified.systemBlock }, ...messages]
          : messages
