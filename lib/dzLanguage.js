...
const INTENT_CONFIG = [
  {
    type: 'greeting',
    patterns: [
      /\b(salam|salem|salam\s+aleykoum|aslama|السلام عليكم|سلام|مرحبا|أهلا|صباح الخير|مساء الخير|bonjour|bonsoir|salut|hi|hello|hey|wach\s+rak|واش\s+راك|كيف\s+حالك)\b/i,
    ],
  },
  {
    type: 'thanks',
    patterns: [
      /\b(merci|شكرا|شكراً|بارك الله فيك|صحيت|sahit|sa7it|thanks|thank\s+you|شكراً\s+جزيلاً)\b/i,
    ],
  },
  {
    type: 'farewell',
    patterns: [
      /\b(bslama|بسلامة|وداعاً|مع\s+السلامة|au\s+revoir|bye|ciao|نراك|نشوفك)\b/i,
    ],
  },
  {
    type: 'search_places',
    patterns: [
      /\b(مطعم|مطاعم|ماكلة|أكل|restaurant|resto|cafe|كافيه|قهوة|فندق|فنادق|hôtel|hotel|ملعب|salle\s+de\s+sport|سوق|أسواق|marché|محل|محلات)\b/i,
      /\b(وين\s+نلقى|وين\s+كاين|كاين\s+شي|win\s+nlqa|kayn\s+shi)\b/i,
      /\b(أقرب|أحسن|أفضل|قريب|قريبة|meilleur|proche|le\s+plus\s+proche)\b.*\b(مطعم|فندق|محل|صيدلية|مستشفى)/i,
    ],
  },
  {
    type: 'search_pharmacy',
    patterns: [
      /\b(صيدلية|صيدليات|دواء|أدوية|pharmacie|médicament|dawa|دوا)\b/i,
      /\b(صيدلية|pharma)\b/i,
    ],
  },
  {
    ...
    patterns: [
      /\b(مستشفى|مستشفيات|عيادة|طبيب|أطباء|دكتور|hôpital|clinique|médecin|docteur|urgences|اسعاف|إسعاف)\b/i,
    ],
  },
  {
    type: 'generate_website',
    patterns: [
      /\b(ديرلي\s+موقع|dir\s*li\s+site|dirli\s+site|اصنع\s+لي\s+موقع|أنشئ\s+موقع|ابني\s+موقع|generate\s+website|create\s+website|موقع\s+ويب|landing\s+page|صفحة\s+ويب|portfolio|portfolyo)\b/i,
      /\b(موقع|site\s+web|website)\b.*\b(تصميم|design|احترافي|professionnel)\b/i,
    ],
  },
  {
    type: 'coding_help',
    patterns: [
      /\b(كود|code|برمجة|programming|خطأ|error|bug|بق|debug|إصلاح|fix|كيفاش\s+ن|how\s+to\s+code|html|css|javascript|js|python|react|nodejs|api|server|backend|frontend|دالة|function|component|class)\b/i,
      /\b(3lah|علاه)\s+.*(ma\s+ykh|ma\s+y3|ما\s+يعمل|ما\s+يخدم|لا\s+يعمل|لا\s+يخدم)\b/i,
    ],
  },
  {
    type: 'map_query',
    patterns: [
      /\b(خريطة|خرائط|map|carte|localisation|موقع\s+جغرافي|أين\s+تقع|وين\s+كاينة|leaflet|openstreetmap)\b/i,
    ],
  },
  {
    type: 'weather',
    patterns: [
      /\b(طقس|weather|météo|درجة\s+حرارة|temperature|حرارة|بارد|حار|مطر|رياح|vent|pluie|soleil|شمس)\b/i,
    ],
  },
  {
    type: 'news',
    patterns: [
      /\b(أخبار|خبر|news|actualités|جديد|آخر\s+أخبار|ماذا\s+حدث|واش\s+صرا|ch\s+sara|أحداث)\b/i,
    ],
  },
  {
    type: 'education',
    patterns: [
      /\b(درس|دروس|cours|تمرين|exercice|بكالوريا|bac|baccalauréat|امتحان|examen|شرح|explain|explique|اشرح|تعلم|learn|apprendre|eddirasa|مادة|sujet)\b/i,
    ],
  },
  {
    type: 'translation',
    patterns: [
      /\b(ترجم|translate|traduire|traduis|traduction|ma3na|معنى|معنى\s+كلمة|كلمة|كلمة\s+بالعربية|بالفرنسية|بالإنجليزية)\b/i,
    ],
  },
  {
    type: 'explanation',
    patterns: [
      /\b(واش\s+هو|واش\s+هي|واش\s+هذا|wach\s+hada|c'est\s+quoi|qu'est-ce\s+que|ما\s+هو|ما\s+هي|ما\s+معنى|شرح|explique|expliquer)\b/i,
    ],
  },
  {
    type: 'question',
    patterns: [
      /[؟?]|^\s*(واش|كيفاش|كيف|متى|أين|من|لماذا|ماذا|قداش|علاه|شكون|wach|wech|kifach|combien|comment|pourquoi|quand|où|qui|quoi|how|what|when|where|why|who)\b/i,
    ],
  },
  {
    type: 'request',
    patterns: [
      /\b(عاونّي|عاوني|ساعدني|نحتاج|بغيت|أبغي|أريد|ديرلي|aide|aidez|je\s+veux|je\s+voudrais|peux-tu|peut-on|stp|svp|please|can\s+you|help|3awni|dirli)\b/i,
    ],
  },
]
...
export function detectIntent(text) {
  ...
  for (const cfg of INTENT_CONFIG) {
    for (const pat of cfg.patterns) {
      const m = t.match(pat)
      if (m) {
        hits.push({ type: cfg.type, keyword: m[0] })
        break
      }
    }
  }
