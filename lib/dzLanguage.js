...
const PHRASE_MAP = [
  ...
  // ── Health / State / Emotional — أمراض + أحوال + مشاعر ──────────────────
  // CRITICAL: عيان = مريض/تعبان (وليس "playwright" أو معنى فصيح آخر)
  [/\brani\s+3yan\b/gi,       'أنا مريض'],
  [/\brani\s+3yana\b/gi,      'أنا مريضة'],
  [/\brani\s+t3ebt\b/gi,      'أنا تعبت'],
  [/\brani\s+mri[dD]\b/gi,    'أنا مريض'],
  [/\brani\s+ta3ban\b/gi,     'أنا تعبان'],
  [/\b3yan\b/gi,              'مريض'],
  [/\b3yana\b/gi,             'مريضة'],
  [/\bt3ebt\b/gi,             'تعبت'],
  [/\bta3ban\b/gi,            'تعبان'],
  [/\bwja3ni\b/gi,            'يؤلمني'],
  [/\bwja3\b/gi,              'ألم'],
  [/\bdayqni\b/gi,            'يضايقني'],
  [/\bdayaq\b/gi,             'ضايق'],
  [/\bh[iy]rani\b/gi,         'أربكني'],
  [/\brayhni\b/gi,            'يريحني'],
  [/\bdarli\b/gi,             'حدث لي'],
  [/\bdar\s+li\b/gi,          'حدث لي'],
  [/\bm?chitli\b/gi,          'ذهبت إليه'],
  [/\bm?chit\b/gi,            'ذهبت'],
  [/\bsbi?tar\b/gi,           'مستشفى'],
  [/\bsbi?tal\b/gi,           'مستشفى'],
  [/\bdwa\b/gi,               'دواء'],
  [/\bdoktar\b/gi,            'دكتور'],
  [/\bdoktor\b/gi,            'دكتور'],
  [/\btbib\b/gi,              'طبيب'],
  [/\binfirmier\b/gi,         'ممرض'],
  [/\binfirmiere\b/gi,        'ممرضة'],
  // ── Health — Arabic script (نص عربي) ────────────────────────────────────
  [/راني\s+عيان/g,            'أنا مريض'],
  [/راني\s+عيانة/g,           'أنا مريضة'],
  [/راني\s+تعبان/g,           'أنا تعبان'],
  [/راني\s+مريض/g,            'أنا مريض'],
  [/وجعني/g,                  'يؤلمني'],
  [/يوجعني/g,                 'يؤلمني'],
  [/ضايقني/g,                 'يضايقني'],
  [/حيراني/g,                 'أربكني'],
...
const TOKEN_MAP = {
  ...
  mzyan:'مليح', mzyana:'مليحة', mezyan:'مليح',
  // Logic
...
const DARIJA_HINTS_AR = [
  ...
  'شكون','قداش','بصاح','برشا','شوية','مليح','صحيت','هاك','هاكي','هاكم',
  'نروح','نجي','تعيا','مانيش','ماشي','ماعندوش','عندو','عندي','عندك',
  'كيما','تاعي','تاعك','تاعو','تاعهم','وحدو','وحدي','وحدك',
  'رزق','دير','ديري','اهدر','يهدر','نهدر','تهدر','هدرت',
  'فلوس','خدمة','مرا','راجل','درارية','ولاد','نتوما','هوما',
  // ── إضافات v2 — تحية + تعابير اجتماعية + دارجة شائعة ──────────────────
  'كيداير','كي داير','كيرانك','لاباس','صحا','غدوة','بالتوفيق','اليوم',
  'يعطيك','يعطيك الصحة','يسلمو','بلا مزية','ولا يهمك','ماعلاباليش',
  'ندير','نخدم','راهو','راها','رانا','راهم','نحب','حابس','نقدر',
  'مرسي','برافو','عيشك','زوين','صعيب','هاذ','هاذا','هاذي','ذاك',
  'سبيطار','دوا','عيان','تعبان','وجعني','ضايق','حيران',
  'تحيا','يحيا','عاش','ياسر','نتاع','بعدا','كامل',
]
...
export const darijaDictionary = Object.freeze({
  ...
  'قداش':'كم', 'برشا':'كثير', 'شوية':'قليل', 'مليح':'جيد',
...
const STYLE_TEMPLATES = {
  darija: {
    default:    'أجب بالدارجة الجزائرية الأصيلة. استخدم كلمات مثل: "كاين، ماكانش، راني، بزاف، واش، يلا، خويا، صحيت، مليح، بصح، علاه، شكون، قداش، كيفاش، دابا، توا، هاك، تاع، وحدو". كن طبيعياً وقريباً كأنك صاحب.',
    technical:  'أجب بالدارجة الجزائرية مع مصطلحات تقنية عند الضرورة. مثال: "شوف هذا الكود — كاين مشكلة في..." ثم واصل بالدارجة. استخدم: "خدم، ما يخدمش، صحيح، غلط".',
    search:     'أجب بالدارجة الجزائرية وأعطِ معلومات مباشرة وعملية. استخدم: "كاين...", "تلقى...", "روح لـ...", "هاك...", "وين نلقى...".',
    formal:     'أجب بالعربية الفصحى مع لمسة ودية.',
  },
  franco: {
    default:    'أجب بمزيج فرانكو-عربي جزائري طبيعي. مثال: "wach rak? ana rani mlih, bezzef..." أو دارجة مكتوبة بحروف لاتينية. كن قريباً وودياً.',
...
// كشف اللغات الآسيوية — ممنوعة في DZ Agent
const _ASIAN_LANG_RE = /[\u4E00-\u9FFF\u3040-\u309F\u30A0-\u30FF\uAC00-\uD7AF\u0900-\u097F\u0E00-\u0E7F\u0600-\u06FF]{0}[\u4E00-\u9FFF]{2,}|[\u3040-\u30FF]{2,}|[\uAC00-\uD7AF]{2,}|[\u0900-\u097F]{4,}|[\u0E00-\u0E7F]{4,}/

export function moderateMessage(text, userName = '') {
  if (!text || typeof text !== 'string') return { ok: true, severity: 'clean', reason: '', replyIfBlocked: '' }
  const probe = deobfuscate(text)
  const nameTag = userName ? `يا ${userName} ` : 'يا صاحبي '

  // ── كشف اللغات الآسيوية (صينية، يابانية، كورية، هندية، تايلاندية) ──────────
  if (_ASIAN_LANG_RE.test(text)) {
    return {
      ok: false,
      severity: 'unsupported_language',
      reason: 'asian_language',
      replyIfBlocked: `${nameTag}عذراً، DZ Agent يتكلم بالعربية، الدارجة الجزائرية، الفرنسية والإنجليزية فقط. 😊\nجرّب تكتب بالعربية أو الفرنسية!`,
    }
  }
  // ── Content Safety — طلبات خطيرة ──────────────────────────────────────
  const _DANGEROUS_RE = /(?:كيفاش\s+(?:نصنع|نعمل|نحضر|نبني|نركب)\s+(?:سلاح|قنبلة|متفجر|بارود|سم|مسدس|بندقية|رشاش|صاروخ|قنبلة|عبوة|ناسفة|مفرقع|مفخخة)|(?:how\s+to\s+(?:make|build|create)\s+(?:bomb|weapon|explosive|gun|poison|drug|meth))|(?:comment\s+(?:fabriquer|faire)\s+(?:bombe|arme|explosif|poison)))/i
  if (_DANGEROUS_RE.test(probe)) {
    return {
      ok: false,
      severity: 'dangerous',
      reason: 'dangerous_content',
      replyIfBlocked: `${nameTag}هذا الطلب ما نقدرش نعاونك فيه. 🚫\n\nDZ Agent مبني باش يساعد الناس بطريقة مفيدة وآمنة — خلينا نهدرو في شيء آخر! 💪`,
    }
  }

  // ── Cyber-Attack & Hacking Safety — BUG-1 FIX ─────────────────────────
  // يمنع طلبات الاختراق والقرصنة واستغلال الثغرات
  // NOTE: نستخدم نورمال regex بدون \b لأن العربية لا تعمل معه
  // نغطي: اكتب كود يخترق | كيف أهكر | hack code | اختراق موقع...
  const _HACKING_RE = new RegExp(
    '(?:(?:اكتب|اكتبلي|ولد|اعملي?|' +
    '\u0623\u0646\u0634\u0626|\u0623\u0639\u0637\u064A\u0646\u064A?|' + // أنشئ | أعطيني
    'كود|script|برنامج|سكريبت)\\s+.{0,60}' +
    '(?:يخترق|يقرصن|يهاجم|يسرق|يتجاوز|يفتح.*حساب|يكسر|يستغل.*ثغرة|hacking?|exploit))|' +
    '(?:(?:hacking?|hack|crack|exploit|brute.?force|sql.?inject|xss|csrf|phish|ddos|botnet|keylogger|trojan|ransomware|malware|spyware|rootkit)\\s+(?:code|script|tool|attack|website|server|system))|' +
    '(?:(?:اختراق|قرصنة|تهكير|brute.?force|sql.?inj|xss)\\s+(?:موقع|سيرفر|قاعدة.?بيانات|حساب|نظام|شبكة|حكومي|بنك))|' +
    // كيف + فعل الهكر + هدف — بدون \b لأن العربية
    '(?:كيف\\s+(?:' +
    '\u0623\u062E\u062A\u0631\u0642|' + // أخترق
    '\u062A\u062E\u062A\u0631\u0642|' + // تخترق
    '\u064A\u062E\u062A\u0631\u0642|' + // يخترق
    ...
    '\u064A\u0647\u0643\u0631|'  +      // يهكر
    '\u0627\u062E\u062A\u0631\u0642' +  // اخترق
    ')\\s+(?:موقع|حساب|سيرفر|نظام|شبكة|قاعدة|بريد|إيميل))',
    'i'
  )
  // deobfuscate يحوّل أهكر→اهكر، أخترق→اخترق لذا نفحص النص الأصلي أيضاً
  if (_HACKING_RE.test(probe) || _HACKING_RE.test(text.toLowerCase())) {
    return {
      ok: false,
      severity: 'dangerous',
      reason: 'hacking_request',
      replyIfBlocked: `${nameTag}ما نقدرش نكتبلك كود اختراق أو هجوم إلكتروني. 🚫\n\n**هذا النوع من الطلبات مرفوض** لأنه قد يُستخدم للإضرار بالآخرين.\n\nإذا كنت تريد تعلم **الأمن السيبراني الدفاعي** (ethical hacking, CTF)، يمكنني مساعدتك في الموارد التعليمية المناسبة. 🛡️`,
    }
  }
  ...
  for (const r of SEVERE_ROOTS) {
    if (probe.includes(deobfuscate(r))) {
      return {
        ...
        replyIfBlocked: `${nameTag}احشم شوية.. 🤌\nخلينا نهدرو باحترام باش نعاونو بعضانا ❤️`,
      }
    }
  }
  for (const r of PROFANITY_ROOTS) {
    ...
    if (re.test(' ' + probe + ' ')) {
      return {
        ...
        severity: 'offensive',
        reason: 'profanity',
        replyIfBlocked: `خويا ${nameTag}الكلام المليح خير 😊`,
      }
    }
  }
