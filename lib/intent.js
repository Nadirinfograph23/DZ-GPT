...
const CONVERSATION_ONLY_PATTERNS = [
  ...
  /تاريخ|ثقافة|دين|إسلام|قرآن|حديث|فقه|سيرة/i,
...
export function classifyIntent(rawQuery) {
  ...
  if (!isConversationOnly) {
    ...
    // Conversation-only: still allow news/search/medical/jobs sub-categories
    if (isDoctorSearch)     category = 'DOCTOR_SEARCH'
    else if (isMedical)     category = 'MEDICAL_SYMPTOM'
    else if (isJobSearch)   category = 'JOB_SEARCH'
    else if (isSearch)      category = 'SEARCH_REQUEST'
    else if (isNews)        category = 'NEWS_REQUEST'
...
export function detectIntent(rawQuery) {
  ...
  const { isGithubAction, isBuilder, isNews, isStructured, isUrgent, isSports, isExplicitCode, isPromptEng } = result.flags
  ...
  return {
    primary,
    lang,
    flags: { isBuilder, isGithub: isGithubAction, isNews, isStructured, isUrgent, isSports, isCode: isExplicitCode, isPromptEng },
 ...
 * taskHint values: 'multilingual' | 'technical' | 'retrieval' | 'reasoning' | 'translation' | 'general'
 */
export function getTaskRoutingHint(intent) {
  if (!intent) return 'general'
  const { primary, lang, flags } = intent
  // Arabic / French / mixed → multilingual provider preferred
  if (lang === 'ar' || lang === 'fr') return 'multilingual'
  // Code / builder / github → technical
  if (primary === 'builder' || primary === 'github' || flags?.isCode) return 'technical'
  // News / structured search → retrieval
  if (primary === 'news' || primary === 'structured') return 'retrieval'
  // Reasoning for complex general questions
  if (intent.liveMode) return 'reasoning'
  return 'general'
}
