...
export function classifyIntent(rawQuery) {
  ...
  const isDoctorSearch    = hasAny(query, DOCTOR_SEARCH_KW) ||
    // Natural complete doctor requests such as: "طبيب أسنان في عنابة".
    // These must enter the structured Doctor Search flow even without the
    // explicit phrase "ابحث عن طبيب".
    /(?:طبيب|دكتور|طبيبة|دكتورة)\\s+(?:أسنان|اسنان|قلب|عظام|أطفال|اطفال|عيون|جلدية|نفسي|نساء|توليد|أعصاب|اعصاب|مسالك|هضمي|رئة|أورام|اورام|كلى|غدد|أشعة|اشعة|جراح|عام|باطني)(?:\\s+(?:في|بـ|ب|بولاية|ولاية)\\s+|\\s+)(?:[\\u0600-\\u06FF]{2,}(?:\\s+[\\u0600-\\u06FF]{2,})?|[A-Za-zÀ-ÿ]{2,}(?:\\s+[A-Za-zÀ-ÿ]{2,})?)/i.test(rawQuery) ||
    /(?:dentiste|cardiologue|orthopédiste|orthopediste|pédiatre|pediatre|ophtalmologue|dermatologue|psychiatre|gynécologue|gynecologue|urologue|neurologue|gastro[- ]?entérologue|gastro[- ]?enterologue|pneumologue|néphrologue|nephrologue|endocrinologue|radiologue|chirurgien|médecin généraliste|medecin generaliste)\\s+(?:à|a|dans|en)\\s+[A-Za-zÀ-ÿ\\u0600-\\u06FF]{2,}(?:\\s+[A-Za-zÀ-ÿ\\u0600-\\u06FF]{2,})?/i.test(rawQuery)
