...
export function analyzeMultiIntent(message) {
  ...
  // ── تطبيع مبكر: استبدل " / " و " - " (كفواصل سؤال/مهمة) بـ "؟ " ──────────
  // آمن: URLs تحتوي "://" — لن تتأثر بنمط \s+\/\s+
  // مثال: "سؤال ؟ / سؤال آخر / سؤال ثالث" → يُكشَف كـ 3 أسئلة مستقلة
  let msg = message.trim().replace(/\s+\/\s+/g, '؟ ')

  // ── تطبيع "، و من/ما/كيف/..." → "؟ من/ما/كيف/..." ──────────────────────
  // يُعالج نمط: "من هو X، و من هو Y" أو "ما هو X و ما هو Y بالترتيب"
  // ملاحظة: \b لا يعمل مع الحروف العربية — نستخدم lookahead بدونه
  msg = msg.replace(
    /[،,]?\s+و\s+(?=(?:من|ما|كيف|أين|متى|لماذا|هل|كم|ماذا|what|who|how|when|where|why|which|qui|que|comment|quand|pourquoi|combien))/giu,
    ' ؟ '
  )
  // أيضاً: إزالة "بالترتيب" من نهاية الرسالة (لا يُشكّل سؤالاً مستقلاً)
  msg = msg.replace(/[،,]?\s*بالترتيب\s*$/u, '').trim()
