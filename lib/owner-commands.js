...
const DISAGREE_PATTERNS = [
  /^(?:لا[,،]?\s*|كلا[,،]?\s*)(?:هذا|هذه|ذلك|هاذ)?\s*(?:غلط|خطأ|مش صحيح|ماشي صحيح|مش زين|مش مليح|ماشي مزيان|مش مزيان|خاطئ|مغلوط)/i,
 ...
 * يحفظ التصحيح بعلامة _pending=true في data/agent_training.json.
 ...
 * @returns {{ correct: string, wrong: string, snippet: string } | null}
 */
export function detectAndStorePendingCorrection(currentMsg, lastAgentText = '') {
  if (!currentMsg || currentMsg.length < 5) return null

  const trimmed = currentMsg.trim()

  // هل الرسالة تبدأ بنمط رفض صريح؟
  const isDisagreement = DISAGREE_PATTERNS.some(p => p.test(trimmed))
  if (!isDisagreement) return null

  // استخراج الصواب من الرسالة الحالية
  let correct = ''
  ...
  // إذا لم نجد الصواب الصريح — استخدم الرسالة كاملة بعد حذف كلمات الرفض
  if (!correct) {
    correct = trimmed
      .replace(/^(?:لا|لأ|كلا)[,،]?\s*/i, '')
      .replace(/^(?:غلط|خطأ|ماشي صحيح|مش صحيح)[,،]?\s*/i, '')
      .trim()
  }

  if (!correct || correct.length < 4) return null

  // مقتطف من رد الوكيل الخاطئ (حتى 200 حرف)
  const snippet = (lastAgentText || '').slice(0, 200).trim()

  const timestamp = new Date().toISOString()
  const training  = loadTrainingData()

  // تجنب التكرار — لا نحفظ إذا كان الصواب موجوداً مسبقاً
  const isDup = training.corrections.some(c => c.correct === correct)
  if (isDup) return null

  // حذف أي تصحيح معلَّق سابق (نستبدله بالأحدث)
  training.corrections = training.corrections.filter(c => !c._pending)

  // أضف التصحيح الجديد بعلامة _pending
  ...
  training.commands_log.push({ cmd: 'pending_correction', correct, wrong: snippet, timestamp })
  saveTrainingData(training)

  console.log(`[PendingCorrection] 🕐 stored pending correction — confirm with "احفظ التصحيح"`)
  return { correct, wrong: snippet, snippet }
}
