...
export function detectOwnerCommand(msg) {
  ...
  if (CORRECTION_PATTERNS.some(p =>     p.test(msg)))   return 'train_correction'
  if (DEFINITION_PATTERNS.some(p =>     p.test(msg)))   return 'train_definition'
  if (OWNER_SOURCE_PATTERNS.some(p =>   p.test(msg)))   return 'train_source'
  return null
}
