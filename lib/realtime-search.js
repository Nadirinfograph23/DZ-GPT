/**
 * DZ Agent — Real-Time Search v4 (RSS-First Edition)
...
const REALTIME_TRIGGERS = [
  ...
  // أحداث راهنة — عبارات واضحة فقط (تُحذف: 'حالياً' المفردة لأنها صفة كيان أيضاً)
  'الآن', 'في الوقت الحالي', 'هذا الأسبوع', 'هذا الشهر',
  // حكومة — عبارات الأخبار فقط (حُذف: 'وزير','وزراء' لأنها تُضلّل أسئلة التعريف)
...
export function isRealtimeQuery(q = '') {
  ...
  // "من هو الرئيس الأمريكي الحالي؟" ليست بحثاً حياً — هي سؤال تعريف كيان
  // نفحص هذا أولاً قبل أي trigger آخر
...
export async function fetchRealtimeContext(query) {
  try {
    ...
    return buildSearchContext({
      ...
      extractedContent,
      query,
      label: isSports ? '⚽ أخبار رياضية — مصادر RSS جزائرية' : '📰 أخبار الجزائر — RSS مباشر',
    })
  } catch (err) {
