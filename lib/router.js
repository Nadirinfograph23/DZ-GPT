...
import { webKnowledgeSearch } from './wikipedia.js'
...
async function newsEngine(query, intent, opts) {
  ...
  const techContext   = !!intent.flags.isTech
  const news = await getTopNews({ query, limit: opts.limit || 12, sportsContext, techContext, fetcher: opts.fetcher })
...
async function structuredEngine(query, intent, opts) {
  ...
  const [news, repos] = await Promise.allSettled([
    getTopNews({ query, limit: 8, sportsContext: !!intent.flags.isSports, techContext: !!intent.flags.isTech, fetcher: opts.fetcher }),
...
// "General" intent: hybrid — top news + Wikipedia + DuckDuckGo, ranked.
async function hybridEngine(query, intent, opts) {
  const queries = expandQuery(query, intent.lang)

  const [newsRuns, wikiResult] = await Promise.allSettled([
    Promise.allSettled(
      queries.map(q => getTopNews({ query: q, limit: 6, fetcher: opts.fetcher }))
    ),
    webKnowledgeSearch(query),
  ])

  const allItems = []
  if (newsRuns.status === 'fulfilled') {
    for (const r of newsRuns.value) {
  ...
  // إذا لم تكن هناك أخبار، استخدم Wikipedia وحده
  if (!top.length && !wiki) return fail(FAILSAFE_MSG, { kind: 'general' })
  ...
  return ok({
    ...
    wikiKnowledge: wiki ? wiki.text : null,
    wikiSources: wiki ? wiki.sources : [],
  })
}
