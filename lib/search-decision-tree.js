...
/**
 * searchWithSearXNG — البحث عبر SearXNG كمحرك أساسي
...
export async function searchWithSearXNG(query, {
  ...
  const tryInstance = async (base) => {
    ...
    try {
      const res = await fetch(url, { headers: SEARXNG_HEADERS, signal: AbortSignal.timeout(timeoutMs) })
  ...
  for (let i = 0; i < shuffled.length; i += 3) {
    ...
    const result = await Promise.race([
      ...
      new Promise(r => setTimeout(() => r(null), timeoutMs + 500)),
    ])
    if (result) {
      console.log(`[SearXNG-DT] ✓ ${result.length} results via ${result[0]?.via || '?'}`)
      return result
    }
  }
  ...
  try {
    ...
    return links.map((l, i) => ({ source: 'DuckDuckGo', title: l.title, url: l.url, snippet: snippets[i] || '', date: '' }))
  } catch {
    return []
  }
}
...
export async function resolveQuery(query, { searchWikipediaFn = null } = {}) {
  ...
  switch (queryType) {
    ...
    // ── أخبار حالية: SearXNG → Crawl4AI ───────────────────────────────────
    case 'CURRENT_NEWS': {
      const { results, extractedContent, confidence } = await searchAndExtract(query, {
        categories: 'general,news',
        maxExtract: 2,
      })
      if (results.length > 0) {
        result.context = buildSearXNGContext(results, extractedContent, query)
        result.confidence = confidence
        result.sources = ['SearXNG', extractedContent ? 'Crawl4AI' : null].filter(Boolean)
      } else {
        result.noSource = true
        result.confidence = 0
      }
