...
import { searchPersonWikipedia, searchWikipedia, isPersonArticle, fetchWikipediaByTitle } from './wikipedia.js'
...
export async function searchFirstPipeline(query, {
  ...
  try {
    ...
    // FIX-C2/C5 fallback: جلب مباشر بالاسم النظيف (يعمل لتبون ومن شابهه)
    // يتجاوز searchPersonWikipedia لتفادي فلتر isPersonArticle الصارم
    if (!wikipediaResult?.extract) {
      wikipediaResult = await fetchWikipediaByTitle(cleanQ, 'ar').catch(() => null)
    }
    // إذا لم ينجح المباشر → searchPersonWikipedia كآخر ملاذ
    if (!wikipediaResult?.extract) {
