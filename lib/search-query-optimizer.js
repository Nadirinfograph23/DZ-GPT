...
export async function optimizedSearch(query, {
  ...
  // ── 3. بحث متوازي — 2 استعلامات فقط (بدلاً من 4) لتجنب الضغط على SearXNG ─
  const isTimeSensitive = [
    INTENT.FUTURE_EVENT, INTENT.PAST_EVENT,
    INTENT.BREAKING_NEWS, INTENT.SPORTS_TIME, INTENT.EXAM_SCHEDULE,
  ].includes(intent)

  // الاستعلام الأصلي + أفضل variation واحدة فقط في المرحلة الأولى
  const primary2   = variations.slice(0, 2)
  const secondary3 = variations.slice(2, 5)

  // Timeout عالمي: 8 ثوانٍ كحد أقصى للبحث كله
  const GLOBAL_TIMEOUT = isTimeSensitive ? 8000 : 10000
  const globalDeadline = Date.now() + GLOBAL_TIMEOUT

  const primarySettled = await Promise.allSettled(
    primary2.map(v => searchWithSearXNG(v, {
      categories,
  ...
  // ── 4. استعلامات احتياطية — فقط إذا بقي وقت كافٍ والنتائج قليلة ─────
  const timeLeft = globalDeadline - Date.now()
  if (allResults.length < 4 && secondary3.length > 0 && timeLeft > 2000) {
    console.log(`[QueryOptimizer v2] Low results (${allResults.length}), trying secondary (${timeLeft}ms left)...`)
    const secSettled = await Promise.allSettled(
      secondary3.slice(0, 2).map(v => searchWithSearXNG(v, {
        categories,
        language:  /[\u0600-\u06FF]/.test(v) ? 'ar' : 'en',
        maxResults: 6,
        timeoutMs:  Math.min(4000, timeLeft - 500),
      }))
    )
