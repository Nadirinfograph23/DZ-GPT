...
const LIVE_URL = 'https://dzagent.app'

import { getIntegrationSecret } from '../integration-secrets.js'

function vHeaders(token) {
  return { Authorization: `Bearer ${token || getIntegrationSecret('vercel')}`, 'Content-Type': 'application/json' }
}
...
export async function deployAndWait({ branch, repoId = '1191199822', maxWaitMs = 180000, onProgress, token }) {
  ...
  const vToken = token || getIntegrationSecret('vercel')
...
export async function getSyncStatus({ owner = 'Nadirinfograph23', repo = 'DZ-GPT', branch, projectId = DEFAULT_PROJECT, token }) {
  const vToken = token || getIntegrationSecret('vercel')
  const ghToken = getIntegrationSecret('github')
