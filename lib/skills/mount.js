...
export function mountGitHubSkill(app) {
  ...
  app.post('/api/github-skill/sync/vercel', async (req, res) => {
    ...
    try {
      ...
      res.json({ ok: true, deployment, liveUrl: 'https://dzagent.app' })
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message })
    }
  })
  ...
  app.post('/api/github-skill/sync/full', async (req, res) => {
    ...
    res.json({ ok: results.errors.length === 0, results, report, liveUrl: 'https://dzagent.app' })
  })
