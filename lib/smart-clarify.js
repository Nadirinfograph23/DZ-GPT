...
// ══════════════════════════════════════════════════════════════════════════════
// ENTITY DISAMBIGUATION — أسماء بارزة متعددة / أسماء أولى شائعة
// ══════════════════════════════════════════════════════════════════════════════

// أسماء مشهورة تحتاج توضيحاً قبل البحث في ويكيبيديا
const AMBIGUOUS_PERSONS = [
  {
    // رونالدو = كريستيانو أم R9؟
    triggers: [/^(?:(?:من\s+هو|من\s+هي|شكون\s+هو|qui\s+est|who\s+is)\s+)?رونالدو\s*[؟?]?$/i,
               /^(?:(?:from\s+is|who\s+is)\s+)?ronaldo\s*[؟?]?$/i],
    question: 'يوجد أكثر من لاعب مشهور باسم "رونالدو" — أيّهم تقصد؟',
    options: [
      { n: 1, emoji: '🇵🇹', label: 'كريستيانو رونالدو — البرتغال · النصر السعودي حالياً' },
      { n: 2, emoji: '🇧🇷', label: 'رونالدو R9 — البرازيل · أسطورة التسعينيات والألفية' },
    ],
    hint: 'اكتب **1** أو **2** أو أضف اسمه الكامل.',
  },
  {
    // زيدان — واضح نسبياً لكن نتيجتان ممكنتان
    triggers: [/^(?:(?:من\s+هو|من\s+هي|شكون\s+هو|qui\s+est|who\s+is)\s+)?(?:زين الدين\s+)?زيدان\s*[؟?]?$/i,
               /^(?:who\s+is\s+)?zidane\s*[؟?]?$/i],
    question: 'هل تقصد:',
    options: [
      { n: 1, emoji: '🇫🇷🇩🇿', label: 'زين الدين زيدان — اللاعب والمدرب الجزائري-الفرنسي' },
      { n: 2, emoji: '👤',     label: 'شخص آخر باسم زيدان (حدّد الاسم الكامل)' },
    ],
    hint: 'اكتب **1** للاطلاع على سيرة زيدان الكاملة.',
  },
  {
    // محمد علي — ملاكم أم شخص آخر؟
    triggers: [/^(?:(?:من\s+هو|من\s+هي|شكون\s+هو|qui\s+est|who\s+is)\s+)?محمد\s+علي\s*[؟?]?$/i,
               /^(?:who\s+is\s+)?muhammad\s+ali\s*[؟?]?$/i],
    question: 'هل تقصد:',
    options: [
      { n: 1, emoji: '🥊', label: 'محمد علي كلاي — أسطورة الملاكمة الأمريكية' },
      { n: 2, emoji: '👤', label: 'شخص آخر باسم محمد علي (حدّد المجال أو الجنسية)' },
    ],
    hint: 'اكتب **1** أو أضف تفاصيل أكثر.',
  },
]

// أسماء أولى شائعة جداً بدون لقب → نطلب التوضيح
const AMBIGUOUS_FIRST_NAMES_AR = new Set([
  'بوعلام','بلقاسم','عبدالله','رشيد','سعيد','خالد','كمال',
  'عمار','مصطفى','حسين','حسن','عبدالرحمن','لخضر','الهاشمي',
  'الطاهر','حميد','صالح','قادة','عبدالحميد','نورالدين',
])

/**
 * كشف ما إذا كان الاستعلام عن شخص غامض يحتاج توضيحاً.
 * Returns { needsClarification, question, options, hint } | null
 */
export function detectPersonAmbiguity(message) {
  const msg = String(message || '').trim()
  if (!msg || msg.length < 3) return null

  // ① أسماء بارزة متعددة (رونالدو، زيدان، محمد علي)
  for (const entry of AMBIGUOUS_PERSONS) {
    if (entry.triggers.some(p => p.test(msg))) {
      return {
        needsClarification: true,
  ...
  // ② "من هو X؟" حيث X اسم أول شائع جداً بلا لقب
  const nameMatch = msg.match(/^(?:من\s+هو|من\s+هي|شكون\s+هو|شكون\s+هي)\s+(.+?)[\s؟?]*$/i)
  if (nameMatch) {
    const name = nameMatch[1].trim()
    const words = name.split(/\s+/)
    if (words.length === 1 && AMBIGUOUS_FIRST_NAMES_AR.has(name)) {
      return {
        needsClarification: true,
        question: `"${name}" اسم شائع جداً — هل يمكنك تحديد الشخص المقصود؟`,
        options: [
          { n: 1, emoji: '📝', label: 'أضف اسم العائلة (مثال: بوعلام بنحمودة)' },
          { n: 2, emoji: '🎭', label: 'حدّد المجال: فنان / رياضي / سياسي / كاتب / عالم...' },
          { n: 3, emoji: '🌍', label: 'حدّد البلد أو الولاية أو الحقبة الزمنية' },
        ],
        hint: null,
      }
    }
...
// ── Source Attribution Detection — كشف أسئلة "من أين جاءت المعلومة؟" ──────────
const _SOURCE_ATTRIBUTION_PATTERNS = [
  /(?:من\s+أين|مصدر\s+هذ|أعطني\s+مصدر|ما\s+مصدر|أثبت\s+لي|دليل\s+على|المصدر\s+(?:هو|هي|ما))/i,
  /(?:هل\s+(?:هذا|هذه)\s+من\s+ويكيبيديا|من\s+أين\s+حصلت|كيف\s+عرفت|ما\s+المرجع)/i,
  /(?:source|where\s+did\s+you\s+get|prove\s+it|citation|reference|what\s+(?:is\s+your\s+source|source\s+is))/i,
  /(?:d'où\s+(?:tu|vous)\s+(?:tiens|tenez)|source\s+de\s+cette|quelle\s+source)/i,
]

export function isSourceAttributionQuery(message) {
  return _SOURCE_ATTRIBUTION_PATTERNS.some(p => p.test(message))
}
