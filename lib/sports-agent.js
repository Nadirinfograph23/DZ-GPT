...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  function buildMatchLine(m) {
    ...
    // ── نسب النتيجة لمصدرها الحقيقي (يمنع الخلط مع الهلوسة) ──────────────────
    const srcName = Array.isArray(m._sources) ? m._sources[0] : (m.source || '')
    const srcBadge = hasScore && srcName
      ? ` \`📡 ${srcName}\``
      : (m._srcBadge ? ` ${m._srcBadge}` : '')
  ...
  if (mergedMatches.length > 0) {
    ...
    if (finished.length > 0) {
      ...
      lines.push(`> ✅ **بيانات مباشرة من ${activeSources.join(' + ')}** — ليست مولّدة بالذكاء الاصطناعي`)
