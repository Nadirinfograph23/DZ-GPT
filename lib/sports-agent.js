...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  if (localFixtures.length > 0) {
    ...
    const TABLE_HDR = [`| | المضيف | النتيجة / الموعد | الضيف |`, `|:---:|:---|:---:|---:|`]

    if (p2Live.length > 0) {
      lines.push(`### 🔴 مباشر الآن (${p2Live.length})`)
      lines.push('')
      TABLE_HDR.forEach(h => lines.push(h))
    ...
    if (p2Pending.length > 0) {
      lines.push(`### ⏳ انتهت — النتيجة غير موثّقة بعد (${p2Pending.length})`)
      lines.push(`> ⚠️ النتيجة غير متاحة من المصادر الحية: ${SRC.fotmob} ${SRC.score365}`)
      lines.push('')
      TABLE_HDR.forEach(h => lines.push(h))
  ...
  // عرض أحدث النتائج الموثّقة إن وُجدت
  if (recentVerified.length > 0) {
    noMatchLines.push(`### ✅ آخر النتائج (${recentVerified[0].date})`)
    noMatchLines.push(``)
    noMatchLines.push(`| | المضيف | النتيجة | الضيف | المجموعة |`)
    noMatchLines.push(`|:---:|:---|:---:|---:|:---:|`)
    for (const m of recentVerified) {
      const f1 = getFlag(m.homeTeam)
      const f2 = getFlag(m.awayTeam)
      noMatchLines.push(`| ✅ | ${f1} **${m.homeTeam}** | \`${m.homeScore} – ${m.awayScore}\` | **${m.awayTeam}** ${f2} | ${m.group || ''} |`)
    }
    noMatchLines.push(``)
  }
  ...
  if (nextAlgMatch) {
    ...
    noMatchLines.push(``)
    noMatchLines.push(`| | المضيف | الموعد | الضيف | المجموعة |`)
    noMatchLines.push(`|:---:|:---|:---:|---:|:---:|`)
    ...
    const _aDate = nextAlgMatch.date || ''
    noMatchLines.push(`| 📅 | ${_af1} **${nextAlgMatch.homeTeam}** | ${_aTime} (${_aDate}) | **${nextAlgMatch.awayTeam}** ${_af2} | ${nextAlgMatch.group || ''} |`)
    noMatchLines.push(``)
  }
