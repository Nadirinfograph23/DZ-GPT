  ...
  if (classification.type === 'MATCH_SEARCH') {
    ...
    if (localResult.found) {
      ...
      if (_allUpcomingButPast || _wcAllUpcomingButPast) {
        ...
        if (_needsEnrichment) {
          console.log(`[SportsAgent:LOCAL] 🔍 Missing venue/scorers — enriching from external APIs...`)
          try {
            const _extMatches = await searchMatchAcrossDates(team1, team2, { temporal })
              .catch(() => [])
            for (const localM of localResult.matches) {
              // ابحث عن نفس المباراة في النتائج الخارجية (بالتاريخ + الفرق)
              ...
              if (_ext) {
                ...
                console.log(`[SportsAgent:ENRICH] ✅ Enriched: venue="${_ext.venue}" scorers=${_ext.scorers?.length||0}`)
              }
            }
          } catch (_ee) {
    ...
    if (!foundMatches.length) {
      ...
      if (wc1 && wc2 && wc1 === wc2) {
        ...
        // بناء رد "نفس المجموعة" + سياق مباريات الجزائر الكامل
    ...
    contextLines.push(`\n⚠️ **قاعدة صارمة:** اعرض البيانات الموثّقة أعلاه فقط. لا تخترع نتائج أو أهدافاً أو تواريخ.`)
