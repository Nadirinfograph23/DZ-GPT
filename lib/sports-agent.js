...
export async function runWC2026StandingsAgent(query = '') {
  ...
  if (liveSource) {
    ...
    console.log(`[WC2026:Standings] 📋 No live source — calculating from WC2026_FULL_FIXTURES — Group ${groupLetter}`)
  }

  // ── الاحتياطي الموثوق: احسب الترتيب من WC2026_FULL_FIXTURES ─────────────
  // يُستخدم دائماً إذا فشلت كل المصادر الحية (SofaScore + FotMob)
  if (!liveRows) {
    const { WC2026_FULL_FIXTURES } = await import('./dz-sports-knowledge.js')
    const grpFixtures = WC2026_FULL_FIXTURES.filter(
      f => f.group === groupLetter && f.statusType === 'finished' &&
    ...
    if (grpFixtures.length > 0) {
      ...
      liveSource = `ESPN (${grpFixtures.length} نتيجة موثقة)`
      console.log(`[WC2026:Standings] ✅ Calculated from fixtures: ${liveRows.length} teams, Group ${groupLetter}`)
    }
  }

  const _fixtureSrc = liveSource?.includes('ESPN') && liveSource?.includes('موثقة') ? liveSource : null
