...
import {
  ...
  KOORA_WC2026_URL, JDWEL_WC2026_URL,
...
export function buildMatchDetailedBlock(match = {}, details = null) {
  ...
  const _srcFavicon = {
    'FotMob':   `[![FotMob](https://www.google.com/s2/favicons?sz=16&domain=fotmob.com)](https://fotmob.com)`,
    '365score': `[![365score](https://www.google.com/s2/favicons?sz=16&domain=365scores.com)](https://365scores.com)`,
    'Kooora':   `[![Kooora](https://www.google.com/s2/favicons?sz=16&domain=kooora.com)](https://kooora.com)`,
    'SofaScore':`[![SofaScore](https://www.google.com/s2/favicons?sz=16&domain=sofascore.com)](https://sofascore.com)`,
    'jdwel':    `[![jdwel](https://www.google.com/s2/favicons?sz=16&domain=jdwel.com)](https://jdwel.com)`,
    'Kooora-Algeria': `[![Kooora](https://www.google.com/s2/favicons?sz=16&domain=kooora.com)](https://kooora.com)`,
  }
  const _srcBadge = _srcFavicon[match.source] || `**${match.source}**`
  lines.push(`\n📡 ${_srcBadge}${match.link ? ` — [رابط مباشر](${match.link})` : ''}`)
...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  // ── أيقونات المصادر (favicon badges) ──────────────────────────────────────
  const SRC = {
    fotmob:    `[![FotMob](https://www.google.com/s2/favicons?sz=16&domain=fotmob.com)](https://www.fotmob.com/leagues/77/matches/world-cup)`,
    score365:  `[![365score](https://www.google.com/s2/favicons?sz=16&domain=365scores.com)](https://www.365scores.com/ar/football/league/fifa-world-cup-5930)`,
    kooora:    `[![Kooora](https://www.google.com/s2/favicons?sz=16&domain=kooora.com)](${KOORA_WC2026_URL})`,
    jdwel:     `[![jdwel](https://www.google.com/s2/favicons?sz=16&domain=jdwel.com)](${JDWEL_WC2026_URL})`,
    fifa:      `[![FIFA](https://www.google.com/s2/favicons?sz=16&domain=fifa.com)](https://www.fifa.com/worldcup)`,
    local:     `🏆`,
  }
  ...
  function buildMatchLine(m) {
    ...
    const hasScore = m.homeScore !== null && m.homeScore !== undefined && m.awayScore !== null
    const statusBadge = m.statusType === 'finished' ? '✅' : m.statusType === 'live' ? '🔴 **مباشر**' : '📅'
    const scoreStr = hasScore ? `**${m.homeScore} – ${m.awayScore}**` : (m.startTime ? `⏰ ${m.startTime}` : 'vs')
    const link = m.link ? ` [🔗](${m.link})` : ''
    const grp = m.group ? ` *(${m.group})* ` : ''
    return `${statusBadge} ${f1} **${m.homeTeam}** ${scoreStr} **${m.awayTeam}** ${f2}${grp}${link}`
  }

  function buildSourceLine(src, extra = '') {
    const icons = {
      'FotMob':   SRC.fotmob,
      '365score': SRC.score365,
      'Kooora':   SRC.kooora,
      'jdwel':    SRC.jdwel,
      'DZ-Sports-Knowledge': SRC.local,
    }
    const badge = icons[src] || `**${src}**`
    return `> 📡 ${badge}${extra ? ` — ${extra}` : ''}`
  }

  // ── Phase 0: 365scores — مسابقة كأس العالم 2026 (ID: 5930) ────────────────
  try {
    const wc365 = await Promise.race([
      fetchWC2026Fixtures365(date),
      new Promise(r => setTimeout(() => r(null), 8000)),
    ])
    if (wc365?.matches?.length > 0) {
      const { matches, live = [], finished = [], upcoming = [] } = wc365
      const lines = [
        `## 🌐 مباريات كأس العالم FIFA 2026 — ${dateLabel}`,
        ``,
        buildSourceLine('365score', 'بيانات حية ✅'),
        ``,
      ]
      if (live.length > 0) {
      ...
      if (upcoming.length > 0) {
        lines.push(`### 📅 مباريات اليوم (${upcoming.length})`)
      ...
      console.log(`[WC2026TodayAgent] ✅ 365score: ${matches.length} WC matches on ${date}`)
      return { found: true, userResponse: lines.join('\n'), sources: ['365score'], matchCount: matches.length, date }
    }
  } catch (_e365) {
    console.warn('[WC2026TodayAgent] 365score error:', _e365.message)
  }

  // ── Phase 1: FotMob حي ──────────────────────────────────────────────────────
  try {
    const fotmobAll = await Promise.race([
      fetchFotmobForDate(date),
      new Promise(r => setTimeout(() => r([]), 8000)),
    ])
    const wcLive = fotmobAll.filter(m => isWCLeague(m.league, m.leagueId))

    if (wcLive.length > 0) {
      const live     = wcLive.filter(m => m.statusType === 'live')
      const upcoming = wcLive.filter(m => m.statusType === 'upcoming')
      const finished = wcLive.filter(m => m.statusType === 'finished')

      const lines = [
        `## 🌐 مباريات كأس العالم FIFA 2026 — ${dateLabel}`,
        ``,
        buildSourceLine('FotMob', 'بيانات حية ✅'),
        ``,
      ]
      if (live.length > 0) {
        lines.push(`### 🔴 مباريات جارية الآن (${live.length})`)
      ...
      lines.push(`---`)
      lines.push(`🔗 ${SRC.fotmob} ${SRC.score365} ${SRC.kooora} ${SRC.jdwel} ${SRC.fifa}`)
      console.log(`[WC2026TodayAgent] ✅ FotMob: ${wcLive.length} WC matches on ${date}`)
      return { found: true, userResponse: lines.join('\n'), sources: ['FotMob'], matchCount: wcLive.length, date }
    }
  } catch (_fe) {
    console.warn('[WC2026TodayAgent] FotMob error:', _fe.message)
  }

  // ── Phase 1.5: jdwel.com — مباريات كأس العالم اليوم ─────────────────────────
  try {
    const wcJdwel = await Promise.race([
      fetchJdwelWC2026(date),
      new Promise(r => setTimeout(() => r(null), 8000)),
    ])
    if (wcJdwel?.matches?.length > 0) {
      const { matches } = wcJdwel
      const live     = matches.filter(m => m.statusType === 'live')
      const upcoming = matches.filter(m => m.statusType === 'upcoming')
      const finished = matches.filter(m => m.statusType === 'finished')
      const lines = [
        `## 🌐 مباريات كأس العالم FIFA 2026 — ${dateLabel}`,
        ``,
        buildSourceLine('jdwel', 'بيانات حية ✅'),
        ``,
      ]
      if (live.length > 0) {
        lines.push(`### 🔴 مباريات جارية الآن (${live.length})`)
      ...
      lines.push(`---`)
      lines.push(`🔗 ${SRC.jdwel} ${SRC.score365} ${SRC.fotmob} ${SRC.kooora} ${SRC.fifa}`)
      console.log(`[WC2026TodayAgent] ✅ jdwel: ${matches.length} WC matches on ${date}`)
      return { found: true, userResponse: lines.join('\n'), sources: ['jdwel'], matchCount: matches.length, date }
    }
  } catch (_fjd) {
    console.warn('[WC2026TodayAgent] jdwel error:', _fjd.message)
  }
  ...
  if (localFixtures.length > 0) {
    ...
    const lines = [
      `## 🌐 مباريات كأس العالم FIFA 2026 — ${dateLabel}`,
      ``,
      buildSourceLine('DZ-Sports-Knowledge', 'جدول رسمي FIFA ✅'),
      ``,
    ]
    ...
    for (const rawFix of localFixtures) {
      ...
      if (fix.statusType === 'upcoming' && fix.date && fix.startTime) {
        try {
          ...
          const matchStartUtc = new Date(`${fix.date}T${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}:00+01:00`).getTime()
    ...
    if (hasPendingResults) {
      lines.push(`> ⚠️ **النتائج لم تُحدَّث محلياً** — تحقق من المصادر الحية أدناه للاطلاع على النتيجة الفعلية`)
      lines.push(``)
    }
    lines.push(`---`)
    lines.push(`🔗 ${SRC.score365} ${SRC.fotmob} ${SRC.kooora} ${SRC.jdwel} ${SRC.fifa}`)
    console.log(`[WC2026TodayAgent] 📋 Local fallback: ${localFixtures.length} fixtures on ${date} (pending=${hasPendingResults})`)
    return { found: true, userResponse: lines.join('\n'), sources: ['DZ-Sports-Knowledge'], matchCount: localFixtures.length, date }
  }
