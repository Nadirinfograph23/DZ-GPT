...
export async function runSportsAgent(query = '', history = []) {
  ...
  if (classification.type === 'MATCH_SEARCH') {
    ...
    if (!foundMatches.length) {
      // ══ فحص: هل كلا الفريقَين في كأس العالم 2026 لكن مجموعتان مختلفتان؟ ══
      const wc1 = findWC2026TeamGroup(team1)
      const wc2 = findWC2026TeamGroup(team2)

      if (wc1 && wc2 && wc1 !== wc2) {
        // كلاهما في كأس العالم 2026 لكن مجموعتان مختلفتان → لقاء محتمل في الأدوار الإقصائية
      ...
      if (wc1 && wc2 && wc1 === wc2) {
        // نفس المجموعة → ابحث في مباريات المجموعة
        console.log(`[SportsAgent] ⚽ WC2026 same group: ${team1}(${wc1}) vs ${team2}(${wc2})`)
        const wcAlgCtx = buildWorldCup2026AlgeriaContext()
        return {
          found: true,
          type: 'WORLD_CUP_2026',
          team1, team2, temporal,
          context: wcAlgCtx,
          userResponse: wcAlgCtx,
          sources: ['DZ-Sports-Knowledge'],
          fetchedAt,
        }
      }
