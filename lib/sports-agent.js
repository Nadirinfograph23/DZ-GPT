...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  if (localFixtures.length > 0) {
    ...
    if (hasPendingResults) {
      lines.push(`> ⚠️ **بعض المباريات انتهت لكن النتيجة لم تُوثَّق بعد من المصادر الحية**`)
      lines.push(`> 🔍 **للنتيجة الفعلية:** ${SRC.fotmob} ${SRC.score365} ${SRC.fifa}`)
      lines.push(``)
    }
    lines.push(`---`)
    lines.push(`${SRC.score365} ${SRC.fotmob} ${SRC.kooora} ${SRC.jdwel} ${SRC.fifa}`)
    // إرجاع المباريات المحدّثة (مع تصحيح النتائج الموثّقة)
