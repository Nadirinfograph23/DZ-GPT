...
// الكلمات التي لا تكون اسم لاعب — أفعال ووظائف
const NON_PLAYER_WORDS = new Set([
  'يلعب','يلعبون','يكون','يكونون','ينتمي','يشارك','يسجل','يمثل',
  'الآن','حالياً','هذا','هذه','الموسم','القادم','القادمة','اليوم',
  'في','مع','ل','من','إلى','على','عن','أي','كل','ما',
])

function extractPlayerName(query = '') {
  // "أين يلعب X" / "ما نادي X" / "فريق X" / "أين X الآن"
  const patterns = [
    // ① REVERSED: "X في أي نادي/فريق يلعب" — اسم اللاعب قبل trigger بالضبط
    // مثال: "رونالدو في أي نادي" → "رونالدو"
    // لا تمسك "أين يلعب ميسي" لأن "أين يلعب" ليس قبل "في أي نادي"
    /^([\u0600-\u06FF]{3,25}(?:\s+[\u0600-\u06FF]{3,15})?)\s+في\s+أي\s+(?:نادي|فريق)/i,
    // ② "أين يلعب X" / "ما نادي X" (اسم بعد trigger)
    /(?:أين\s+(?:يلعب|يلعب\s+حالياً?|هو\s+حالياً?)|ما\s+(?:ناد[ي]?|فريق))\s+([\u0600-\u06FF]{3,25}(?:\s+[\u0600-\u06FF]{3,20})?)/i,
    // ③ "انتقل X إلى"
    /(?:إلى\s+أين|أين)\s+انتقل\s+([\u0600-\u06FF\s]{3,35}?)(?:\s*[؟?]|$)/i,
    // ④ "فريق/نادي X" (اسم بعد trigger — لكن نتحقق أنه ليس فعلاً)
    /(?:فريق|نادي|ناد[ي]?)\s+([\u0600-\u06FF]{3,25}(?:\s+[\u0600-\u06FF]{3,20})?)(?:\s+(?:الآن|حالياً?|هذا\s+الموسم))?(?:\s*[؟?]|$)/i,
    // ⑤ بداية الجملة: اسمان عربيان متجاوران (للاستعلامات المختصرة)
    /^([\u0600-\u06FF]{3,}\s+[\u0600-\u06FF]{3,})/,
  ]
  for (const p of patterns) {
    const m = query.match(p)
    const candidate = m?.[1]?.trim()
    // تجاهل الكلمات الوظيفية كأفعال وأدوات
    if (candidate && !NON_PLAYER_WORDS.has(candidate) &&
        !NON_PLAYER_WORDS.has(candidate.split(/\s+/)[0])) {
      return candidate
    }
  }
  return ''
...
// ─── استخراج اسم الدوري من الاستعلام ────────────────────────────────────────
export function extractLeagueName(query = '') {
  if (/(?:إسباني|الليغا|لاليغا|la\s*liga|laliga|إسبانيا)/i.test(query)) return 'La Liga'
  if (/(?:إنجليزي|بريميرليغ|premier\s*league|الدوري\s+الإنجليزي)/i.test(query)) return 'Premier League'
  if (/(?:فرنسي|ليغ\s+1|ligue\s*1|الدوري\s+الفرنسي)/i.test(query)) return 'Ligue 1'
  if (/(?:ألماني|بوندسليغا|bundesliga|الدوري\s+الألماني)/i.test(query)) return 'Bundesliga'
  if (/(?:إيطالي|سيريا\s+آ|serie\s*a|الدوري\s+الإيطالي)/i.test(query)) return 'Serie A'
  if (/(?:أبطال\s+أوروبا|تشامبيونز|champions\s*league|UCL)/i.test(query)) return 'Champions League'
  if (/(?:جزائري|المحترف|الرابطة\s+الأولى|LFP)/i.test(query)) return 'Algerian Ligue Pro'
  return 'عام'
}
...
export function classifySportsQuery(query = '') {
  ...
  // ── ① هداف الدوري — يجب أن يكون قبل الكشف عن اللاعب ─────────────────────
  // "من هو هداف الدوري الإسباني" / "ترتيب الهدافين في الليغا" / "أفضل هداف"
  if (/(?:هداف|هدافو|هدافين|أفضل\s+هداف|ترتيب\s+(?:ال)?هداف|king.*goal|top.*scorer|pichichi|بيتشيتشي)/i.test(q) &&
      !/(?:كأس\s+العالم|world\s+cup|مونديال)/i.test(q)) {
    const league = extractLeagueName(q)
    return { type: 'LEAGUE_TOP_SCORER', query: q, league }
  }

  // ── ② أين يلعب اللاعب حالياً (أولوية عالية) ──────────────────────────────
  if (/(?:أين\s+يلعب|أين\s+هو\s+حالياً?|ما\s+ناد[ي]?|ما\s+فريق|في\s+أي\s+(?:فريق|نادي))\s+/i.test(q) ||
      /^[\u0600-\u06FF][\u0600-\u06FF\s]{2,20}\s+في\s+أي\s+(?:نادي|فريق)/i.test(q)) {
  ...
  // ── ⑧ رزنامة / جدول مباريات ──────────────────────────────────────────────
  if (/رزنامة|جدول.*مباريات|fixture|schedule|برنامج.*مباريات|متى.*مباراة|موعد.*مباراة|الجولة\s+(?:القادمة?|التالية?|الجديدة?)|مباريات\s+(?:الجولة|القادمة?)/i.test(q)) {
...
// ══════════════════════════════════════════════════════════════════════════════
// § 5b — جلب هدافي الدوري من 365scores API
// ══════════════════════════════════════════════════════════════════════════════

const LEAGUE_IDS_365 = {
  'La Liga':           7,
  'Premier League':    2,
  'Ligue 1':           16,
  'Bundesliga':        3,
  'Serie A':           11,
  'Champions League':  4,
  'Algerian Ligue Pro': 1297,
}

// علم الجنسية من اسم النادي
function flagFromClub(club = '') {
  const c = club.toLowerCase()
  if (/madrid|barcelona|sevilla|valencia|villarreal/i.test(c)) return '🇪🇸'
  if (/arsenal|chelsea|city|united|liverpool|tottenham/i.test(c)) return '🏴󠁧󠁢󠁥󠁮󠁧󠁿'
  if (/paris|lyon|marseille|monaco/i.test(c)) return '🇫🇷'
  if (/bayern|dortmund|leverkusen|frankfurt/i.test(c)) return '🇩🇪'
  if (/juventus|milan|inter|napoli|roma/i.test(c)) return '🇮🇹'
  if (/مولودية|شبيبة|اتحاد|وفاق|نصر حسين/i.test(c)) return '🇩🇿'
  return ''
}

export async function fetchLeagueTopScorers(league = 'La Liga') {
  const leagueId = LEAGUE_IDS_365[league]
  if (!leagueId) return null

  const season = new Date().getFullYear()
  const url = `https://webws.365scores.com/web/stats/?appTypeId=5&langId=1&timezoneName=Africa/Algiers&userCountryId=196&competitions=${leagueId}&statsId=1&season=${season}`
  ...
  try {
    ...
    const data = await res.json()
    const stats = data?.statsTable?.rows || data?.rows || []
    if (!stats.length) return null

    return stats.slice(0, 12).map(row => {
      const name    = row.athlete?.name || row.name || '—'
      const goals   = row.statValue ?? row.value ?? row.goals ?? 0
      const club    = row.team?.name || row.teamName || '—'
      return { name, goals, club, flag: flagFromClub(club) }
    }).filter(s => s.goals > 0)
  } catch (err) {
    console.warn('[fetchLeagueTopScorers] 365score failed:', err.message)
    return null
  }
}
...
export async function runSportsAgent(query = '', history = []) {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ── 🥅 LEAGUE_TOP_SCORER — هداف الدوري ───────────────────────────────────
  // ══════════════════════════════════════════════════════════════════════════
  if (classification.type === 'LEAGUE_TOP_SCORER') {
    const league = classification.league || 'عام'
    console.log(`[SportsAgent] 🥅 Top Scorer query: league="${league}"`)

    try {
      const scorerData = await fetchLeagueTopScorers(league)
      if (scorerData?.length > 0) {
        const leagueEmoji = {
          'La Liga': '🇪🇸', 'Premier League': '🏴󠁧󠁢󠁥󠁮󠁧󠁿', 'Ligue 1': '🇫🇷',
          'Bundesliga': '🇩🇪', 'Serie A': '🇮🇹', 'Champions League': '🌍',
          'Algerian Ligue Pro': '🇩🇿',
        }[league] || '⚽'

        const lines = [
          `## ${leagueEmoji} هدافو ${league} — الموسم الحالي`,
          ``,
          `> 📡 المصدر: **365scores.com** — ${new Date().toLocaleDateString('ar-DZ', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'Africa/Algiers' })}`,
          ``,
          `| # | اللاعب | النادي | الأهداف |`,
          `|---|--------|--------|---------|`,
          ...scorerData.slice(0, 10).map((s, i) =>
            `| **${i + 1}** | ${s.flag || ''} **${s.name}** | ${s.club || '—'} | **${s.goals}** ⚽ |`
          ),
          ``,
          `🔗 [التفاصيل على 365scores](https://www.365scores.com/ar/football) | [SofaScore](https://www.sofascore.com) | [FotMob](https://www.fotmob.com)`,
        ]
        const ctx = lines.join('\n')
        return { found: true, type: 'LEAGUE_TOP_SCORER', league, context: ctx, userResponse: ctx, sources: ['365scores'], fetchedAt }
      }
    } catch (_scorerErr) {
      console.warn('[SportsAgent] Scorer fetch failed:', _scorerErr.message)
    }

    // Fallback: روابط مرجعية + نص مساعد
    const SCORER_LINKS = {
      'La Liga':           'https://www.sofascore.com/tournament/football/spain/laliga/8',
      'Premier League':    'https://www.sofascore.com/tournament/football/england/premier-league/17',
      'Champions League':  'https://www.sofascore.com/tournament/football/europe/uefa-champions-league/7',
      'Algerian Ligue Pro':'https://www.lfp.dz/ar',
    }
    const fallbackCtx = [
      `## 🥅 هدافو ${league}`,
      ``,
      `> ⚠️ لم أتمكن من جلب الترتيب الحي الآن — المصادر الخارجية محدودة للوصول.`,
      ``,
      `**تحقق مباشرةً من:**`,
      `- 📊 [SofaScore](${SCORER_LINKS[league] || 'https://www.sofascore.com'}) — ترتيب الهدافين`,
      `- 📈 [FotMob](https://www.fotmob.com) — Top Scorers`,
      `- 🏆 [365scores](https://www.365scores.com/ar) — إحصاءات كاملة`,
      `- ⚽ [Kooora](https://www.kooora.com) — هدافو الموسم`,
    ].join('\n')
    return { found: false, type: 'LEAGUE_TOP_SCORER', league, context: fallbackCtx, userResponse: fallbackCtx, sources: [], fetchedAt }
  }
...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  // الوقت مُخزَّن بالفعل بتوقيت الجزائر (UTC+1) من المصادر (jdwel + FotMob)
  // لا نضيف أي ساعة إضافية — فقط تطبيع الصيغة العربية ص/م إن وُجدت
  ...
  function buildMatchLine(m) {
    ...
    const badge = m.statusType === 'finished' ? '✅'
      : m.statusType === 'live' ? '🔴'
      : isResultPending ? '⏳' : '📅'
    const grp = m.group ? ` *(${m.group})*` : ''
    // سطر واحد: بادج + علم + اسم + نتيجة/وقت + اسم + علم + مجموعة
    if (hasScore) {
      return `${badge} ${f1} **${m.homeTeam}** \`${m.homeScore} – ${m.awayScore}\` **${m.awayTeam}** ${f2}${grp}`
    }
    if (isResultPending) {
      return `${badge} ${f1} **${m.homeTeam}** ⏳ **${m.awayTeam}** ${f2}${grp}`
    }
    const timeStr = m.startTime ? `⏰ ${dzHourText(m.startTime)}` : 'vs'
    return `${badge} ${f1} **${m.homeTeam}** ${timeStr} **${m.awayTeam}** ${f2}${grp}`
  }
  ...
  const TEAM_KEY_ALIASES = {
    ...
    // ── الكونغو الديمقراطية (DRC) — أسماء مختلفة لنفس المنتخب ──
    'جمهورية الكونغو الديمقراطية': 'الكونغو الديمقراطية',
    'الكونغو د': 'الكونغو الديمقراطية',
    ...
    // ── كوت ديفوار ──
    "côte d'ivoire": 'ساحل العاج', "cote d'ivoire": 'ساحل العاج',
    // ── كوريا الشمالية ──
    'north korea': 'كوريا الشمالية', 'كوريا الشمالية': 'كوريا الشمالية',
  }
  ...
  if (mergedMatches.length > 0) {
    ...
    if (resultPending.length > 0) {
      lines.push(`### ⏳ انتهت — النتيجة غير موثّقة`)
      lines.push(`> ⚠️ راجع: ${SRC.fotmob} ${SRC.score365}`)
      lines.push('')
  ...
  if (localFixtures.length > 0) {
    ...
    for (const rawFix of localFixtures) {
      let fix = { ...rawFix }
      // إذا كانت المباراة "upcoming" لكن الوقت تجاوزها
      if (fix.statusType === 'upcoming' && fix.date && fix.startTime) {
        try {
          ...
          if (matchStartUtc + 2 * 60 * 60 * 1000 < _nowMs) {
            // ✅ البيانات الموثّقة (verified:true + finished) تُلغي result-pending فوراً
            const verifiedFix = WC2026_FULL_FIXTURES.find(vf =>
              vf.verified && vf.statusType === 'finished' &&
              ((vf.homeTeam === fix.homeTeam && vf.awayTeam === fix.awayTeam) ||
               (vf.homeTeam === fix.awayTeam && vf.awayTeam === fix.homeTeam))
            )
            if (verifiedFix) {
              const sameOrder = verifiedFix.homeTeam === fix.homeTeam
              fix = {
                ...fix,
                statusType: 'finished',
                homeScore: sameOrder ? verifiedFix.homeScore : verifiedFix.awayScore,
                awayScore: sameOrder ? verifiedFix.awayScore : verifiedFix.homeScore,
                winner: verifiedFix.winner,
                _verified: true,
              }
            } else {
    ...
    // ── تصنيف المباريات حسب الحالة (جدول مضغوط — نفس أسلوب Phase 1) ─────────
    const updatedMatches = Object.values(byGroup).flat()
    const p2Live     = updatedMatches.filter(m => m.statusType === 'live')
    const p2Finished = updatedMatches.filter(m => m.statusType === 'finished')
    const p2Pending  = updatedMatches.filter(m => m.statusType === 'result-pending')
    const p2Upcoming = updatedMatches.filter(m => m.statusType === 'upcoming')

    if (p2Live.length > 0) {
      lines.push(`### 🔴 مباشر الآن`)
      lines.push('')
      for (const m of p2Live) lines.push(buildMatchLine(m))
      lines.push('')
    }
    if (p2Finished.length > 0) {
      lines.push(`### ✅ نتائج — ${dateLabel}`)
      if (p2Finished.some(m => m._verified)) lines.push(`> ✅ **بيانات موثّقة (365scores)**`)
      lines.push('')
      for (const m of p2Finished) lines.push(buildMatchLine(m))
      lines.push('')
    }
    if (p2Upcoming.length > 0) {
      lines.push(`### 📅 مواعيد — ${dateLabel}`)
      lines.push('')
      for (const m of p2Upcoming) lines.push(buildMatchLine(m))
      lines.push('')
    }
    if (p2Pending.length > 0) {
      lines.push(`### ⏳ انتهت — النتيجة غير موثّقة`)
      lines.push(`> ⚠️ راجع: ${SRC.fotmob} ${SRC.score365}`)
      lines.push('')
      for (const m of p2Pending) lines.push(buildMatchLine(m))
      lines.push('')
    }
    ...
    return { found: true, userResponse: lines.join('\n'), sources: ['DZ-Sports-Knowledge'], matchCount: updatedMatches.length, matches: updatedMatches, date }
  }

  // ── Phase 3: لا توجد مباريات هذا اليوم — نعرض نتائج أمس الموثّقة ─────────────
  // إذا لم تجد مباريات اليوم، ابحث عن نتائج الأيام الأخيرة من البيانات الموثّقة
  const _yesterdayStr = new Date(Date.now() - 86400000).toISOString().slice(0, 10)
  const recentVerified = WC2026_FULL_FIXTURES
    .filter(f => f.verified && f.statusType === 'finished' && f.date >= _yesterdayStr)
  ...
  const noMatchLines = [
    ...
    ``,
    `> ℹ️ لا مباريات كأس العالم اليوم.`,
    ``,
  ]

  if (recentVerified.length > 0) {
    noMatchLines.push(`### ✅ آخر النتائج`)
    noMatchLines.push(``)
    for (const m of recentVerified) {
      const f1 = getFlag(m.homeTeam)
      const f2 = getFlag(m.awayTeam)
      const grp = m.group ? ` *(${m.group})*` : ''
      noMatchLines.push(`✅ ${f1} **${m.homeTeam}** \`${m.homeScore} – ${m.awayScore}\` **${m.awayTeam}** ${f2}${grp}`)
    }
    noMatchLines.push(``)
  }
  ...
  if (nextAlgMatch) {
    ...
    noMatchLines.push(``)
    const _af1 = getFlag(nextAlgMatch.homeTeam)
    const _af2 = getFlag(nextAlgMatch.awayTeam)
    const _aTime = nextAlgMatch.startTime ? `⏰ ${nextAlgMatch.startTime}` : '📅'
    const grp = nextAlgMatch.group ? ` *(${nextAlgMatch.group})*` : ''
    noMatchLines.push(`📅 ${_af1} **${nextAlgMatch.homeTeam}** ${_aTime} **${nextAlgMatch.awayTeam}** ${_af2}${grp}`)
    noMatchLines.push(``)
  }

  noMatchLines.push(`> ${SRC.score365} ${SRC.fotmob} ${SRC.kooora} ${SRC.jdwel} ${SRC.fifa}`)
  return {
    found: recentVerified.length > 0,
    userResponse: noMatchLines.join('\n'),
    sources: recentVerified.length > 0 ? ['DZ-Sports-Knowledge'] : [],
    matches: recentVerified,
    date,
  }
}
...
export async function runWC2026StandingsAgent(query = '') {
  const {
    extractWC2026GroupFromQuery,
    buildWC2026StandingsTable,
    isWC2026AllGroupsQuery,
    buildWC2026AllGroupsStandings,
    WC2026_FULL_FIXTURES,
  } = await import('./dz-sports-knowledge.js')

  // ══ استعلام "كل المجموعات" — أو استعلام عام بدون مجموعة محددة ════════════
  const specificGroup = extractWC2026GroupFromQuery(query)
  if (isWC2026AllGroupsQuery(query) || !specificGroup) {
    console.log(`[WC2026:Standings] 📊 All-groups query — trying ESPN API first`)

    // ── محاولة ESPN API للحصول على ترتيب حي لكل المجموعات ─────────────────
    try {
      ...
      if (espnRes.ok) {
        ...
        const children = espnJson?.children || espnJson?.standings || []
        if (children.length >= 4) {
          console.log(`[WC2026:Standings] ✅ ESPN API live — ${children.length} groups`)
          // ── بناء جدول الترتيب من بيانات ESPN ──────────────────────────────
          const now = new Date().toLocaleDateString('ar-DZ', {
            timeZone: 'Africa/Algiers', day: 'numeric', month: 'long', year: 'numeric',
          })
          const WC2026_FLAGS_LOCAL = {
            'Mexico':'🇲🇽','South Africa':'🇿🇦','South Korea':'🇰🇷','Czech Republic':'🇨🇿',
            'Canada':'🇨🇦','Bosnia and Herzegovina':'🇧🇦','Qatar':'🇶🇦','Switzerland':'🇨🇭',
            'Brazil':'🇧🇷','Morocco':'🇲🇦','Haiti':'🇭🇹','Scotland':'🏴󠁧󠁢󠁳󠁣󠁴󠁿',
          ...
          const lines = [
            ...
            ``,
            `> 📅 **${now}** · 48 منتخب · 12 مجموعة · 📡 بيانات حية من ESPN`,
            ``,
          ]
          for (const grp of children) {
            const grpName = grp.name || grp.abbreviation || ''
            const letter = (grp.abbreviation || grpName.replace(/Group\s*/i,'')).trim().toUpperCase()
            const entries = grp.standings?.entries || grp.entries || []
            const isAlgeriaGroup = entries.some(e => /algeria|الجزائر/i.test(e.team?.displayName || ''))
            const playedCount = entries.reduce((acc, e) => {
              const stats = e.stats || []
              const gp = stats.find(s => s.abbreviation === 'GP' || s.name === 'gamesPlayed')
              return Math.max(acc, gp?.value || 0)
            }, 0)
            const playNote = playedCount > 0
              ? `⚽ **${Math.round(playedCount)}** مباراة لُعبت · ESPN ✅`
              : `⏳ لم تبدأ بعد`
            lines.push(`---`)
            lines.push(``)
            lines.push(`### المجموعة **${letter}**${isAlgeriaGroup ? ' 🇩🇿' : ''} — ${playNote}`)
            lines.push(``)
            lines.push(`| # | المنتخب | لع | ف | ت | خ | لـه | عل | +/- | نق | حالة |`)
            lines.push(`|:-:|:-------|:--:|:-:|:-:|:-:|:---:|:--:|:---:|:--:|:----:|`)
            entries.forEach((e, i) => {
              const stats = e.stats || []
              const getStat = (ab) => stats.find(s => s.abbreviation === ab || s.name === ab)?.value ?? 0
              const name  = e.team?.displayName || e.team?.name || '—'
              const flag  = WC2026_FLAGS_LOCAL[name] || '🏴'
              const played = Math.round(getStat('GP') || getStat('gamesPlayed'))
              const w  = Math.round(getStat('W')  || getStat('wins'))
              const d  = Math.round(getStat('D')  || getStat('T') || getStat('ties'))
              const l  = Math.round(getStat('L')  || getStat('losses'))
              const gf = Math.round(getStat('F')  || getStat('GF') || getStat('pointsFor'))
              const ga = Math.round(getStat('A')  || getStat('GA') || getStat('pointsAgainst'))
              const pts = Math.round(getStat('P') || getStat('PTS') || getStat('points'))
              const gdRaw = getStat('GD') || getStat('pointDifferential')
              const gd = gdRaw !== undefined && gdRaw !== 0 ? Math.round(gdRaw) : (gf - ga)
              const gdStr = gd > 0 ? `+${gd}` : `${gd}`
              // WC2026: top 2 qualify directly, 3rd may qualify as best 3rd (8/12), 4th eliminated
              const posStr = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `4`
              const qualStatus = i === 0 ? '🟢 تأهل'
                : i === 1 ? '🟢 تأهل'
                : i === 2 ? '🟡 محتمل'
                : '🔴 خروج'
              const isAlg = /algeria|الجزائر/i.test(name)
              const b = isAlg ? '**' : ''
              const ptsStr = isAlg ? `🟢 **${pts}**` : i < 2 && played > 0 ? `**${pts}**` : `${pts}`
              lines.push(`| ${posStr} | ${flag} ${b}${name}${b} | ${played} | ${w} | ${d} | ${l} | ${gf} | ${ga} | ${gdStr} | ${ptsStr} | ${qualStatus} |`)
            })
            lines.push(``)
          }
          lines.push(`---`)
          lines.push(``)
          lines.push(`> 🟢 **تأهل** — المركزان الأول والثاني من كل مجموعة (24 منتخب) · 🟡 **محتمل** — أفضل 8 مجموعات ثالثة · 🔴 **خروج** — الرابع`)
          lines.push(`>`)
          lines.push(`> **لع**=لعب · **ف**=فوز · **ت**=تعادل · **خ**=خسارة · **لـه**=أهداف له · **عل**=عليه · **+/-**=الفارق · **نق**=نقاط`)
          lines.push(``)
          lines.push(`🔗 **نتائج حية:** [FIFA](https://www.fifa.com/fifaplus/ar/tournaments/mens/worldcup/canadamexicousa2026) · [ESPN](https://www.espn.com/soccer/standings/_/league/FIFA.WORLD) · [Kooora](https://www.kooora.com/كرة-القدم/مسابقة/كأس-العالم/جدول/70excpe1synn9kadnbppahdn7) · [365score](https://www.365scores.com/ar/football/world-cup-2026)`)

          return {
            groupLetter: 'ALL',
            liveSource: 'ESPN API (حي ✅)',
            userResponse: lines.join('\n'),
            found: true,
          }
        }
      ...
      console.log(`[WC2026:Standings] ESPN API failed — using local data: ${espnErr.message?.slice(0,60)}`)
    }

    // ── Fallback: البيانات المحلية المحسوبة ────────────────────────────────
    console.log(`[WC2026:Standings] 📋 Fallback to local WC2026_FULL_FIXTURES`)
    const allTable = buildWC2026AllGroupsStandings()
    return {
      groupLetter: 'ALL',
      liveSource: 'DZ-Sports-Knowledge (calculated from WC2026_FULL_FIXTURES)',
      userResponse: allTable,
      found: true,
    }
  }

  const groupLetter = specificGroup

  // ── احسب الترتيب من WC2026_FULL_FIXTURES (بيانات موثقة من ESPN/BBC/FIFA) ──
  let liveRows = null
  let liveSource = null

  const grpFixtures = WC2026_FULL_FIXTURES.filter(
    f => f.group === groupLetter &&
         f.statusType === 'finished' &&
         f.homeScore !== null && f.homeScore !== undefined
  )

  if (grpFixtures.length > 0) {
    const statsMap = {}
    for (const f of grpFixtures) {
      if (!statsMap[f.homeTeam]) statsMap[f.homeTeam] = { team: f.homeTeam, played: 0, win: 0, draw: 0, loss: 0, gf: 0, ga: 0, pts: 0 }
      if (!statsMap[f.awayTeam]) statsMap[f.awayTeam] = { team: f.awayTeam, played: 0, win: 0, draw: 0, loss: 0, gf: 0, ga: 0, pts: 0 }
      const hs = f.homeScore || 0
      const as_ = f.awayScore || 0
      statsMap[f.homeTeam].played++
      statsMap[f.homeTeam].gf += hs
      statsMap[f.homeTeam].ga += as_
      if (hs > as_)       { statsMap[f.homeTeam].win++;  statsMap[f.homeTeam].pts += 3 }
      else if (hs === as_) { statsMap[f.homeTeam].draw++; statsMap[f.homeTeam].pts += 1 }
      else                { statsMap[f.homeTeam].loss++ }
      statsMap[f.awayTeam].played++
      statsMap[f.awayTeam].gf += as_
      statsMap[f.awayTeam].ga += hs
      if (as_ > hs)       { statsMap[f.awayTeam].win++;  statsMap[f.awayTeam].pts += 3 }
      else if (as_ === hs) { statsMap[f.awayTeam].draw++; statsMap[f.awayTeam].pts += 1 }
      else                { statsMap[f.awayTeam].loss++ }
    }
    const srcs = [...new Set(grpFixtures.map(f => f.source).filter(Boolean))]
    liveRows = Object.values(statsMap)
    liveSource = `${srcs.join('/') || 'DZ-Knowledge'} (${grpFixtures.length} نتيجة موثقة)`
    console.log(`[WC2026:Standings] ✅ Calculated from ${grpFixtures.length} fixtures — Group ${groupLetter} — src: ${liveSource}`)
  } else {
    console.log(`[WC2026:Standings] 📋 No finished fixtures yet — Group ${groupLetter} (pre-match)`)
  }

  // ── محاولة تحديث من SofaScore (اختياري — لا يُحظر إذا فشل) ────────────────
  if (!liveRows) {
    try {
      const espnRes = await Promise.race([
        fetch(`https://site.api.espn.com/apis/v2/sports/soccer/fifa.world/standings`, {
      ...
      if (espnRes.ok) {
        const espnJson = await espnRes.json()
        const groups = espnJson?.children || espnJson?.standings || []
        const grpData = groups.find(g => {
          const n = (g.name || g.abbreviation || '').toUpperCase()
          return n.includes(groupLetter) || n === `GROUP ${groupLetter}`
        })
        const espnRows = grpData?.standings?.entries || grpData?.entries || []
        if (espnRows.length >= 4) {
          liveRows = espnRows.map(e => {
            const stats = e.stats || []
            const getStat = (abbrev) => stats.find(s => s.abbreviation === abbrev || s.name === abbrev)?.value || 0
            return {
              team: e.team?.displayName || e.team?.name || '',
              played: getStat('GP') || getStat('gamesPlayed') || 0,
              win: getStat('W') || getStat('wins') || 0,
              draw: getStat('T') || getStat('ties') || 0,
              loss: getStat('L') || getStat('losses') || 0,
              gf: getStat('GF') || getStat('pointsFor') || 0,
              ga: getStat('GA') || getStat('pointsAgainst') || 0,
              pts: getStat('PTS') || getStat('points') || 0,
            }
          })
          liveSource = `ESPN API (المجموعة ${groupLetter})`
          console.log(`[WC2026:Standings] ✅ ESPN API live — Group ${groupLetter}`)
        }
      }
    } catch (_espnErr) {
      console.log(`[WC2026:Standings] ESPN API unavailable — using local data`)
    }
  }

  const _fixtureSrc = liveSource?.includes('نتيجة موثقة') ? liveSource : null
  const table = buildWC2026StandingsTable(groupLetter, liveRows, _fixtureSrc)
