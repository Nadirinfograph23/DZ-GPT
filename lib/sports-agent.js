...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  function buildMatchLine(m) {
    ...
    return `${statusBadge} ${f1} **${m.homeTeam}** ${scoreStr} **${m.awayTeam}** ${f2}${link}`
  }
  ...
  try {
    ...
    if (wcLive.length > 0) {
      ...
      if (upcoming.length > 0) {
        lines.push(`### 📅 مباريات قادمة اليوم (${upcoming.length})`)
      ...
      lines.push(`---`)
      lines.push(`🔗 [كل مباريات كأس العالم على FotMob](https://www.fotmob.com/leagues/77/matches/world-cup) | [365score](https://www.365scores.com/ar/football/world-cup-2026) | [FIFA.com](https://www.fifa.com/worldcup)`)
  ...
  // ── Phase 2: القاعدة المحلية (Algeria Group J) ──────────────────────────────
  const localFixtures = buildWC2026TodayFixtures(date)
  if (localFixtures.length > 0) {
    const lines = [
      `## 🌐 مباريات كأس العالم FIFA 2026 — ${dateLabel}`,
      ``,
      `> 📡 *المصدر: قاعدة بيانات DZ-Sports-Knowledge ✅*`,
      ``,
    ]
    ...
    lines.push(`---`)
    lines.push(`🔗 [نتائج حية على 365score](https://www.365scores.com/ar/football/world-cup-2026) | [FotMob WC](https://www.fotmob.com/leagues/77/matches/world-cup)`)
    console.log(`[WC2026TodayAgent] 📋 Local fallback: ${localFixtures.length} fixtures on ${date}`)
    return {
      found: true,
      userResponse: lines.join('\n'),
      sources: ['DZ-Sports-Knowledge'],
      matchCount: localFixtures.length,
      date,
    }
  }

  // ── Phase 3: لا توجد مباريات هذا اليوم ─────────────────────────────────────
  const algFixtures = WORLD_CUP_2026.algeriaGroup.fixtures
    .filter(f => f.statusType === 'upcoming')
  ...
  const noMatchLines = [
    ...
    ``,
    `> ℹ️ لا توجد مباريات كأس العالم مجدولة في قاعدة بياناتنا لهذا اليوم.`,
    ``,
  ]
  if (nextAlgMatch) {
  ...
  noMatchLines.push(`🔗 [تحقق من الجدول الكامل على 365score](https://www.365scores.com/ar/football/world-cup-2026) | [FotMob](https://www.fotmob.com/leagues/77/matches/world-cup) | [FIFA.com](https://www.fifa.com/worldcup)`)
