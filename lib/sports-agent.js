...
export async function runWC2026TodayAgent(dateStr = '') {
  ...
  const SRC = {
    fotmob:   `[![FotMob](https://www.google.com/s2/favicons?sz=16&domain=fotmob.com)](${FOTMOB_WC2026_URL})`,
    ...
    bein:     `[![beIN Sports](https://www.google.com/s2/favicons?sz=16&domain=beinsports.com)](${BEIN_WC2026_URL})`,
  ...
  function buildMatchLine(m) {
    ...
    const srcBadge = m._srcBadge ? ` ${m._srcBadge}` : ''
    return `${statusBadge} ${f1} **${m.homeTeam}** ${scoreStr} **${m.awayTeam}** ${f2}${grp}${srcBadge}${link}`
  }
  ...
  const mergedMatches = [...dedupMap.values()]

  if (mergedMatches.length > 0) {
    const live     = mergedMatches.filter(m => m.statusType === 'live')
    const finished = mergedMatches.filter(m => m.statusType === 'finished')
    const upcoming = mergedMatches.filter(m => m.statusType === 'upcoming')

    const srcLabel = activeSources.length > 0
      ? `${activeSources.join(' + ')} ✅`
      : 'بيانات متعددة ✅'
    ...
    const lines = [
      ...
      ``,
      `> 📡 ${srcBadges || ALL_SRC_BADGES} — ${srcLabel}`,
      ``,
    ]
