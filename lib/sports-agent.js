 ...
 *   ✅ الأولوية للأحداث القادمة ← الحية ← المنتهية
 *
 * المصادر (حسب الأولوية):
 *   365score → FotMob → SofaScore → API-Football → Koora → CAF/FIFA RSS
 * ══════════════════════════════════════════════════════════════════════════════
 */
...
function getMatchTypeLabel(competition = '') {
  ...
  if (c.includes('friendly') || c.includes('ودية') || c.includes('وديّة')) return '🤝 ودية'
 ...
 *
 * يبحث عن مباراة بين فريقين عبر:
 *   - الماضي: 60 يوم
 *   - المستقبل: 90 يوم
 *
 * يُعيد مصفوفة مباريات مرتبة: القادمة أولاً ← المباشرة ← المنتهية
 */
export async function searchMatchAcrossDates(team1 = '', team2 = '', options = {}) {
  const { temporal = 'UNKNOWN', maxPastDays = 60, maxFutureDays = 90 } = options

  const today = new Date()
  const dateList = []

  // بناء قائمة التواريخ حسب التصنيف الزمني
  if (temporal === 'UPCOMING') {
    ...
    // UNKNOWN/LIVE: ابحث في المستقبل القريب أولاً ثم الماضي
    for (let i = 0; i <= 30; i++) {
      const d = new Date(today); d.setDate(d.getDate() + i)
      dateList.push({ str: d.toISOString().slice(0, 10), type: 'future' })
    }
  ...
  // جلب موازٍ من 365score + FotMob لأقرب 7 أيام في كل اتجاه
  // ثم بحث فردي للتواريخ المتبقية
  const coreRange = dateList.slice(0, 15)
  const extendedRange = dateList.slice(15)

  const found = []

  // المرحلة 1: جلب سريع متوازٍ للنطاق الأساسي (15 يوم)
  const corePromises = coreRange.flatMap(({ str }) => [
    fetch365ForDate(str).then(ms => ({ date: str, matches: ms, src: '365score' })),
    fetchFotmobForDate(str).then(ms => ({ date: str, matches: ms, src: 'FotMob' })),
  ])
  ...
  // إذا وجدنا مباريات → لا داعي للبحث الممتد
  if (found.length > 0) {
    return sortMatchResults(found)
  }

  // المرحلة 2: بحث ممتد (أبطأ) في النطاق الموسّع
  const batchSize = 5
  for (let i = 0; i < extendedRange.slice(0, 45).length; i += batchSize) {
