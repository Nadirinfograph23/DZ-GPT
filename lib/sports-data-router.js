...
export async function fetchKooraWC2026(dateStr = '') {
  const d = dateStr || new Date().toISOString().slice(0, 10)
  const cKey = `koora_wc2026_${d}`
  const cached = _KOORA_WC2026_CACHE.get(cKey)
  if (cached) return cached

  const tryUrls = [
    `https://r.jina.ai/${KOORA_WC2026_URL}`,
    `https://r.jina.ai/https://www.kooora.com/?g=1`,
  ]

  for (const jinaUrl of tryUrls) {
    try {
      const res = await timedFetch(jinaUrl, {
      ...
      const matches = []
      // Pattern Arabic: "فريق أ - فريق ب" with optional score
      const patterns = [
        /\|\s*([^\|]{3,30})\s*\|\s*(\d+)\s*[:\-]\s*(\d+)\s*\|\s*([^\|]{3,30})\s*\|/g,
        /([^\n\|]{3,25})\s+(?:vs\.?|ضد|-)\s+([^\n\|]{3,25})(?:\s+(\d+)\s*[\-:]\s*(\d+))?/g,
      ]
      for (const pat of patterns) {
        let m
        pat.lastIndex = 0
        while ((m = pat.exec(text)) !== null && matches.length < 50) {
          const t1 = m[1].trim()
          const t2 = m[2].trim()
          if (t1.length < 2 || t2.length < 2) continue
          if (/^\d+$/.test(t1) || /^\d+$/.test(t2)) continue
          const hasScore = m[3] !== undefined && m[4] !== undefined
          matches.push({
            homeTeam:  t1,
            awayTeam:  t2,
            homeScore: hasScore ? parseInt(m[3]) : null,
            awayScore: hasScore ? parseInt(m[4]) : null,
            statusType: hasScore ? 'finished' : 'upcoming',
            competition: 'كأس العالم FIFA 2026',
            source: 'Kooora',
            link: KOORA_WC2026_URL,
          })
        }
      ...
      _KOORA_WC2026_CACHE.set(cKey, result)
      console.log(`[WC2026:Kooora] ✓ ${matches.length} WC matches parsed`)
      return result
    } catch (err) {
      console.warn('[WC2026:Kooora] failed:', err.message)
    }
  }
  return null
}
...
export async function fetchJdwelWC2026(dateStr = '') {
  ...
  const tryUrls = [
    `https://r.jina.ai/${JDWEL_WC2026_URL}`,
    JDWEL_WC2026_URL,
  ]
  ...
  for (const url of tryUrls) {
    try {
      ...
      const matches = []
      // Parse structured match data from jdwel
      const patterns = [
        /([A-Za-z\u0600-\u06FF][^\n\|]{2,28})\s+(?:vs\.?|ضد)\s+([A-Za-z\u0600-\u06FF][^\n\|]{2,28})(?:[\s\|]+(\d+)\s*[-:]\s*(\d+))?/g,
        /\|\s*([^\|]{3,28})\s*\|\s*(\d+)\s*-\s*(\d+)\s*\|\s*([^\|]{3,28})\s*\|/g,
      ]
      for (const pat of patterns) {
        let m
        pat.lastIndex = 0
        while ((m = pat.exec(text)) !== null && matches.length < 80) {
          const t1 = (m[1] || '').replace(/[*_`#]/g, '').trim()
          const t2 = (pat.source.includes('vs') ? m[2] : m[4] || '').replace(/[*_`#]/g, '').trim()
          const s1 = pat.source.includes('vs') ? m[3] : m[2]
          const s2 = pat.source.includes('vs') ? m[4] : m[3]
          if (!t1 || !t2 || t1.length < 2 || t2.length < 2) continue
          if (/^\d/.test(t1) || /^\d/.test(t2)) continue
          matches.push({
            homeTeam:  t1,
            awayTeam:  t2,
            homeScore: s1 !== undefined ? parseInt(s1) : null,
            awayScore: s2 !== undefined ? parseInt(s2) : null,
            statusType: (s1 !== undefined && s2 !== undefined) ? 'finished' : 'upcoming',
            competition: 'كأس العالم FIFA 2026',
            source: 'jdwel',
      ...
      console.log(`[WC2026:jdwel] ✓ ${matches.length} WC fixtures parsed`)
      return result
    } catch (err) {
