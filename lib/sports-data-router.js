...
// ─────────────────────────────────────────────────────────────
// WC 2026 — FotMob fixtures page scraper (via jina.ai)
// https://www.fotmob.com/ar/leagues/77/fixtures/world-cup?group=by-date
//
// Format returned by jina.ai:
//   ### اليوم
//   [كوريا الجنوبية ![Image 3](...) FT 2 - 1 FT التشيك ![Image 4](...)](match_url)
//   [كندا ![Image 5](...) 7:00م البوسنة والهرسك ![Image 6](...)](match_url)
// ─────────────────────────────────────────────────────────────
...
export async function fetchFotmobWC2026Scraper(dateStr = '') {
  const d = dateStr || new Date().toISOString().slice(0, 10)
  const cKey = `fotmob_wc2026_scraper_${d}`
  ...
  try {
    ...
    // ── تحديد القسم المطلوب: اليوم أو الأمس ──────────────────────────────
    const _dzNow = Date.now() + 3600000 // UTC+1 الجزائر
    const _dzToday = new Date(_dzNow).toISOString().slice(0, 10)
    const _dzYesterday = new Date(_dzNow - 86400000).toISOString().slice(0, 10)
    const _isYesterdayReq = d === _dzYesterday

    let todaySection
    if (_isYesterdayReq) {
      // بحث عن قسم الأمس في FotMob (الأمس / أمس / Yesterday)
      const yestMatch = text.match(/###\s+(?:الأمس|أمس|البارحة|Yesterday)\s*\n([\s\S]*?)(?=###\s|\n---|\n##\s|$)/)
      if (yestMatch) {
        todaySection = yestMatch[1]
      } else {
        // FotMob لا يعرض الأمس أحياناً — نأخذ كامل النص ونترشّح بالـ FT
        todaySection = text
      }
    } else {
      const todayMatch = text.match(/###\s+(?:اليوم|Today)\s*\n([\s\S]*?)(?=###\s|\n---|\n##\s|$)/)
      todaySection = todayMatch ? todayMatch[1] : text
    }

    const matches = []

    // ── مباريات منتهية: [...] FT score - score FT [...](url) ───────────────
    const FINISHED_RE = /\[([^\[!\]]{2,35}?)\s*!\[[^\]]*\]\([^\)]+\)\s+FT\s+(\d+)\s*-\s*(\d+)\s+FT\s+([^\[!\]]{2,35}?)\s*!\[[^\]]*\]\([^\)]+\)\]\((https:\/\/www\.fotmob\.com\/ar\/matches\/[^\)]+)\)/g
    let fm
    FINISHED_RE.lastIndex = 0
    while ((fm = FINISHED_RE.exec(todaySection)) !== null) {
      const homeTeam = fm[1].trim()
      const awayTeam = fm[4].trim()
      if (!homeTeam || !awayTeam || homeTeam.length < 2 || awayTeam.length < 2) continue
      matches.push({
        homeTeam, awayTeam,
        homeScore: parseInt(fm[2]),
        awayScore: parseInt(fm[3]),
        statusType: 'finished',
        startTime: '',
        date: d,
        competition: 'كأس العالم FIFA 2026',
    ...
    // ── مباريات لم تبدأ: [...] time [...](url) ──────────────────────────────
    const UPCOMING_RE = /\[([^\[!\]]{2,35}?)\s*!\[[^\]]*\]\([^\)]+\)\s+(\d+:\d+[صم]?)\s+([^\[!\]]{2,35}?)\s*!\[[^\]]*\]\([^\)]+\)\]\((https:\/\/www\.fotmob\.com\/ar\/matches\/[^\)]+)\)/g
    UPCOMING_RE.lastIndex = 0
    while ((fm = UPCOMING_RE.exec(todaySection)) !== null) {
      const homeTeam = fm[1].trim()
      const awayTeam = fm[3].trim()
      if (!homeTeam || !awayTeam || homeTeam.length < 2 || awayTeam.length < 2) continue
      // تجنب التكرار مع المباريات المنتهية
      const alreadyAdded = matches.some(x =>
        x.homeTeam.toLowerCase() === homeTeam.toLowerCase() ||
        x.awayTeam.toLowerCase() === awayTeam.toLowerCase()
      )
    ...
    if (!matches.length) {
      console.log('[WC2026:FotMob-scraper] no matches found in jina.ai response')
      return null
    }
    ...
    console.log(`[WC2026:FotMob-scraper] ✓ ${matches.length} matches (${result.finished.length} FT, ${result.upcoming.length} upcoming, ${result.live.length} live)`)
    return result
  } catch (err) {
    console.warn('[WC2026:FotMob-scraper] failed:', err.message)
    return null
  }
}
