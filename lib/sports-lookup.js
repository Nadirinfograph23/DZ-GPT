...
async function fetch365ScorePlayer(arabicName, englishName) {
  try {
    ...
    // ⚠️ الخطأ الأصلي: data.competitors = [] (فارغة = truthy في JS!) كانت تحجب data.athletes
    // الإصلاح: دمج الكل معاً — 365score يضع اللاعبين في data.athletes وليس data.competitors
    const allCandidates = [
      ...(data?.athletes    || []),
      ...(data?.competitors || []),
      ...(data?.players     || []),
    ...
    const player = allCandidates.find(p =>
...
export async function universalPlayerSearch(arabicQuery) {
  ...
  const WIKI_HEADERS = { 'User-Agent': 'DZ-GPT/2.0 (https://dz-gpt.vercel.app)' }
