...
const DZ_FACTS = [
  ...
  {
    // ⚠️ هذا النمط مقيّد بعدم وجود كلمات أخبار — لأن "أخبار الاقتصاد" تُعالَج بوحدة RSS المخصصة
    patterns: [/^(?!.*(?:أخبار|خبر|جديد|آخر|اليوم|مستجدات|news|latest|actualité)).*(?:ناتج.*محلي.*الجزائر|PIB.*algérie|GDP.*algeria)/i],
  ...
  // ── المعرفة الإسلامية والعامة ─────────────────────────────────────────────
  // أنماط متسامحة مع الأخطاء الإملائية الشائعة (الأئمة / الأئتمة / الائمة / الأيتمة)
  {
    patterns: [
      /ما.{0,4}هي.{0,12}(الأئت?مة|الأيتمة|الائمة)/i,
      /شنو?.{0,4}(الأئت?مة|الأيتمة|الائمة)/i,
      /معنى.{0,8}(الأئت?مة|الأيتمة|الائمة)/i,
      /ما.{0,4}معنى.{0,8}(الأئت?مة|الأيتمة|الائمة)/i,
      /تعريف.{0,8}(الأئت?مة|الأيتمة|الائمة)/i,
    ],
    answer: `🕌 **الأئمة** (مفردها: إمام) هم القادة والعلماء والمرشدون في الدين الإسلامي.

في الفقه الإسلامي، يُطلق **"الأئمة"** في الغالب على **الأئمة الأربعة** أئمة المذاهب الفقهية السنية:
1. **أبو حنيفة النعمان** — صاحب المذهب الحنفي
2. **مالك بن أنس** — صاحب المذهب المالكي (إمام دار الهجرة)
3. **محمد بن إدريس الشافعي** — صاحب المذهب الشافعي
4. **أحمد بن حنبل** — صاحب المذهب الحنبلي

كما تُطلق كلمة "إمام" على:
• **إمام الصلاة** — من يقود المصلين في الصلاة.
• **أئمة اللغة** — علماء اللغة كالخليل بن أحمد وسيبويه.
• **الأئمة الأربعة عشر عند الشيعة** — والعقيدة السنية لا تعتمد ذلك الاصطلاح.

> 📚 **المصدر:** معارف إسلامية عامة ثابتة (مذاهب فقهية معروفة).`,
  },
  {
    patterns: [
      /الأئت?مة\s+الأربعة/,
      /الأيتمة\s+الأربعة/,
      /الائمة\s+الأربعة/,
      /من\s+هم\s+الأئت?مة\s+الأربعة/,
      /الأئت?مة\s+الاربعة/,
      /المذاهب\s+الفقهية\s+الأربعة/,
      /المذاهب\s+الاربعة/,
      /كم\s+عدد\s+المذاهب/,
      /les\s+quatre\s+imams/,
      /four\s+imams/,
    ],
    answer: `🕌 **الأئمة الأربعة** هم أئمة المذاهب الفقهية الأربعة المشهورة عند أهل السنة والجماعة:
1. **الإمام أبو حنيفة (النعمان بن ثابت)** — مؤسس المذهب **الحنفي** (توفي 150هـ).
2. **الإمام مالك بن أنس** — مؤسس المذهب **المالكي**، مؤلف "الموطأ"، الملقب بإمام دار الهجرة (توفي 179هـ).
3. **الإمام محمد بن إدريس الشافعي** — مؤسس المذهب **الشافعي** (توفي 204هـ).
4. **الإمام أحمد بن حنبل** — مؤسس المذهب **الحنبلي**، صاحب "المسند" (توفي 241هـ).

هذه المذاهب الأربعة هي الركائز الفقهية المعتمدة في العالم الإسلامي، والمشرق العربي والمغاربي (في الجزائر يُعتمد المذهب المالكي غالباً).`,
  },
  {
    patterns: [/ما.{0,4}هو\s+المذهب\s+ال(?:مالكي|حنفي|شافعي|حنبلي)/i, /المذهب\s+ال(?:مالكي|حنفي|شافعي|حنبلي).{0,12}ما/i],
    answer: `🕌 **المذهب المالكي** أحد المذاهب الفقهية الأربعة، أسسه **الإمام مالك بن أنس** في المدينة المنورة، ويُعدّ **المذهب الأغلب في الجزائر والمغرب العربي**.\nيعتمد على: القرآن والسنة، وعمل أهل المدينة، والقياس، والمصالح المرسلة.`,
  },
  {
    patterns: [/ما.{0,4}هي\s+(?:الإمامة|الامامة|الاامامة)/i, /معنى\s+(?:الإمامة|الامامة)/i],
    answer: `🕌 **الإمامة** هي القيادة في الدين أو في الصلاة:
• **إمامة الصلاة**: تقدّم الرجل الصالح للصلاة بالمصلين.
• **الإمامة العظمى (الخلافة)**: القيادة العامة للمسلمين (كما عند أهل السنة).
• **الإمامة الخاصة عند الشيعة**: مقام ديني لعترة النبي ﷺ.\nوفي العلم يُقال "أئمة" لكبار العلماء الحفاظ كأئمة الحديث والفقه واللغة.`,
  },
...
🎬 [شاهد على يوتيوب](https://m.youtube.com/watch?v=gAzvBi4N7ic)`,
  },
  {
    ...
    answer: `🤖 **DZ AGENT** صنعه **نذير حوامرية (Nadir Infograph)** 🇩🇿 — مطوّر ومهندس ذكاء اصطناعي جزائري من عنابة.

🌍 الموقع: [dzagent.app](https://dzagent.app)
...
const GENERAL_FACTS = [
  {
    patterns: [/سكان\s+(?:الأرض|العالم)|عدد\s+سكان\s+(?:الأرض|العالم)|population\s+of\s+(?:the\s+)?world|world\s+population/i],
    answer: '🌍 يُقدَّر عدد سكان الأرض بنحو **8.2 مليار نسمة** وفق تقديرات الأمم المتحدة لعام 2024، ويتغير العدد باستمرار بسبب الولادات والوفيات والهجرة.',
  },
  {
...
// ══════════════════════════════════════════════════════════════════════════════
// TERM DEFINITIONS — بحث موحّد مقاوِم لأخطاء الكتابة العربية
// ══════════════════════════════════════════════════════════════════════════════

/**
 * توحيد العربية للبحث:
 *  - حذف التشكيل والتنوين والتطويل
 *  - توحيد الهمزات (أ إ آ ء ؤ ئ) → ا-و-ي
 *  - تحويل ة → ه
 *  - إسقاط تاء زائدة متطرفة قابلة للخطأ في كلمات مثل الأئت?مة (الأئمة)
 */
function normalizeArabic(text = '') {
  return String(text)
    .replace(/[\u064B-\u0652\u0670\u0640]/g, '')     // تشكيل + تنوين + تطويل
    .replace(/[أإآٱ]/g, 'ا')
    .replace(/[ؤ]/g, 'و')
    .replace(/[ئيى]/g, 'ي')
    .replace(/[ء]/g, '')
...
/**
 * بحث مباشر عن المصطلح في الرسالة بعد التوحيد.
 * يدعم الأخطاء الشائعة: الأئمة / الأئتمة / الائمة / الأيتمة ...
 * بعد التوحيد تصبح كلها: "الايمه" أو "الايتمه"
 */
function matchNormalizedTerm(query = '') {
  const nq = ' ' + normalizeArabic(query) + ' '
  if (nq.length < 6) return null

  // الأئمة (والمشتقات): الأئمة/الائمة → "الايمه" ; الأئتمة/الأيتمة → "الايتمه"
  if (/الايم[ه]|الايتمه|الايمه/.test(nq)) {
    return 'imams'
  }
  return null
}
...
export function lookupStaticFact(query) {
  ...
  // 5. Normalized term lookup — يلتقط الأخطاء الإملائية (الأئتمة...)
  const termMatch = matchNormalizedTerm(q)
  if (termMatch === 'imams') {
    // أعد استخدام إجابة الأئمة من DZ_FACTS دون تكرار
    const imamsEntry = DZ_FACTS.find(f =>
      f.patterns.some(p => /الأئت?مة|الأيتمة|الائمة/i.test(p.source)))
    return imamsEntry ? imamsEntry.answer : null
  }
