...
function parseRss(xml, source) {
  ...
  while ((match = re.exec(xml)) && items.length < MAX_RESULTS) {
    ...
    const link = get('link')
    if (title && link) items.push({ title, url: link.trim(), snippet: '', source, published: get('pubDate') })
  }
  return items
}
...
async function searchWikipedia(query) {
  ...
  try {
    ...
    return (data?.query?.search || []).slice(0, 5).map(item => ({
      ...
      snippet: cleanText(item.snippet), source: 'Wikipedia العربية', published: '',
    }))
  } catch {
    return []
  }
