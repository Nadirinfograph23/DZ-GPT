...
const NON_MAP_REGEXES = [
  ...
  // ── JOBS / CONCOURS / EMPLOYMENT — وظائف ومسابقات ───────────────────
  // "مسابقة توظيف" / "وظائف في وهران" / "concours الجزائر" → NOT a map query
  // Even though wilaya names appear, these are employment searches, not navigation.
  /(?:مسابقة\s+توظيف|توظيف\s+مسابقة|مسابقات\s+توظيف|مسابقة\s+(?:وطنية|ولائية|عامة|في))/i,
  /(?:وظائف|عروض\s+عمل|فرص\s+عمل|عرض\s+عمل)\s+(?:في|ب|بـ)/i,
  /(?:إعلان|إعلانات)\s+(?:توظيف|مسابقة)/i,
  /(?:concours|recrutement|emploi|offre\s+(?:d')?emploi)\s+(?:algérie|algeria|alger|oran|constantine)/i,
  /(?:anem|emploitic|dzmosabakat|emploipartner|dzjobs)/i,
  /(?:شروط|ملف|وثائق)\s+(?:الترشح|التسجيل|المسابقة|الترشيح)/i,
  /(?:تسجيل|الاشتراك|التسجيل)\s+(?:في|ل)\s+(?:مسابقة|توظيف)/i,
  /(?:نتائج|قائمة\s+الناجحين|المقبولين)\s+(?:مسابقة|توظيف)/i,
  // Darija jobs patterns
  /(?:نبحث|نحوس|نبغي|بغيت)\s+(?:على\s+)?(?:خدمة|وظيفة|عمل)/i,
  /(?:كاين|واش\s+كاين)\s+(?:خدمة|وظيفة|مسابقة)/i,
  ...
  // ── RELIGIOUS / ISLAMIC JURISPRUDENCE — حكم وفقه وفتوى ─────────────────
  // "حكم الصلاة" / "واش حكم..." / "ما حكم..." → NOT a map query
  /(?:واش|ما|هل)\s+حكم\s+/i,
  /حكم\s+(?:الصلاة|الزكاة|الصوم|الحج|الطلاق|الزواج|القرآن|الربا|الخمر|الغيبة|التبرج|الموسيقى|الغناء)/i,
  /(?:فقه|فتوى|فتاوى|شريعة|حلال|حرام|مكروه|مستحب|واجب|سنة نبوية|حديث نبوي|تفسير|عقيدة|توحيد|إيمان|عبادة|معاملات)/i,
  /(?:واش|هل)\s+(?:حلال|حرام|جائز|يجوز|مسموح|مباح)\s+/i,
  /(?:ما\s+)?حكم\s+\S/i,
  /كيف\s+(?:أصلي|نصلي|أتوضأ|نتوضأ|أصوم|نصوم|أزكي|نزكي|أحج|نحج)/i,
]
