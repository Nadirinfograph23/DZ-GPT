...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Canonical deployment/version endpoint for Cloudflare production.
      // The old /api/version implementation depended on Vercel environment
      // variables, but dzagent.app is now served by a Cloudflare Worker.
      // Read the exact version.json generated by the deployment workflow so
      // the frontend and Service Worker observe the same deployment marker.
      if (url.pathname === '/api/version' && request.method === 'GET') {
        try {
          const versionRequest = new Request(new URL('/version.json', request.url), {
            method: 'GET',
          ...
          return new Response(JSON.stringify(body), {
            ...
            headers: {
              ...
              'expires': '0',
              'access-control-allow-origin': '*',
              'x-dz-version-source': 'cloudflare-assets-version-json',
            },
          })
        } catch (e) {
          return new Response(JSON.stringify({
            ...
            message: e?.message || 'version endpoint unavailable',
          }), {
            status: 503,
            headers: {
              'content-type': 'application/json; charset=utf-8',
